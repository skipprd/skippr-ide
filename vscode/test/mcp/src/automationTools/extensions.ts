...
export function applyExtensionsTools(server: McpServer, appService: ApplicationService): RegisteredTool[] {
	...
	// 	{
	// 		extensionId: z.string().describe('Extension ID to search for (e.g., "skippr-python.python")')
	// 	},
	// 	async (args) => {
	...
	tools.push(server.tool(
		...
		{
			extensionId: z.string().describe('Extension ID to open (e.g., "skippr-python.python")')
		},
		async (args) => {
			const { extensionId } = args;
	...
	tools.push(server.tool(
		...
		{
			extensionId: z.string().describe('Extension ID to install (e.g., "skippr-python.python")'),
