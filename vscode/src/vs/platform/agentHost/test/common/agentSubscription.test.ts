...
suite('SessionStateSubscription', () => {
	...
	test('ignores actions for different session', () => {
		...
		sub.receiveEnvelope(makeEnvelope(
			{ type: ActionType.SessionTitleChanged, session: 'skippr:///other', title: 'Other' },
			1,
		));
...
suite('AgentSubscriptionManager', () => {
	...
	test('creating session subscription for skippr: URI', async () => {
		const mgr = createManager();
		const mySessionUri = URI.from({ scheme: 'skippr', path: '/my-session' });
	...
	test('getSubscriptionUnmanaged returns undefined when no subscription exists', () => {
		const mgr = createManager();
		const result = mgr.getSubscriptionUnmanaged<SessionState>(URI.parse('skippr:/nonexistent'));
