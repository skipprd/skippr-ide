...
suite('FileEditTracker', () => {
	...
	setup(async () => {
		...
		tracker = instantiationService.createInstance(FileEditTracker, 'skippr:/test-session', db);
	});
	...
	test('tracks edit start and complete for existing file', async () => {
		...
		assert.strictEqual(beforeFields.sessionUri, 'skippr:/test-session');
	...
	test('Write to non-existent file records kind=create with removed=0', async () => {
		...
		const localTracker = inst.createInstance(FileEditTracker, 'skippr:/test-session', db);
...
suite('buildSessionDbUri / parseSessionDbUri', () => {
	...
	test('round-trips a simple URI', () => {
		const uri = buildSessionDbUri('skippr:/abc-123', 'tc-1', '/workspace/file.ts', 'before');
		const parsed = parseSessionDbUri(uri);
		assert.ok(parsed);
		assert.deepStrictEqual(parsed, {
			sessionUri: 'skippr:/abc-123',
			toolCallId: 'tc-1',
	...
	test('round-trips with special characters in filePath', () => {
		const uri = buildSessionDbUri('skippr:/s1', 'tc-2', '/work space/file (1).ts', 'after');
		const parsed = parseSessionDbUri(uri);
		assert.ok(parsed);
	...
	test('round-trips with special characters in toolCallId', () => {
		const uri = buildSessionDbUri('skippr:/s1', 'call_abc=123&x', '/file.ts', 'before');
		const parsed = parseSessionDbUri(uri);
		assert.ok(parsed);
	...
	test('parseSessionDbUri returns undefined for malformed session-db URIs', () => {
		assert.strictEqual(parseSessionDbUri('session-db:skippr:/s1'), undefined);
		assert.strictEqual(parseSessionDbUri('session-db:skippr:/s1?toolCallId=tc-1'), undefined);
		assert.strictEqual(parseSessionDbUri('session-db:skippr:/s1?toolCallId=tc-1&filePath=/f&part=middle'), undefined);
	});

	test('URI path ends with the basename of the file', () => {
		const uri = buildSessionDbUri('skippr:/s1', 'tc-1', '/workspace/src/index.ts', 'before');
		const parsed = URI.parse(uri);
		assert.ok(parsed.path.endsWith('/index.ts'));
	});

	test('URI path ends with basename for files with spaces and special chars', () => {
		const uri = buildSessionDbUri('skippr:/s1', 'tc-1', '/work space/file (1).ts', 'after');
		const parsed = URI.parse(uri);
