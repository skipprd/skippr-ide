...
import { mapSessionEventsToHistoryRecords, type IHistoryRecord as ISessionEvent } from './historyRecordFixtures.js';
import { createNoopGitService, createSessionDataService } from '../common/sessionTestHelpers.js';

/**
 * Loads a JSONL fixture of raw Skippr SDK events, runs them through
 ...
 * `events.jsonl` files from `~/.skippr/session-state/`.
 */
...
suite('AgentService (node dispatcher)', () => {
	...
	setup(async () => {
		...
		skipprAgent = new MockAgent('skippr');
		disposables.add(toDisposable(() => skipprAgent.dispose()));
	});
	...
	suite('registerProvider', () => {
		...
		test('registers a provider successfully', () => {
			service.registerProvider(skipprAgent);
			// No throw - success
		});

		test('throws on duplicate provider registration', () => {
			service.registerProvider(skipprAgent);
			const duplicate = new MockAgent('skippr');
		...
		test('maps progress events to protocol actions via onDidAction', async () => {
			service.registerProvider(skipprAgent);
			const session = await service.createSession({ provider: 'skippr' });
	...
	suite('dispatchAction', () => {
		...
		test('applies and persists root config changes from clients', async () => {
			...
			try {
				...
				const agent = new MockAgent('skippr');
	...
	suite('user-message attachment rewriting', () => {
		...
		async function setup(): Promise<{
			...
			disposables.add(toDisposable(() => agent.dispose()));
			svc.registerProvider(agent);
			const session = await svc.createSession({ provider: 'skippr' });
	...
	suite('createSession', () => {
		...
		test('honors requested session URI', async () => {
			...
			const requestedSession = AgentSession.uri('skippr', 'requested-session');
			const session = await service.createSession({ provider: 'skippr', session: requestedSession });
	...
	suite('disposeSession', () => {
		...
		test('dispatches to the correct provider and cleans up tracking', async () => {
			...
			assert.strictEqual(skipprAgent.disposeSessionCalls.length, 1);
		});
	...
	suite('aggregation', () => {
		...
		test('listSessions aggregates sessions from all providers', async () => {
			...
			await service.createSession({ provider: 'skippr' });
		...
		test('listSessions overlays custom title from session database', async () => {
			...
			const sessionUri = AgentSession.uri('skippr', sessionId);
		...
		test('listSessions uses SDK title when no custom title exists', async () => {
			service.registerProvider(skipprAgent);
			skipprAgent.sessionMetadataOverrides = { summary: 'Auto-generated Title' };
		...
		test('createSession attaches git state into state _meta when working directory is present', async () => {
			...
			const session = await localService.createSession({ provider: 'skippr' });
		...
		test('createSession stores live session config', async () => {
			...
			const session = await service.createSession({ provider: 'skippr', config });
		...
		test('seeds activeClient into the initial session state when provided', async () => {
			...
			const session = await service.createSession({ provider: 'skippr', activeClient });
	...
	suite('authenticate', () => {
		...
		test('routes token to provider matching the resource', async () => {
			...
			assert.deepStrictEqual(skipprAgent.authenticateCalls, [{ resource: 'https://api.github.com', token: 'ghp_test123' }]);
		});
		...
		test('returns not authenticated for unknown resource', async () => {
			...
			assert.strictEqual(skipprAgent.authenticateCalls.length, 0);
		});

		test('fans out to every provider that owns the resource', async () => {
			// Two providers share the same protected resource (the real
			// motivating example: both Skippr Agent and Claude consume the
			// Skippr Data Agent token). Both must see the token — the
			...
			assert.deepStrictEqual({
				result,
				skipprCalls: skipprAgent.authenticateCalls,
				claudeCalls: claudeAgent.authenticateCalls,
			}, {
				result: { authenticated: true },
				skipprCalls: [{ resource: 'https://api.github.com', token: 'tok' }],
	...
	suite('shutdown', () => {
		...
		test('shuts down all providers', async () => {
			let skipprShutdown = false;
			skipprAgent.shutdown = async () => { skipprShutdown = true; };

			service.registerProvider(skipprAgent);

			await service.shutdown();
			assert.ok(skipprShutdown);
		});
	});
	...
	suite('restoreSession', () => {
		...
		test('restores a session with message history', async () => {
			service.registerProvider(skipprAgent);
			const { session } = await skipprAgent.createSession();
			const sessions = await skipprAgent.listSessions();
			const sessionResource = sessions[0].session;

			skipprAgent.sessionMessages = [
		...
		test('throws when session is not found on backend', async () => {
			...
			await assert.rejects(
				() => service.restoreSession(AgentSession.uri('skippr', 'nonexistent')),
		...
		test('restores known session without listing all provider sessions', async () => {
			...
			skipprAgent.listSessions = async () => {
		...
		test('falls back to listing sessions when direct metadata restore fails', async () => {
			...
			skipprAgent.getSessionMetadata = async () => {
				throw new Error('direct metadata unavailable');
			};
			const originalListSessions = skipprAgent.listSessions.bind(skipprAgent);
			let listSessionsCalled = false;
			skipprAgent.listSessions = async () => {
				listSessionsCalled = true;
		...
		test('inner assistant messages from subagent do not create extra turns (fixture)', async () => {
			...
			// Load real SDK events from fixture (sanitized from ~/.skippr/session-state/)
			skipprAgent.sessionMessages = await loadFixtureMessages('subagent-session.jsonl', session);
	...
	suite('subscriber refcount eviction', () => {
		...
		test('an empty session created in this lifetime stays observable until GC fires', async () => {
			service.registerProvider(skipprAgent);
			const sessionResource = await service.createSession({ provider: 'skippr' });
	...
	suite('empty-session GC', () => {
		...
		test('an empty unsubscribed session is disposed after the grace period', () => {
			return runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				assert.strictEqual(skipprAgent.disposeSessionCalls.length, 0, 'no GC before grace expires');

				// After the grace period, the session is disposed entirely.
				await new Promise(resolve => setTimeout(resolve, 30_000));
				assert.deepStrictEqual(
					skipprAgent.disposeSessionCalls.map(u => u.toString()),
		...
		test('a session with at least one turn is not GC-disposed', () => {
			return runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				assert.strictEqual(skipprAgent.disposeSessionCalls.length, 0, 'session with turns must not be GC-disposed');
			});
		});
		...
		test('resubscribe within the grace period cancels GC', () => {
			return runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				assert.strictEqual(skipprAgent.disposeSessionCalls.length, 0, 'GC must be cancelled after resubscribe');
			});
		});
		...
		test('GC is rearmed after a resubscribe-then-unsubscribe cycle', () => {
			return runWithFakedTimers({ useFakeTimers: true }, async () => {
				...
				assert.strictEqual(skipprAgent.disposeSessionCalls.length, 0, 'rearmed timer not yet fired');
				await new Promise(resolve => setTimeout(resolve, 2_000));
				assert.strictEqual(skipprAgent.disposeSessionCalls.length, 1, 'rearmed timer fires after fresh 30s');
			});
		});
		...
		test('createSession on the same URI cancels a pending GC', () => {
			...
			return runWithFakedTimers({ useFakeTimers: true }, async () => {
				service.registerProvider(skipprAgent);
				const sessionResource = await service.createSession({ provider: 'skippr', session: AgentSession.uri('skippr', 'recreate-test') });
				service.addSubscriber(sessionResource, 'client-1');
				service.unsubscribe(sessionResource, 'client-1');

				// Re-issue createSession mid-grace.
				await new Promise(resolve => setTimeout(resolve, 5_000));
				await service.createSession({ provider: 'skippr', session: AgentSession.uri('skippr', 'recreate-test') });

				// Wait past the original grace window. If GC wasn't
				// cancelled by createSession, dispose would have fired.
				await new Promise(resolve => setTimeout(resolve, 30_000));
				assert.strictEqual(skipprAgent.disposeSessionCalls.length, 0, 'createSession on same URI must cancel pending GC');
			});
		});
	});
	...
	suite('session config persistence', () => {
		...
		test('createSession persists initial config values to the session DB', async () => {
			...
			const localAgent = new MockAgent('skippr');
			...
			await localService.createSession({ provider: 'skippr', config: { autoApprove: 'autoApprove' } });
		...
		test('createSession does not write configValues when there are no values', async () => {
			...
			await localService.createSession({ provider: 'skippr' });
		...
		test('createSession + restoreSession round-trip restores initial config without any mid-session changes', async () => {
			...
			const session = await localService.createSession({ provider: 'skippr', config: { autoApprove: 'autoApprove' } });
	...
	suite('worktree working directory', () => {
		...
		test('createSession uses agent-resolved working directory in state', async () => {
			...
			skipprAgent.resolvedWorkingDirectory = worktreeDir;
			service.registerProvider(skipprAgent);

			const sourceDir = URI.file('/source/repo');
			const session = await service.createSession({ provider: 'skippr', workingDirectory: sourceDir });
		...
		test('createSession falls back to config working directory when agent does not resolve', async () => {
			...
			skipprAgent.resolvedWorkingDirectory = undefined;
			service.registerProvider(skipprAgent);
		...
		test('restoreSession uses agent working directory in state', async () => {
			...
			const worktreeDir = URI.file('/source/repo.worktrees/agents-xyz');
			skipprAgent.sessionMetadataOverrides = { workingDirectory: worktreeDir };
			service.registerProvider(skipprAgent);
