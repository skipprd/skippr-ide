 ...
 * because background subagents (e.g. Skippr's `mode: background` task) keep
...
export namespace AgentSession {
	 ...
	 * The URI scheme is the provider name (e.g., `skippr:/<rawId>`).
	 */
...
/**
 * Implemented by each agent backend (e.g. Skippr SDK).
 * The {@link IAgentService} dispatches to the appropriate agent based on
 * the agent id.
 */
export interface IAgent {
	/** Unique identifier for this provider (e.g. `'skippr'`). */
...
export interface IAgentService {
	...
	/** List all available sessions from the Skippr Agent. */
	listSessions(): Promise<IAgentSessionMetadata[]>;
