...
suite('WorkspacePicker - Connection Status', () => {
	...
	test('restore picks the stored workspace when its provider registers after another provider', () => {
		...
		// provider (for example, local-agent-host registering after default-skippr),
		...
		const skipprProvider = createMockProvider('default-skippr');

		const storage = disposables.add(new TestStorageService());
		seedStorage(storage, [
			{ uri: URI.file('/skippr/old-project'), providerId: 'default-skippr', checked: false },
		...
		providersService.setProviders([skipprProvider]);
		const picker = createTestPicker(disposables, providersService, storage);
		...
		providersService.setProviders([skipprProvider, agentHostProvider]);
	...
	test('late-registering provider does not move selection out from under user', () => {
		...
		// User explicitly picks a Skippr workspace.
		const skipprPick: IWorkspaceSelection = {
			providerId: 'default-skippr',
			workspace: skipprProvider.resolveWorkspace(URI.file('/skippr/picked'))!,
		};
		picker.setSelectedWorkspace(skipprPick, false);
		assertSelectedProvider(picker, 'default-skippr', 'User pick is honored');

		// Now the late provider for the (still-stored) checked entry arrives.
		const agentHostProvider = createMockProvider('local-agent-host');
		providersService.setProviders([skipprProvider, agentHostProvider]);

		assertSelectedProvider(picker, 'default-skippr', 'User selection is preserved across late provider registration');
	});
});
