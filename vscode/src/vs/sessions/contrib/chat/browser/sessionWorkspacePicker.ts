...
export class WorkspacePicker extends Disposable {
	...
	private async _loadVSCodeRecentFolders(): Promise<void> {
		...
		this._vsCodeRecentFolderUris = recentlyOpened.workspaces
			...
			.map(f => f.folderUri)
			.filter(uri => !this._isSkipprWorktree(uri))
			.slice(0, 10);
	}

	/**
	 * Returns whether the given URI points to a skippr-managed folder
	 * (a folder whose name starts with `skippr-`).
	 */
	private _isSkipprWorktree(uri: URI): boolean {
		return basename(uri).startsWith('skippr-');
	}
