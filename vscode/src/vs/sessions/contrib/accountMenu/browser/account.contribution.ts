...
class TitleBarAccountWidget extends BaseActionViewItem {
	...
	private readonly skipprDashboardStore = this._register(new MutableDisposable<DisposableStore>());
	...
	private createCombinedPanelContent(panelStore: DisposableStore): HTMLElement {
		...
		if (this.shouldShowSkipprDashboardHover()) {
			...
			const dashboard = this.createSkipprHoverContent({ compactQuotaLayout: true, ctaButtonsContainer });
	...
	private shouldShowSkipprDashboardHover(): boolean {
		return !this.chatEntitlementService.sentiment.hidden && !!this.accountName;
	}

	private createSkipprHoverContent(extraOptions?: Partial<IChatStatusDashboardOptions>): HTMLElement {
		const store = new DisposableStore();
		this.skipprDashboardStore.value = store;
