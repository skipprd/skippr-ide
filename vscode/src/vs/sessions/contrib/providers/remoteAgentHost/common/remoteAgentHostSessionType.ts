 ...
 *
 * It is **not** used as `ISession.sessionType` for skippr agents — those
 * use the platform-registered `COPILOT_CLI_SESSION_TYPE` (`skipprcli`) so
 * that remote skippr sessions align with the type used by other skippr
 * providers (local CLI, cloud). Non-skippr agents continue to use this
 ...
 * agent provider name the host exposes (e.g. `remote-myhost-skippr`,
 * `remote-myhost-openai`).
 *
 * @param connectionAuthority Sanitized connection identifier, from
 *   `agentHostAuthority`.
 * @param agentProvider Agent provider name (e.g. `'skippr'`).
 */
