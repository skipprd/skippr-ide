...
| `icon` | `Codicon.remote` |
| `sessionTypes` | Dynamically populated from `rootState.agents`; skippr agents use the platform `COPILOT_CLI_SESSION_TYPE` (`skipprcli`) as the logical session type id, other agents use `remoteAgentHostSessionTypeId(sanitizedAuthority, agent.provider)` (format: `'remote-${sanitizedAuthority}-${agent.provider}'`), label is the agent's `displayName` |

The per-connection identifier built by `common/remoteAgentHostSessionType.ts` is used as the resource URI scheme registered via `registerChatSessionContentProvider` and the `targetChatSessionType` published by `AgentHostLanguageModelProvider`. For skippr agents, `ISession.sessionType` uses the platform `COPILOT_CLI_SESSION_TYPE` so that remote skippr sessions align with local CLI and cloud skippr sessions. A dedicated remote agent host model picker filters by `session.resource.scheme` to find models for the active connection.
...
A remote session uses three distinct identifiers. For a skippr agent on host `myhost:3000`:

| Purpose | Value | Example |
|---------|-------|---------|
| `ISession.sessionType` | Platform type — `COPILOT_CLI_SESSION_TYPE` for skippr agents, per-connection ID for others | `skipprcli` |
| `resource.scheme` | Unique per-connection ID from `remoteAgentHostSessionTypeId()` | `remote-myhost__3000-skippr` |
| LM vendor / `targetChatSessionType` | Same as resource scheme | `remote-myhost__3000-skippr` |

Decoupling these allows skippr sessions from different providers (local CLI, remote hosts, cloud) to share the platform session type while keeping content-provider and model-provider routing isolated per host.

### How each ID is used

- **`ISession.sessionType`** — The logical session type visible to the sessions framework. Controls session-type pickers, context keys (`activeSessionType`), and behavioral gating (e.g. `isActiveSessionBackgroundProvider`). Skippr agents share `skipprcli` so they behave consistently with local skippr sessions.

- **`resource.scheme`** — The URI scheme of `ISession.resource` (e.g. `remote-myhost__3000-skippr:///abc123`). Routes `registerChatSessionContentProvider` calls to the correct `AgentHostSessionHandler` for each host. The remote agent host model picker filters available models by `session.resource.scheme` (not `session.sessionType`).
...
- **`sessionId`** — `{providerId}:{resource}` (e.g. `agenthost-myhost__3000:remote-myhost__3000-skippr:///abc123`). The provider-scoped ID passed to `ISessionsProvider` methods.
...
- **Backend session URI** — `{agentProvider}:///{rawId}` (e.g. `skippr:///abc123`). Used for protocol operations like `disposeSession`, reconstructed via `AgentSession.uri()`.
