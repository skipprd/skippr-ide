...
suite('remoteAgentHostSessionType', () => {
	...
	// Note: `ISession.sessionType` for skippr agents uses the platform
	// `COPILOT_CLI_SESSION_TYPE` instead of this per-connection value.
	test('remoteAgentHostSessionTypeId pins the wire format', () => {
		assert.strictEqual(remoteAgentHostSessionTypeId('foo', 'skippr'), 'remote-foo-skippr');
		assert.strictEqual(remoteAgentHostSessionTypeId('10.0.0.1__8080', 'skippr'), 'remote-10.0.0.1__8080-skippr');
