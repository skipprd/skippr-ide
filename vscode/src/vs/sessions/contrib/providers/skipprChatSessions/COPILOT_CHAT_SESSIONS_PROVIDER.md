# SkipprChatSessionsProvider — Default Skippr Provider

**File:** `src/vs/sessions/contrib/skipprChatSessions/browser/skipprChatSessionsProvider.ts`

The default sessions provider, registered with ID `'default-skippr'`. Wraps the existing agent session infrastructure into the extensible provider model. Supports three session types: **Skippr Agent** (local), **Skippr Cloud** (remote), and **Claude** (local, gated by `sessions.chat.claudeAgent.enabled`).
...
```
src/vs/sessions/contrib/skipprChatSessions/browser/skipprChatSessions.contribution.ts
```

```typescript
class DefaultSessionsProviderContribution extends Disposable {
    constructor(instantiationService, sessionsProvidersService) {
        const provider = instantiationService.createInstance(SkipprChatSessionsProvider);
...
|----------|-------|
| `id` | `'default-skippr'` |
| `label` | `'Skippr Data Agent'` |
| `icon` | `Codicon.skippr` |
| `sessionTypes` | `[SkipprCLISessionType, SkipprCloudSessionType]` (+ `ClaudeCodeSessionType` when enabled) |

## Browse Actions

- **"Folders"** — Opens a folder dialog; creates a workspace with a `file://` URI
- **"Repositories"** — Executes `skippr.data-agent.chat.cloudSessions.openRepository`; creates a workspace with a `github-remote-file://` URI
...
**`SkipprCLISession`** — For local `file://` workspaces:
...
1. Validate the session is a current new session (`SkipprCLISession`, `RemoteNewSession`, or `ClaudeCodeNewSession`)
...
**File:** `src/vs/sessions/contrib/skipprChatSessions/browser/skipprChatSessionsActions.ts`
...
| Expression | Matches |
|------------|---------|
| `IsActiveSessionSkipprChatCLI` | Skippr Agent sessions |
| `IsActiveSessionSkipprChatCloud` | Skippr Cloud sessions |
| `IsActiveSessionSkipprChatClaudeCode` | Claude sessions |
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            id: 'sessions.defaultSkippr.myPicker',
            ...
            menu: [{
                ...
                group: 'navigation',
                order: 1,
                when: IsActiveSessionSkipprChatCLI, // gate to session type
            }],
        });
    }
...
// 2. Register the action view item (in SkipprPickerActionViewItemContribution)
this._register(actionViewItemService.register(
    Menus.NewSessionControl, 'sessions.defaultSkippr.myPicker',
    () => {
...
The picker model is currently **hardcoded per session type**. Each session type that needs pickers must register its own actions and widgets with appropriate `when` clauses. For example, the Skippr Agent permission picker (`PermissionPicker`) and the Claude permission mode picker (`ClaudePermissionModePicker`) are separate, hardcoded widgets even though they serve a similar purpose.

Context-menu actions on session list items are similarly hardcoded per session type. The `Delete...` action registered for `SessionItemContextMenuId` gates on both `chatSessionProviderId == COPILOT_PROVIDER_ID` *and* `chatSessionType != CLAUDE_CODE_SESSION_TYPE`, because Claude sessions (although exposed through the Skippr provider) don't support the native delete flow. Any new session type that opts into the Skippr provider but not into a shared action needs its own `chatSessionType` exclusion in the action's `when` clause.
