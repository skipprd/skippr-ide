...
import { SkipprChatSessionsProvider, ISkipprChatSession } from './skipprChatSessionsProvider.js';
...
export class BranchPicker extends Disposable {
	...
	private _getSession(): ISkipprChatSession | undefined {
		const session = this.sessionsManagementService.activeSession.get();
		if (!session) {
			return undefined;
		}
		const provider = this.sessionsProvidersService.getProvider(session.providerId);
		return provider instanceof SkipprChatSessionsProvider ? provider.getSession(session.sessionId) : undefined;
	}
