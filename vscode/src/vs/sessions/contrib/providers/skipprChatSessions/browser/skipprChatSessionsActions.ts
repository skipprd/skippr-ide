...
import { ClaudeCodeSessionType, COPILOT_PROVIDER_ID, SkipprChatSessionsProvider, SkipprCloudSessionType, LocalSessionType } from './skipprChatSessionsProvider.js';
import { IsolationPicker } from './isolationPicker.js';
import { ModePicker } from './modePicker.js';
import { CloudModelPicker } from './modelPicker.js';
import { SkipprPermissionPickerDelegate, PermissionPicker } from './permissionPicker.js';
import { SessionType } from '../../../../../workbench/contrib/chat/common/chatSessionsService.js';
import { reportNewChatPickerClosed } from '../../../chat/browser/newChatPickerTelemetry.js';
import { SkipprCLISessionType } from '../../agentHost/browser/baseAgentHostSessionsProvider.js';

const IsActiveSessionSkipprCLI = ContextKeyExpr.equals(ActiveSessionTypeContext.key, SkipprCLISessionType.id);
const IsActiveSessionSkipprCloud = ContextKeyExpr.equals(ActiveSessionTypeContext.key, SkipprCloudSessionType.id);
const IsActiveSessionLocal = ContextKeyExpr.equals(ActiveSessionTypeContext.key, LocalSessionType.id);
const IsActiveSkipprChatSessionProvider = ContextKeyExpr.equals(ActiveSessionProviderIdContext.key, COPILOT_PROVIDER_ID);
const IsActiveSessionSkipprChatCLI = ContextKeyExpr.and(IsActiveSessionSkipprCLI, IsActiveSkipprChatSessionProvider);
const IsActiveSessionSkipprChatCloud = ContextKeyExpr.and(IsActiveSessionSkipprCloud, IsActiveSkipprChatSessionProvider);
const IsActiveSessionClaudeCode = ContextKeyExpr.equals(ActiveSessionTypeContext.key, ClaudeCodeSessionType.id);
const IsActiveSessionSkipprChatClaudeCode = ContextKeyExpr.and(IsActiveSessionClaudeCode, IsActiveSkipprChatSessionProvider);
const IsActiveSessionSkipprChatLocal = ContextKeyExpr.and(IsActiveSessionLocal, IsActiveSkipprChatSessionProvider);
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: 'sessions.defaultSkippr.isolationPicker',
			...
			menu: [{
				...
				when: ContextKeyExpr.and(
					...
					IsActiveSessionSkipprChatCLI,
					ContextKeyExpr.equals('config.skippr.data-agent.chat.cli.isolationOption.enabled', true),
				),
			}],
		});
	}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: 'sessions.defaultSkippr.branchPicker',
			...
			menu: [{
				...
				group: 'navigation',
				order: 2,
				when: ContextKeyExpr.and(IsNewChatSessionContext, IsActiveSessionSkipprChatCLI),
			}],
		});
	}
	override async run(): Promise<void> { /* handled by action view item */ }
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: 'sessions.defaultSkippr.modePicker',
			...
			menu: [{
				...
				group: 'navigation',
				order: 0,
				when: ContextKeyExpr.or(IsActiveSessionSkipprChatCLI, IsActiveSessionSkipprChatLocal),
			}],
		});
	}
	override async run(): Promise<void> { /* handled by action view item */ }
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: 'sessions.defaultSkippr.localModelPicker',
			...
			menu: [{
				...
				group: 'navigation',
				order: 1,
				when: ContextKeyExpr.or(IsActiveSessionSkipprChatCLI, IsActiveSessionSkipprChatClaudeCode, IsActiveSessionSkipprChatLocal),
			}],
		});
	}
	override async run(): Promise<void> { /* handled by action view item */ }
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: 'sessions.defaultSkippr.cloudModelPicker',
			...
			menu: [{
				...
				group: 'navigation',
				order: 1,
				when: IsActiveSessionSkipprChatCloud,
			}],
		});
	}
	override async run(): Promise<void> { /* handled by action view item */ }
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: 'sessions.defaultSkippr.permissionPicker',
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: 'sessions.defaultSkippr.claudePermissionModePicker',
			...
			menu: [{
				...
				group: 'navigation',
				order: 1,
				when: IsActiveSessionSkipprChatClaudeCode,
			}],
		});
	}
	override async run(): Promise<void> { /* handled by action view item */ }
 ...
 *
 * Exported so the web-only `SkipprPermissionPickerWebContribution`
...
class SkipprPickerActionViewItemContribution extends Disposable implements IWorkbenchContribution {

	static readonly ID = 'workbench.contrib.skipprPickerActionViewItems';
	...
	constructor(
		...
		this._register(actionViewItemService.register(
			Menus.NewSessionRepositoryConfig, 'sessions.defaultSkippr.isolationPicker',
			() => {
		...
		this._register(actionViewItemService.register(
			Menus.NewSessionRepositoryConfig, 'sessions.defaultSkippr.branchPicker',
			() => {
		...
		this._register(actionViewItemService.register(
			Menus.NewSessionConfig, 'sessions.defaultSkippr.modePicker',
			() => {
		...
		this._register(actionViewItemService.register(
			Menus.NewSessionConfig, 'sessions.defaultSkippr.localModelPicker',
			() => {
		...
		this._register(actionViewItemService.register(
			Menus.NewSessionConfig, 'sessions.defaultSkippr.cloudModelPicker',
			() => {
		...
		// web-only `SkipprPermissionPickerWebContribution` (registered
		...
		if (!isWeb) {
			this._register(actionViewItemService.register(
				Menus.NewSessionControl, 'sessions.defaultSkippr.permissionPicker',
				() => {
		...
		this._register(actionViewItemService.register(
			Menus.NewSessionControl, 'sessions.defaultSkippr.claudePermissionModePicker',
			() => {
...
class SkipprActiveSessionContribution extends Disposable implements IWorkbenchContribution {

	static readonly ID = 'workbench.contrib.skipprActiveSession';
	...
	constructor(
		...
		this._register(autorun((reader: IReader) => {
			...
			if (session?.providerId === COPILOT_PROVIDER_ID) {
				...
				const providerSession = provider instanceof SkipprChatSessionsProvider ? provider.getSession(session.sessionId) : undefined;
...
registerWorkbenchContribution2(SkipprPickerActionViewItemContribution.ID, SkipprPickerActionViewItemContribution, WorkbenchPhase.AfterRestored);
registerWorkbenchContribution2(SkipprActiveSessionContribution.ID, SkipprActiveSessionContribution, WorkbenchPhase.AfterRestored);
 ...
 */
class SkipprSessionContextMenuBridge extends Disposable implements IWorkbenchContribution {
	static readonly ID = 'skipprChatSessions.contextMenuBridge';
	...
	private _bridgeItems(): void {
		...
		for (const item of items) {
			...
			if (!commandId.startsWith('skippr.data-agent.')) {
				continue;
			}
			if (commandId === 'skippr.data-agent.cli.sessions.delete') {
...
registerWorkbenchContribution2(SkipprSessionContextMenuBridge.ID, SkipprSessionContextMenuBridge, WorkbenchPhase.AfterRestored);

registerAction2(class DeleteSessionAction extends Action2 {
	constructor() {
		super({
			id: 'sessionsViewPane.skippr.deleteSession',
