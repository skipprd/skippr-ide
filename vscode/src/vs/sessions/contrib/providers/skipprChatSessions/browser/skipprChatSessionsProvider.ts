...
import { SkipprCLISessionType } from '../../agentHost/browser/baseAgentHostSessionsProvider.js';
...
/** Skippr Cloud session type - cloud-hosted agent. */
export const SkipprCloudSessionType: ISessionType = {
	id: 'skippr-cloud-agent',
	label: localize('skipprCloud', "Cloud"),
	icon: Codicon.cloud,
};
...
export interface ISkipprChatSession {
	...
	/** Session type ID (e.g., 'skippr-cli', 'skippr-cloud', 'local'). */
...
const OPEN_REPO_COMMAND = 'skippr.data-agent.chat.cloudSessions.openRepository';

/** Provider ID for the Skippr Data Agent Sessions provider. */
export const COPILOT_PROVIDER_ID = 'default-skippr';

/** Setting key controlling whether the Skippr provider supports multiple chats per session. */
export const COPILOT_MULTI_CHAT_SETTING = 'sessions.skippr.data-agent.multiChatSessions';
...
type NewSession = SkipprCLISession | RemoteNewSession | ClaudeCodeNewSession | LocalNewSession;

function isNewSession(session: ISkipprChatSession): session is NewSession {
	return session instanceof SkipprCLISession || session instanceof RemoteNewSession || session instanceof ClaudeCodeNewSession || session instanceof LocalNewSession;
}

/**
 * Local new session for Background agent sessions.
 * Implements {@link ISkipprChatSession} (session facade) and provides
 * pre-send configuration methods for the new-session flow.
 */
class SkipprCLISession extends Disposable implements ISkipprChatSession {

	static readonly COPILOT_WORKTREE_PATTERN = 'skippr-worktree-';

	// -- ISessionData fields --

	readonly id: string;
	readonly providerId: string;
	readonly sessionType: typeof SessionType.SkipprCLI;
	readonly icon: ThemeIcon;
	readonly createdAt: Date;
	...
	constructor(
		...
		this.icon = SkipprCLISessionType.icon;
	...
	private _loadBranches(repo: IGitRepository): void {
		...
		repo.getRefs({ pattern: 'refs/heads' }, cts.token).then(refs => {
			...
			const branches = refs
				...
				.filter(name => !name.includes(SkipprCLISession.COPILOT_WORKTREE_PATTERN));
 ...
 * pre-send configuration methods for the new-session flow.
 */
export class RemoteNewSession extends Disposable implements ISkipprChatSession {
	...
	constructor(
		...
		this.icon = SkipprCloudSessionType.icon;
		this.createdAt = new Date();
 ...
 * Implements {@link ISkipprChatSession} (session facade) for local sessions
 ...
 */
class LocalNewSession extends Disposable implements ISkipprChatSession {
	...
	constructor(
		...
		const modelRef = this._register(this.chatService.startNewLocalSession(
			...
			{ debugOwner: 'SkipprChatSessionsProvider#createNewSession.local' },
		));
 ...
 * Implements {@link ISkipprChatSession} (session facade) and provides
 * pre-send configuration methods for the new-session flow.
 * Simpler than {@link SkipprCLISession} because the Claude agent manages
 * its own worktrees and branches at runtime.
 */
class ClaudeCodeNewSession extends Disposable implements ISkipprChatSession {
...
/**
 * Adapts an existing {@link IAgentSession} from the chat layer into the new {@link ISkipprChatSession} facade.
 */
class AgentSessionAdapter implements ISkipprChatSession {
	...
	private _getSessionTypeIcon(session: IAgentSession): ThemeIcon {
		switch (session.providerType) {
			case AgentSessionProviders.Background:
				return SkipprCLISessionType.icon;
			case AgentSessionProviders.Cloud:
				return SkipprCloudSessionType.icon;
...
/**
 * Default sessions provider for Skippr Agent, Cloud, Claude, and Local session types.
 * Wraps the existing session infrastructure into the extensible provider model.
 */
export class SkipprChatSessionsProvider extends Disposable implements ISessionsProvider {

	readonly id = COPILOT_PROVIDER_ID;
	readonly label = localize('skipprChatSessionsProvider', "Skippr Data Agent");
	readonly icon = Codicon.skippr;
	get sessionTypes(): readonly ISessionType[] {
		const types: ISessionType[] = [SkipprCLISessionType, SkipprCloudSessionType];
	...
	private readonly _sessionCache = new Map<string, AgentSessionAdapter | SkipprCLISession | RemoteNewSession | ClaudeCodeNewSession | LocalNewSession>();
	...
	private _chatByRawSessionIdCache: Map<string, ISkipprChatSession> | undefined;
	...
	getSessionTypes(workspaceUri: URI): ISessionType[] {
		if (workspaceUri.scheme === GITHUB_REMOTE_FILE_SCHEME || workspaceUri.scheme === SessionType.SkipprCloud) {
			return [SkipprCloudSessionType];
		}
		const types: ISessionType[] = [SkipprCLISessionType];
		if (this._localSessionEnabled) {
			types.push(LocalSessionType);
		}
		if (this._claudeEnabled) {
	...
	getSession(sessionId: string): ISkipprChatSession | undefined {
	...
	createNewSession(workspaceUri: URI, sessionTypeId: string): ISession {
		...
		if (workspaceUri.scheme === GITHUB_REMOTE_FILE_SCHEME) {
			if (sessionTypeId !== SkipprCloudSessionType.id) {
				throw new Error('Only Skippr Cloud sessions can be created for GitHub repositories');
			}
		...
		if (sessionTypeId !== SkipprCLISessionType.id) {
		...
		const session = this.instantiationService.createInstance(SkipprCLISession, resource, workspace, this.id);
		this._currentNewSession = session;
		return this._chatToSession(session);
	}
	...
	async renameChat(sessionId: string, chatUri: URI, title: string): Promise<void> {
		...
		if (agentSession?.providerType === SkipprCLISessionType.id) {
			await this.commandService.executeCommand('skippr.data-agent.cli.sessions.setTitle', { resource: chatUri }, title);
			return;
		}
		if (agentSession?.providerType === AgentSessionProviders.Claude) {
			await this.commandService.executeCommand('skippr.data-agent.claude.sessions.rename', { resource: chatUri }, title);
			return;
		}
	...
	private async _deleteAgentSessions(agentSessions: IAgentSession[]): Promise<void> {
		...
		if (cliSessionItems.length > 0) {
			await this.commandService.executeCommand('agents.skippr.data-agent.cli.deleteSessions', cliSessionItems, { skipConfirmation: true });
		}
	}
	...
	async sendRequest(sessionId: string, chatResource: URI, options: ISendRequestOptions): Promise<ISession> {
		...
		if (!chatSession || !(chatSession instanceof SkipprCLISession)) {
	 ...
	 */
	private async _sendFirstChat(session: SkipprCLISession | RemoteNewSession | ClaudeCodeNewSession | LocalNewSession, options: ISendRequestOptions): Promise<ISession> {
		...
		disposable.dispose();
		if (!chatWidget) {
			throw new Error('[DefaultSkipprProvider] Failed to open chat widget');
		}

		// Send request
		this.logService.debug(`[SkipprChatSessionsProvider] Sending first chat for session ${session.id} with options:`, {
		...
		if (result.kind === 'rejected') {
			throw new Error(`[DefaultSkipprProvider] sendRequest rejected: ${result.reason}`);
		}
	...
	private async _sendFirstChatLocal(
		...
		disposable.dispose();
		if (!chatWidget) {
			throw new Error('[SkipprChatSessionsProvider] Failed to open chat widget for local session');
		}

		// Obtain user-selected tools from the chat widget so the skippr
		...
		this.logService.debug(`[SkipprChatSessionsProvider] Sending first chat for local session ${session.id}`);
		...
		if (result.kind === 'rejected') {
			throw new Error(`[SkipprChatSessionsProvider] Local sendRequest rejected: ${result.reason}`);
		}
	...
	private async _sendFirstChatViaController(
		...
		if (!newItem) {
			throw new Error('[SkipprChatSessionsProvider] Failed to create Claude session item');
		}
		...
		disposable.dispose();
		if (!chatWidget) {
			throw new Error('[SkipprChatSessionsProvider] Failed to open chat widget');
		}

		// Send request to the real URI — sendRequest skips the
		// createNewChatSessionItem block since the URI is not untitled.
		this.logService.debug(`[SkipprChatSessionsProvider] Sending first Claude chat to ${realResource.toString()} with options:`, {
			userSelectedModelId: sendOptions.userSelectedModelId,
		});
		const result = await this.chatService.sendRequest(realResource, query, sendOptions);
		if (result.kind === 'rejected') {
			throw new Error(`[SkipprChatSessionsProvider] sendRequest rejected: ${result.reason}`);
		}
	 ...
	 * Creates a new {@link SkipprCLISession} from the existing workspace and
	...
	private async _sendSubsequentChat(sessionId: string, options: ISendRequestOptions): Promise<ISession> {
		...
		let newChatSession: SkipprCLISession;
		if (this._currentNewSession && this._getGroupIdForChat(this._currentNewSession) === sessionId) {
			newChatSession = this._currentNewSession as SkipprCLISession;
		} else {
	 ...
	 * in the cache.
	 */
	private async _sendExistingChat(sessionId: string, newChatSession: SkipprCLISession, options: ISendRequestOptions): Promise<ISession> {
		...
		if (!chatWidget) {
			this._sessionCache.delete(key);
			this._invalidateGroupingCaches();
			throw new Error('[DefaultSkipprProvider] Failed to open chat widget for subsequent chat');
		}
	...
	/**
	 * Creates a new {@link SkipprCLISession} from an existing session's workspace.
	 * Used for subsequent chats that share the same workspace but are independent conversations.
	 */
	private _createNewSessionFrom(sessionId: string): SkipprCLISession {
		...
		const resource = URI.from({ scheme: AgentSessionProviders.Background, path: `/untitled-${generateUuid()}` });
		const session = this.instantiationService.createInstance(SkipprCLISession, resource, newWorkspace, this.id);
	...
	private _ensureGroupingCaches(): void {
		...
		const chatByRawSessionId = new Map<string, ISkipprChatSession>();
		for (const chat of chats) {
			chatByRawSessionId.set(chat.resource.path.slice(1), chat);
		}

		const groupIdByChatId = new Map<string, string>();
		const chatsByGroupId = new Map<string, ISkipprChatSession[]>();

		const resolveGroupId = (chat: ISkipprChatSession): string => {
			const cachedGroupId = groupIdByChatId.get(chat.id);
			if (cachedGroupId) {
				return cachedGroupId;
			}

			const trail: ISkipprChatSession[] = [];
			const seen = new Set<string>();
			let current: ISkipprChatSession = chat;
	...
	private _refreshSessionCache(): void {
		...
		const addedData: ISkipprChatSession[] = [];
		const changedData: ISkipprChatSession[] = [];
		...
		const removedData: ISkipprChatSession[] = [];
		...
		let removedGroupIds: Map<ISkipprChatSession, string> | undefined;
	...
	private _refreshSessionCacheMultiChat(
		addedData: ISkipprChatSession[],
		removedData: ISkipprChatSession[],
		changedData: ISkipprChatSession[],
		removedGroupIds: Map<ISkipprChatSession, string>,
	): void {
		...
		const trulyRemovedSessions: { chat: ISkipprChatSession; groupId: string }[] = [];
		...
		const newSessions: ISkipprChatSession[] = [];
		...
		const deduplicatedChanged: ISkipprChatSession[] = [];
	...
	private _findChatSession(chatId: string): ISkipprChatSession | undefined {
	 ...
	 */
	private _getGroupIdForChat(chat: ISkipprChatSession): string {
	...
	private _getDirectParentRawSessionId(chat: ISkipprChatSession): string | undefined {
	...
	/**
	 * Wraps a primary {@link ISkipprChatSession} and its sibling chats into an {@link ISession}.
	 ...
	 */
	private _chatToSession(chat: ISkipprChatSession): ISession {
		if (!this._isMultiChatEnabled()) {
		...
		const chatsObs = observableFromEvent<readonly IChat[]>(
			...
			() => {
				...
				const resolved: ISkipprChatSession[] = [];
		...
		const session: ISession = {
			...
			mainChat,
			capabilities: {
				supportsMultipleChats: primaryChat.sessionType === SkipprCLISessionType.id && this._isMultiChatEnabled(),
			},
		};
	...
	private _chatToSingleChatSession(chat: ISkipprChatSession): ISession {
	...
	private _toChat(chat: ISkipprChatSession): IChat {
		return {
			resource: chat.resource,
			createdAt: chat.createdAt,
			title: chat.title,
