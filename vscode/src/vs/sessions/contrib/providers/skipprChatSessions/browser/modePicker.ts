...
import { SkipprChatSessionsProvider } from './skipprChatSessionsProvider.js';
...
export class ModePicker extends Disposable {
	...
	constructor(
		...
		this._chatModes = this.chatModeService.getModes(SkipprCLISessionType.id);
	...
	private _getAvailableModes(): IChatMode[] {
		const customAgentTarget = this.chatSessionsService.getCustomAgentTargetForSessionType(SkipprCLISessionType.id);
		const effectiveTarget = customAgentTarget && customAgentTarget !== Target.Undefined ? customAgentTarget : Target.GitHubSkippr;
