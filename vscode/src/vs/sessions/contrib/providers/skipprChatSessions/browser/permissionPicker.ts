...
import { SkipprChatSessionsProvider } from '../../skipprChatSessions/browser/skipprChatSessionsProvider.js';
 ...
 * {@link SkipprPermissionPickerDelegate} below for the default Skippr
		...
		const items: IActionListItem<IPermissionItem>[] = [
			{
				...
				label: localize('permissions.default', "Default Approvals"),
				detail: localize('permissions.default.subtext', "Skippr uses your configured settings"),
				disabled: false,
			},
			{
				kind: ActionListItemKind.Action,
		...
		const delegate: IActionListDelegate<IPermissionItem> = {
			onSelect: async (item) => {
				...
				if (item.level) {
					...
					await this.openerService.open(URI.parse('https://skippr.com/ide/docs/docs/skippr/agents/agent-tools#_permission-levels'));
				}
			},
...
/**
 * Default-Skippr {@link IPermissionPickerDelegate}: writes the user's chosen
 * level back to the active {@link SkipprChatSessionsProvider} session.
 *
 ...
 */
export class SkipprPermissionPickerDelegate extends Disposable implements IPermissionPickerDelegate {
	constructor(
	...
	setPermissionLevel(level: ChatPermissionLevel): void {
		...
		if (provider instanceof SkipprChatSessionsProvider) {
