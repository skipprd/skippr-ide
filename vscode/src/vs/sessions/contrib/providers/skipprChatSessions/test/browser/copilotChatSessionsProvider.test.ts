...
import { CLAUDE_CODE_ENABLED_SETTING, SkipprChatSessionsProvider, COPILOT_PROVIDER_ID, ClaudeCodeSessionType } from '../../browser/skipprChatSessionsProvider.js';
...
function createProviderWithConfig(
	...
	model: MockAgentSessionsModel,
	opts?: { multiChatEnabled?: boolean; claudeEnabled?: boolean },
): { provider: SkipprChatSessionsProvider; configService: TestConfigurationService } {
	const instantiationService = disposables.add(new TestInstantiationService());

	const configService = new TestConfigurationService();
	configService.setUserConfiguration('sessions.skippr.data-agent.multiChatSessions', opts?.multiChatEnabled ?? true);
	...
	instantiationService.stub(ICommandService, {
		executeCommand: async (_id: string, ...args: any[]) => {
			// Simulate 'agents.skippr.data-agent.cli.deleteSessions' removing sessions
			const items = args[0];
	...
	instantiationService.stub(IChatSessionsService, {
		getChatSessionContribution: () => ({ type: 'test-skippr', name: 'test', displayName: 'Test', description: 'test', icon: undefined }),
	...
	// Stub IInstantiationService so provider can use createInstance for SkipprCLISession
	instantiationService.stub(IInstantiationService, instantiationService);
	instantiationService.stub(ILabelService, {
		getUriLabel: (uri: URI) => uri.path,
	});

	const provider = disposables.add(instantiationService.createInstance(SkipprChatSessionsProvider));
 ...
 * needed by SkipprCLISession and _sendFirstChat, including IGitService and a
...
function createProviderForSendTests(
	...
	const configService = new TestConfigurationService();
	configService.setUserConfiguration('sessions.skippr.data-agent.multiChatSessions', true);
	configService.setUserConfiguration(CLAUDE_CODE_ENABLED_SETTING, opts?.claudeEnabled ?? true);
	...
	return disposables.add(instantiationService.createInstance(SkipprChatSessionsProvider));
}

suite('SkipprChatSessionsProvider', () => {
	...
	// ---- Session creation -------
	// Note: createNewSession tests are limited because SkipprCLISession
	...
	test('skippr CLI sessions have supportsMultipleChats capability', () => {
		const resource = URI.from({ scheme: AgentSessionProviders.Background, path: '/session-1' });
	...
	test('skippr cloud sessions do not have supportsMultipleChats capability', () => {
	...
	test('skippr CLI sessions do not have supportsMultipleChats when setting is disabled', () => {
		const resource = URI.from({ scheme: AgentSessionProviders.Background, path: '/session-1' });
		model.addSession(createMockAgentSession(resource));
	...
	test('setModel applies to existing sessions and their new chats', async () => {
		...
		const provider = createProvider(disposables, model);
		const session = provider.getSessions()[0];
		provider.setModel(session.sessionId, 'skippr/gpt-4o');

		assert.strictEqual(session.modelId.get(), 'skippr/gpt-4o');

		const chat = provider.addChat(session.sessionId);
		try {
			assert.strictEqual(chat.modelId.get(), 'skippr/gpt-4o');
		} finally {
	...
	function makeClaudeInFlightProvider(): { provider: SkipprChatSessionsProvider; cancelRequest: () => void; realResource: URI; commitSession: () => void } {
	...
	function waitForSessionAdded(provider: SkipprChatSessionsProvider): Promise<void> {
	...
	suite('uncommitted temp session cleanup', () => {
		...
		function makeInFlightProvider(): {
			provider: SkipprChatSessionsProvider;
			cancelRequest: () => void;
		} {
			let resolveComplete!: () => void;
			let resolveCreated!: (r: IChatResponseModel) => void;
		...
		test('deleteSession removes a temp session that is awaiting commit', async () => {
			...
			const newSession = provider.createNewSession(workspace, SkipprCLISessionType.id);
