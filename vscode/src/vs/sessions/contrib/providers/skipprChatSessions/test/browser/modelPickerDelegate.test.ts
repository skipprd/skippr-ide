...
import { getAvailableModels, modelPickerStorageKey, SessionModelPicker } from '../../browser/skipprChatSessionsActions.js';
import { SkipprCLISessionType } from '../../../agentHost/browser/baseAgentHostSessionsProvider.js';
import { ClaudeCodeSessionType } from '../../browser/skipprChatSessionsProvider.js';
...
function makeActiveSession(session: Partial<IActiveSession>): IActiveSession {
	...
	return {
		providerId: session.providerId ?? 'default-skippr',
		sessionId,
		sessionType: session.sessionType ?? SkipprCLISessionType.id,
...
suite('modelPickerStorageKey', () => {
	...
	test('produces per-session-type keys', () => {
		assert.strictEqual(modelPickerStorageKey(SkipprCLISessionType.id), `sessions.modelPicker.${SkipprCLISessionType.id}.selectedModelId`);
...
suite('getAvailableModels', () => {
	...
	test('returns empty when no active session', () => {
		const models = [makeModel('model-1', SkipprCLISessionType.id)];
	...
	test('filters models by session type', () => {
		const models = [
			makeModel('cli-model', SkipprCLISessionType.id),
			makeModel('cloud-model', 'skippr-cloud'),
			makeModel('claude-model', ClaudeCodeSessionType.id),
		];
		const { instantiationService } = stubServices(disposables, {
			models,
			activeSession: { providerId: 'default-skippr', sessionId: 'sess-1', sessionType: ClaudeCodeSessionType.id },
		});
		const languageModelsService = instantiationService.get(ILanguageModelsService);
		const sessionsManagementService = instantiationService.get(ISessionsManagementService);
		const result = getAvailableModels(languageModelsService, sessionsManagementService);
...
suite('SessionModelPicker', () => {
	...
	test('stores selected model under session-type-scoped key', () => {
		...
		assert.strictEqual(storage.has(modelPickerStorageKey(SkipprCLISessionType.id)), false);
	});
	...
	test('different session types use independent storage keys', () => {
		const cliModels = [makeModel('cli-m', SkipprCLISessionType.id)];
		...
		const { instantiationService, storage, activeSession } = stubServices(disposables, {
			models: allModels,
			activeSession: { providerId: 'default-skippr', sessionId: 's1', sessionType: SkipprCLISessionType.id },
		});
		disposables.add(instantiationService.createInstance(SessionModelPicker));
		assert.strictEqual(storage.get(modelPickerStorageKey(SkipprCLISessionType.id)), 'cli-m');

		// Switch session type
		activeSession.set(makeActiveSession({ providerId: 'default-skippr', sessionId: 's2', sessionType: ClaudeCodeSessionType.id }), undefined);
	...
	test('propagates selected model to a new session of the same type (#313385)', () => {
		const models = [makeModel('cli-a', SkipprCLISessionType.id), makeModel('cli-b', SkipprCLISessionType.id)];
		const storedEntries = new Map([[modelPickerStorageKey(SkipprCLISessionType.id), 'cli-b']]);
		const calls: { sessionId: string; modelId: string }[] = [];
		...
		activeSession.set(makeActiveSession({ providerId: 'default-skippr', sessionId: 's2', sessionType: SkipprCLISessionType.id }), undefined);
	...
	test('does not re-push model to the same session when language models change', () => {
		const models = [makeModel('cli-a', SkipprCLISessionType.id)];
		const calls: { sessionId: string; modelId: string }[] = [];
	...
	test('does not overwrite existing session model on init', () => {
		...
		const { instantiationService, storage } = stubServices(disposables, {
			models,
			activeSession: {
				providerId: 'default-skippr',
				sessionId: 's1',
				sessionType: SkipprCLISessionType.id,
	...
	test('keeps unresolved existing session model pending instead of defaulting', () => {
		const models = [makeModel('skipprcli/cli-a', SkipprCLISessionType.id)];
		const calls: { sessionId: string; modelId: string }[] = [];
		const { instantiationService, storage, fireLanguageModelsChanged } = stubServices(disposables, {
			...
			activeSession: {
				...
				sessionId: 's1',
				sessionType: SkipprCLISessionType.id,
				modelId: observableValue<string | undefined>('existingModelId', 'skipprcli/cli-b'),
				status: observableValue<SessionStatus>('existingStatus', SessionStatus.Completed),
			},
			setModelSpy: (sessionId, modelId) => calls.push({ sessionId, modelId }),
		});
		...
		models.push(makeModel('skipprcli/cli-b', SkipprCLISessionType.id));
		fireLanguageModelsChanged();
	...
	test('defaults existing session with removed model after vendor resolves', () => {
		const models = [makeModel('skipprcli/cli-a', SkipprCLISessionType.id), makeModel('skipprcli/cli-c', SkipprCLISessionType.id)];
		const resolvedVendors = new Set<string>();
		const storedEntries = new Map([[modelPickerStorageKey(SkipprCLISessionType.id), 'skipprcli/cli-c']]);
		const calls: { sessionId: string; modelId: string }[] = [];
		const { instantiationService, fireLanguageModelsChanged } = stubServices(disposables, {
			models,
			resolvedVendors,
			...
			activeSession: {
				...
				sessionId: 's1',
				sessionType: SkipprCLISessionType.id,
				modelId: observableValue<string | undefined>('existingModelId', 'skipprcli/removed'),
				status: observableValue<SessionStatus>('existingStatus', SessionStatus.Completed),
			},
			setModelSpy: (sessionId, modelId) => calls.push({ sessionId, modelId }),
		});
		...
		resolvedVendors.add('skipprcli');
		fireLanguageModelsChanged();

		assert.deepStrictEqual(calls, [{ sessionId: 's1', modelId: 'skipprcli/cli-c' }]);
	});
	...
	test('does not push previous current model into existing session without model state', () => {
		...
		const { instantiationService, activeSession } = stubServices(disposables, {
			models,
			activeSession: { providerId: 'default-skippr', sessionId: 'new-session', sessionType: SkipprCLISessionType.id },
			storedEntries,
			setModelSpy: (sessionId, modelId) => calls.push({ sessionId, modelId }),
		});
		disposables.add(instantiationService.createInstance(SessionModelPicker));
