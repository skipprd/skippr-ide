...
class MockAgentHostService extends mock<IAgentHostService>() {
	...
	private _rootStateValue: RootState | Error | undefined = { agents: [{ provider: 'skipprcli', displayName: 'Skippr', description: '', models: [] } as AgentInfo] };
	...
	override async createSession(config?: { session?: URI }): Promise<URI> {
		const uri = config?.session ?? URI.parse('skipprcli:///auto-' + this._nextSeq);
...
function createSession(id: string, opts?: { provider?: string; summary?: string; model?: string; project?: { uri: URI; displayName: string }; workingDirectory?: URI; startTime?: number; modifiedTime?: number }): IAgentSessionMetadata {
	return {
		session: AgentSession.uri(opts?.provider ?? 'skipprcli', id),
...
function createProvider(disposables: DisposableStore, agentHostService: MockAgentHostService, contributions = [
	{ type: 'agent-host-skipprcli', name: 'skippr', displayName: 'Skippr', description: 'test', icon: undefined },
...
function fireSessionAdded(agentHost: MockAgentHostService, rawId: string, opts?: { provider?: string; title?: string; model?: string; modelConfig?: Record<string, string>; project?: { uri: string; displayName: string }; workingDirectory?: string }): void {
	const provider = opts?.provider ?? 'skipprcli';
...
function fireSessionRemoved(agentHost: MockAgentHostService, rawId: string, provider = 'skipprcli'): void {
	const sessionUri = AgentSession.uri(provider, rawId);
	agentHost.fireNotification({
...
suite('LocalAgentHostSessionsProvider', () => {
	...
	test('has correct id, label, and sessionType from rootState agents', () => {
		...
		// the same agent (e.g. `skipprcli`) shares one session type across
		// local and remote hosts and the standalone Skippr Agent provider.
		assert.strictEqual(provider.sessionTypes[0].id, 'skipprcli');
		assert.strictEqual(provider.sessionTypes[0].label, 'Skippr');
	});
	...
	test('session types update when the local host advertises additional agents', () => {
		...
		agentHost.setAgents([
			{ provider: 'skipprcli', displayName: 'Skippr', description: '', models: [] } as AgentInfo,
	...
	test('uses project metadata as workspace group source', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		const workingDirectory = URI.file('/tmp/skippr-worktrees/vscode-feature');
	...
	test('uses model metadata as selected model for listed sessions', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		assert.strictEqual(session?.modelId.get(), 'agent-host-skipprcli:claude-sonnet-4.5');
	}));
	...
	test('uses model metadata from session added notification', () => {
		...
		assert.strictEqual(session?.modelId.get(), 'agent-host-skipprcli:gpt-5');
	});
	...
	test('setModel updates existing session model and dispatches raw model', () => {
		...
		provider.setModel(session!.sessionId, 'agent-host-skipprcli:new-model');

		assert.strictEqual(session!.modelId.get(), 'agent-host-skipprcli:new-model');
		assert.deepStrictEqual(agentHost.dispatchedActions.at(-1)?.action, {
			type: ActionType.SessionModelChanged,
			session: AgentSession.uri('skipprcli', 'set-model').toString(),
			model: { id: 'new-model' },
		});
	});
	...
	test('setModel preserves current model config when model id is unchanged', () => {
		...
		provider.setModel(session!.sessionId, 'agent-host-skipprcli:configured-model');

		assert.deepStrictEqual(agentHost.dispatchedActions.at(-1)?.action, {
			type: ActionType.SessionModelChanged,
			session: AgentSession.uri('skipprcli', 'set-model-config').toString(),
	...
	test('deleteSession calls disposeSession and removes from cache', async () => {
		...
		assert.strictEqual(AgentSession.provider(disposedUri), 'skipprcli');
	...
	test('renameChat dispatches SessionTitleChanged action', async () => {
		...
		assert.strictEqual(AgentSession.provider(actionSession), 'skipprcli');
	...
	test('server-echoed SessionTitleChanged updates cached title', () => {
		...
		agentHost.fireAction({
			action: {
				...
				session: AgentSession.uri('skipprcli', 'echo-sess').toString(),
				title: 'Server Title',
			},
			serverSeq: 1,
			origin: undefined,
	...
	test('server-echoed SessionModelChanged updates cached model', () => {
		...
		agentHost.fireAction({
			action: {
				type: ActionType.SessionModelChanged,
				session: AgentSession.uri('skipprcli', 'model-change').toString(),
		...
		assert.strictEqual(target!.modelId.get(), 'agent-host-skipprcli:new-model');
		assert.strictEqual(changes.length, 1);
		assert.strictEqual(changes[0].changed.length, 1);
	});
	...
	test('turnComplete action triggers session refresh', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		agentHost.fireAction({
			action: {
				...
				session: AgentSession.uri('skipprcli', 'turn-sess').toString(),
			},
			serverSeq: 1,
			origin: undefined,
		} as ActionEnvelope);
	...
	test('getSessionConfig seeds running config from session state subscription with full schema', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		const fakeState: SessionState = {
			summary: { resource: AgentSession.uri('skipprcli', 'seed-1').toString(), provider: 'skipprcli', title: 'Seeded Session', status: ProtocolSessionStatus.Idle, createdAt: 0, modifiedAt: 0 },
			lifecycle: SessionLifecycle.Ready,
			turns: [],
			config,
		};
		agentHost.setSessionState('seed-1', 'skipprcli', fakeState);
	...
	test('removing a session disposes its session-state subscription', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		const sessionUriStr = AgentSession.uri('skipprcli', 'seed-2').toString();
	...
	test('session-state subscription auto-releases after the idle window', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		const sessionUriStr = AgentSession.uri('skipprcli', 'idle-1').toString();
	...
	test('replaceSessionConfig only replaces sessionMutable, non-readOnly values and preserves everything else', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		const fakeState: SessionState = {
			summary: { resource: AgentSession.uri('skipprcli', 'rep-1').toString(), provider: 'skipprcli', title: 'Replace Session', status: ProtocolSessionStatus.Idle, createdAt: 0, modifiedAt: 0 },
			lifecycle: SessionLifecycle.Ready,
			turns: [],
			config,
		};
		agentHost.setSessionState('rep-1', 'skipprcli', fakeState);
		await waitForSessionConfig(provider, session!.sessionId, c => c?.values.autoApprove === 'default');
		...
		const sessionUri = AgentSession.uri('skipprcli', 'rep-1').toString();
	...
	test('replaceSessionConfig is a no-op when nothing editable actually changes', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		const fakeState: SessionState = {
			summary: { resource: AgentSession.uri('skipprcli', 'rep-2').toString(), provider: 'skipprcli', title: 'No-op Session', status: ProtocolSessionStatus.Idle, createdAt: 0, modifiedAt: 0 },
			lifecycle: SessionLifecycle.Ready,
			turns: [],
			config,
		};
		agentHost.setSessionState('rep-2', 'skipprcli', fakeState);
		await waitForSessionConfig(provider, session!.sessionId, c => c?.values.autoApprove === 'default');
	...
	test('server-echoed SessionConfigChanged merges config values into the running cache by default', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		const fakeState: SessionState = {
			summary: { resource: AgentSession.uri('skipprcli', 'cfg-merge').toString(), provider: 'skipprcli', title: 'Merge Session', status: ProtocolSessionStatus.Idle, createdAt: 0, modifiedAt: 0 },
			lifecycle: SessionLifecycle.Ready,
			turns: [],
			config: {
				schema: {
		...
		agentHost.setSessionState('cfg-merge', 'skipprcli', fakeState);
		await waitForSessionConfig(provider, session!.sessionId, c => c?.values.autoApprove === 'default');

		agentHost.fireAction({
			action: {
				type: ActionType.SessionConfigChanged,
				session: AgentSession.uri('skipprcli', 'cfg-merge').toString(),
	...
	test('server-echoed SessionConfigChanged with replace:true overwrites the running cache', () => runWithFakedTimers<void>({ useFakeTimers: true }, async () => {
		...
		const fakeState: SessionState = {
			summary: { resource: AgentSession.uri('skipprcli', 'cfg-replace').toString(), provider: 'skipprcli', title: 'Replace Session', status: ProtocolSessionStatus.Idle, createdAt: 0, modifiedAt: 0 },
			lifecycle: SessionLifecycle.Ready,
			turns: [],
			config: {
				schema: {
		...
		agentHost.setSessionState('cfg-replace', 'skipprcli', fakeState);
		await waitForSessionConfig(provider, session!.sessionId, c => c?.values.autoApprove === 'default');

		agentHost.fireAction({
			action: {
				type: ActionType.SessionConfigChanged,
				session: AgentSession.uri('skipprcli', 'cfg-replace').toString(),
