...
/** Skippr Agent session type */
export const SkipprCLISessionType: ISessionType = {
	id: 'skipprcli',
	label: localize('skipprCLI', "Skippr Agent"),
	icon: Codicon.skippr,
};
...
export abstract class BaseAgentHostSessionsProvider extends Disposable implements IAgentHostSessionsProvider {
	 ...
	 * agent provider name itself, so the same agent (e.g. `skipprcli`)
	 * appears under one shared session type across hosts.
	 */
	protected abstract resourceSchemeForProvider(provider: string): string;

	/** Format the human-readable label for a session type entry (e.g. `Skippr [Local]`). */
	...
	private iconForAgentProvider(provider: string): ThemeIcon | undefined {
		if (provider === SkipprCLISessionType.id) {
