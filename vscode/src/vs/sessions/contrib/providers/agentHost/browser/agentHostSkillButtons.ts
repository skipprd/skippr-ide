 ...
 * skill toolbar buttons and to suppress the Skippr Agent extension's own
 ...
 * `agent-host-*` session.
 *
 * They mirror the buttons the Skippr Agent extension contributes for its own
 * `chatSessionType == skipprcli` sessions, sending the same `/<skill-name>`
 ...
 * as the Skippr Agent extension's Sync PR button. Exported so the changes
...
function registerAgentHostSkillButton(spec: IAgentHostSkillButtonSpec): void {
	registerAction2(class extends Action2 {
		...
		async run(accessor: ServicesAccessor): Promise<void> {
			...
			// `agent-host-skipprcli`), which is the agent id the chat
			// service uses for routing. The `sessionType` field is the
			// logical, user-facing id (e.g. `skipprcli`) that is shared
			// between the Skippr Agent extension and the local/remote
