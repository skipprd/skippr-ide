...
export class LocalAgentHostSessionsProvider extends BaseAgentHostSessionsProvider {
	...
	constructor(
		...
		// settle because the underlying agent (e.g. SkipprAgent) throws
	 ...
	 * (e.g. `skipprcli`) so the same agent shares one session type across
	...
	protected _formatSessionTypeLabel(agentLabel: string): string {
		// Use the unadorned agent label (e.g. "Skippr") rather than tagging it
		// with `[Local]`. The session type id is shared with the extension-host
		// Skippr Agent provider, so the filter menu / new-session picker entry
