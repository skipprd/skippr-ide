...
class MobileChatPhoneInputPresenter extends Disposable implements IChatPhonePresenterImpl {
	...
	async showCombinedModeAndModelSheet(
		...
		// (e.g. model identifiers like `skippr:gpt-4o`) round-trip
		...
		// default Skippr chat protocol and are wrong for agent-host
		// sessions.
		...
		if (activeSession && agentHostProvider) {
			...
			// Default Skippr chat path: requires the workbench
