...
class ChangesButtonBarWidget extends Disposable {
	...
	private _getButtonConfiguration(action: IAction, outgoingChanges: number, reviewState: { isLoading: boolean; commentCount: number | undefined }, hasGitOperationInProgress: boolean, runningLabelObs: IObservable<string | IMarkdownString | undefined>): { showIcon: boolean; showLabel: boolean; isSecondary?: boolean; customLabel?: string | IMarkdownString; customLabelObs?: IObservable<string | IMarkdownString | undefined>; customClass?: string } | undefined {
		if (
			action.id === 'skippr.data-agent.sessions.commit' ||
			action.id === 'skippr.data-agent.chat.createPullRequestSkipprCLIAgentSession.createPR'
		) {
			if (!hasGitOperationInProgress) {
		...
		if (
			action.id === 'skippr.data-agent.sessions.sync' ||
			action.id === 'skippr.data-agent.sessions.commitAndSync'
		) {
		...
		if (
			action.id === 'skippr.data-agent.claude.sessions.sync' ||
		...
		if (
			...
			action.id === 'skippr.data-agent.chat.openPullRequestSkipprCLIAgentSession.openPR'
		) {
			return { showIcon: true, showLabel: false, isSecondary: true };
		}
		...
		if (
			action.id === 'skippr.data-agent.chat.createPullRequestSkipprCLIAgentSession.createPR' ||
			action.id === 'skippr.data-agent.chat.mergeSkipprCLIAgentSessionChanges.merge' ||
			action.id === 'skippr.data-agent.chat.checkoutPullRequestReroute' ||
			action.id === 'pr.checkoutFromChat' ||
			action.id === 'skippr.data-agent.sessions.initializeRepository' ||
			action.id === 'skippr.data-agent.claude.sessions.initializeRepository' ||
			action.id === 'skippr.data-agent.claude.sessions.commit' ||
			action.id === 'skippr.data-agent.claude.sessions.commitAndSync' ||
...
export class ChangesViewPane extends ViewPane {
	...
	private getTreeRootInfo(items: readonly IChangesFileItem[]): IChangesTreeRootInfo | undefined {
		...
		if (workspaceFolderUri.scheme === GITHUB_REMOTE_FILE_SCHEME) {
			// Cloud session
			resourceTreeRootUri = URI.from({ scheme: Schemas.skipprPr, path: '/' });
			...
			[this.instantiationService.createInstance(ChangesTreeRenderer, this.viewModel, resourceLabels, actionRunner,
				() => {
					...
					return folder?.root.scheme === GITHUB_REMOTE_FILE_SCHEME
						? URI.from({ scheme: Schemas.skipprPr, path: '/' })
