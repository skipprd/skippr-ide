...
import { IDefaultAccount, IDefaultAccountAuthenticationProvider, ISkipprTokenInfo, IPolicyData } from '../../base/common/defaultAccount.js';
 ...
 * window think the user is signed in with a Free Skippr plan.
 */
class MockChatEntitlementService implements IChatEntitlementService {
	...
	readonly sku = 'free';
	readonly chatTrackingId = 'mock-tracking-id';
...
class MockDefaultAccountService implements IDefaultAccountService {
	...
	readonly skipprTokenInfo: ISkipprTokenInfo | null = null;
	readonly onDidChangeSkipprTokenInfo = Event.None;
...
class MockChatAgentContribution extends Disposable implements IWorkbenchContribution {
	 ...
	 * ChangesViewPane can render them for background (skipprcli) sessions.
	...
	private addSessionItem(resource: URI, message: string, responseText: string, fileEdits?: MockFileEdit[]): void {
		...
		this._sessionHistory.get(key)!.push(
			{ type: 'request', prompt: message, participant: 'skippr' },
			{ type: 'response', parts: [{ kind: 'markdownContent', content: { value: responseText, isTrusted: false, supportThemeIcons: false, supportHtml: false } }], participant: 'skippr' },
		);
	...
	private registerMockAgents(): void {
		const agentIds = ['skipprcli', 'skippr-cloud-agent'];
	...
	private registerMockSessionProvider(): void {
		const schemes = ['skipprcli', 'skippr-cloud-agent'];
		const self = this;
		for (const scheme of schemes) {
			try {
				...
				// Only skipprcli (Background) sessions need real items — the
				// skippr-cloud-agent controller must return an empty array to
				// prevent it from overwriting sessions with the wrong providerType
				// during a full model resolve.
				const controllerItems = scheme === 'skipprcli' ? this._sessionItems : [];
...
export class TestSessionsBrowserMain extends SessionsBrowserMain {
	...
	private preseedFolder(storageService: IStorageService): void {
		...
		const providerId = 'default-skippr';
