...
- No folder selected
- No user customizations created (from this tool or others i.e. Skippr Agent)
...
- Instructions includes root-level files (AGENTS.md, CLAUDE.md, skippr-instructions.md) under "Workspace"
...
- If user-level files exist in `~/.skippr/` or `~/.claude/`, a "User" group appears for applicable sections
...
5. Verify the file path shown in the editor header is `~/.skippr/instructions/<name>.instructions.md`
...
11. Check the "Source Folders (creation targets)" section — verify `[user]` points to `~/.skippr/instructions`

#### Expected Results

- User file is created under `~/.skippr/instructions/` (not the VS Code profile folder)
- The file appears in the "User" group in the list
- Sidebar badge count includes the new user file
- Debug report confirms the user creation target is `~/.skippr/instructions`

#### Notes

- This validates that `AgenticPromptsService.getSourceFolders()` correctly redirects user creation to `~/.skippr/`
...
- JSON skeleton has correct Skippr Agent format: `"version": 1`, `"bash"` field
...
- Only `"bash"` is used (not `"command"`) to match the Skippr Agent schema
