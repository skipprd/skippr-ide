...
/**
 * Compile .scenario.md files into .commands.json using Skippr Agent.
 *
 ...
 * 3. Sends each step + snapshot to Skippr Agent to get the playwright-cli commands
...
// ---------------------------------------------------------------------------
// Ask Skippr Agent to translate a step
// ---------------------------------------------------------------------------

function askSkippr(step, snapshot) {
	const prompt = `Snapshot:\n\`\`\`\n${snapshot}\n\`\`\`\n\nStep: ${step}\n\nOutput the semantic commands:`;

	const result = cp.spawnSync('skippr', ['-p', `${SYSTEM_PROMPT}\n\n${prompt}`, '--model', 'claude-sonnet-4.6'], {
		cwd: APP_ROOT,
	...
	if (result.status !== 0) {
		throw new Error(`Skippr Agent failed: ${stderr || stdout}`);
	}
...
function compileScenario(scenario) {
	...
	for (const [i, step] of scenario.steps.entries()) {
		...
		try {
			const commands = askSkippr(step, snapshot.stdout);
