...
[`playwright-cli`](https://github.com/microsoft/playwright-cli) and Skippr Agent.
...
| Service | Mock | Why |
|---------|------|-----|
| `IChatEntitlementService` | Returns `ChatEntitlement.Free` | No real Skippr account in CI |
...
| Chat agents (`skipprcli`, etc.) | Canned keyword-matched responses with `textEdit` progress items | No real LLM backend |
...
| Code Review command | Returns canned review comments per file (extension) | No real Skippr AI review |
...
- **`CodeReviewService`** — Orchestrates review requests, processes results from
  the mock `skippr.data-agent.chat.codeReview.run` command, and stores comments
...
Code Review button clicked → sessions.codeReview.run (core action)
  → CodeReviewService.requestReview()
    → commandService.executeCommand('chat.internal.codeReview.run')
      → Bridge forwards to 'skippr.data-agent.chat.codeReview.run'
...
Create PR button clicked → skippr.data-agent.chat.createPullRequestSkipprCLIAgentSession.createPR
  → Mock extension logs and shows info message
```

The PR buttons (Create PR, Open PR, Merge) are contributed via the mock
extension's `package.json` menus, gated by `chatSessionType == skipprcli`.
The `chatSessionType` context key is derived from the session URI scheme
(`getChatSessionType()`), which returns `skipprcli` for mock sessions.
...
3. Sends each natural-language step + snapshot to **Skippr Agent**, which returns
...
├── generate.cjs              # Compiles scenarios → .commands.json via Skippr Agent
...
- Dependencies installed:
  ...
  ```
- Skippr Agent available (for `npm run generate` only):
  ```bash
  skippr --version
  ```
...
Write steps in plain English. The Skippr agent interprets them against the
...
Skippr Agent returns: `click e143`
