...
suite('deduplicateSessions', () => {
	...
	test('removes duplicate when same deduplicationKey appears across providers', () => {
		const local = stubSession({ sessionId: 'local-1', providerId: LOCAL_AGENT_HOST_PROVIDER_ID, deduplicationKey: 'skippr:///abc123' });
		const remote = stubSession({ sessionId: 'remote-1', providerId: 'agenthost-tunnel', deduplicationKey: 'skippr:///abc123' });
	...
	test('keeps first occurrence when no local provider exists among duplicates', () => {
		const r1 = stubSession({ sessionId: 'r1', providerId: 'agenthost-a', deduplicationKey: 'skippr:///abc123' });
		const r2 = stubSession({ sessionId: 'r2', providerId: 'agenthost-b', deduplicationKey: 'skippr:///abc123' });
	...
	test('does not deduplicate sessions with different keys', () => {
		const s1 = stubSession({ sessionId: 's1', providerId: LOCAL_AGENT_HOST_PROVIDER_ID, deduplicationKey: 'skippr:///aaa' });
		const s2 = stubSession({ sessionId: 's2', providerId: 'agenthost-tunnel', deduplicationKey: 'skippr:///bbb' });
		const result = deduplicateSessions([s1, s2]);
		assert.deepStrictEqual(result, [s1, s2]);
	});

	test('mixes sessions with and without deduplication keys', () => {
		const keyed1 = stubSession({ sessionId: 'k1', providerId: LOCAL_AGENT_HOST_PROVIDER_ID, deduplicationKey: 'skippr:///abc123' });
		const keyed2 = stubSession({ sessionId: 'k2', providerId: 'agenthost-tunnel', deduplicationKey: 'skippr:///abc123' });
		const noKey = stubSession({ sessionId: 'nk', providerId: 'skippr-chat' });
