...
- **Well-known root helpers** — `getCliUserRoots(userHome)` and `getClaudeUserRoots(userHome)` centralize the `~/.skippr`, `~/.claude`, `~/.agents` path knowledge.
...
| `cli` | Skippr Agent | Restricts user roots to `~/.skippr`, `~/.claude`, `~/.agents` |
...
| Agents, Skills, Instructions | `[local, user, plugin, builtin]` | `[~/.skippr, ~/.claude, ~/.agents]` |
...
| Agents, Skills, Instructions | `[local, user, plugin]` | `[~/.skippr, ~/.claude, ~/.agents]` |
...
- `instructionFileFilter: ['CLAUDE.md', 'CLAUDE.local.md', '.claude/rules/', 'skippr-instructions.md']`
- `workspaceSubpaths: ['.claude']` (instruction files matching `instructionFileFilter` are exempt)
- `sectionOverrides`: Hooks → `skippr.claude.hooks` command; Instructions → "Add CLAUDE.md" primary, "Rule" type label, `.md` file extension

### Built-in Extension Grouping (Core VS Code)

In core VS Code, customization items contributed by the default chat extension (`productService.defaultChatAgent.chatExtensionId`, typically `Skippr.data-agent`) are grouped under the "Built-in" header in the management editor list widget, separate from third-party "Extensions".
...
- **Creation targets**: `getSourceFolders()` override replaces VS Code profile user roots with `~/.skippr/{subfolder}` for CLI compatibility
