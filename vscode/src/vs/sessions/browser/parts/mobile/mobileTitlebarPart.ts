 ...
 * account info, skippr status dashboard, and sign-in/sign-out actions.
 */
	...
	private showAccountPanel(): void {
		...
		panelStore.add({
			dispose: () => {
				...
				this.skipprDashboardStore.clear();
				this.renderAccountState();
			}
		});
		...
		// Skippr status dashboard — only when signed in AND entitlements
		// have resolved. When entitlement is Unknown or Available (setup
		// pending), the dashboard shows a "Set up Skippr" prompt that
