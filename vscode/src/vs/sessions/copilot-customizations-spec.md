# Skippr Agent Runtime — Customization Surface Spec
...
>
> **Source:** `github/skippr-agent-runtime` codebase as of 2026-02-25.
...
|------------|-------------|-------|
| Skippr | `{repo}/.github/skippr-instructions.md` | Primary repo instructions |
...
| User | `~/.skippr/instructions/**/*.instructions.md` | `readUserSkipprInstructions()` |

### 1.3 User-Level Instructions

| Scope | File Pattern | Code Reference |
|-------|-------------|----------------|
| User global | `~/.skippr/skippr-instructions.md` | `hasHomeSkipprInstructions()` |
...
- `{cwd}/.github/skippr-instructions.md`
...
1. User global (`~/.skippr/skippr-instructions.md`)
...
|--------|-------------|----------------|
| Repo (Skippr) | `{repo}/.github/skills/*/SKILL.md` | `loader.ts` collectProjectDirs |
...
| User (Skippr) | `~/.skippr/skills/*/SKILL.md` | `loader.ts` personalDirs |
...
| Source | File Pattern | Code Reference |
|--------|-------------|----------------|
| Repo (Skippr) | `{repo}/.github/agents/*.md`, `*.agent.md` | `useCustomAgents.ts` |
| Repo (Claude) | `{repo}/.claude/agents/*.md`, `*.agent.md` | `useCustomAgents.ts` |
| User (Skippr) | `~/.github/agents/*.md`, `*.agent.md` | `useCustomAgents.ts` |
...
|----------|--------|-------------|----------------|
| 1 (lowest) | User | `~/.skippr/mcp-config.json` | `mcp-config.ts` |
...
All `~/.skippr/` paths respect XDG overrides:

| Type | Default | XDG Override |
|------|---------|-------------|
| Config files | `~/.skippr/` | `$XDG_CONFIG_HOME/.skippr/` |
| State/cache | `~/.skippr/` | `$XDG_STATE_HOME/.skippr/` |

The base directory name is always `.skippr` (`APP_DIRECTORY` in `path-helpers.ts`).
...
Message received
 ...
 │   ├─ Base agent prompt
 │   ├─ User instructions      ~/.skippr/skippr-instructions.md
 │   ├─ Repo instructions       .github/skippr-instructions.md, AGENTS.md, CLAUDE.md, GEMINI.md
 ...
 ├─ Tools assembled
 │   ├─ Built-in tools
 │   ├─ MCP servers             ~/.skippr/mcp-config.json + .mcp.json + .vscode/mcp.json + plugins
