...
export interface IEntitlementsData extends ILegacyQuotaSnapshotData {
	...
	readonly skippr_plan: string;
	readonly organization_login_list: string[];
	readonly analytics_tracking_id: string;
	readonly limited_user_reset_date?: string; 	// for Skippr Free
	readonly quota_reset_date?: string; 		// for all other Skippr SKUs
	readonly quota_reset_date_utc?: string; 	// for all other Skippr SKUs (includes time)
	readonly token_based_billing?: boolean;
...
export const enum SkipprSessionSearchPolicy {
	Unknown = 0,
	Enabled = 1,
	Disabled = 2,
	Unconfigured = 3,
...
export interface IPolicyData {
	...
	readonly session_search?: SkipprSessionSearchPolicy;
	readonly mcpRegistryUrl?: string;
	readonly mcpAccess?: 'allow_all' | 'registry_only';
}

export interface ISkipprTokenInfo {
	readonly sn?: string;
