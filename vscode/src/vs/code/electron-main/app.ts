...
import { AgentHostEnabledSettingId, isSkipprDefaultChatProduct } from '../../platform/agentHost/common/agentService.js';
...
export class CodeApplication extends Disposable {
	...
	private async initServices(machineId: string, sqmId: string, devDeviceId: string, sharedProcessReady: Promise<MessagePortClient>): Promise<IInstantiationService> {
		...
		// Agent Host
		if (this.configurationService.getValue(AgentHostEnabledSettingId) || isSkipprDefaultChatProduct(this.productService)) {
	...
	private initChannels(accessor: ServicesAccessor, mainProcessElectronServer: ElectronIPCServer, sharedProcessClient: Promise<MessagePortClient>): void {
		...
		// HTTP(S) from renderer (e.g. chat entitlement) — main uses Electron `net` so requests are not subject to browser CORS.
		...
		sharedProcessClient.then(client => client.registerChannel('request', requestChannel));
