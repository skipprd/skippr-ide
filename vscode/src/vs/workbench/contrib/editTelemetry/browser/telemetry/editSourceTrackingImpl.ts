...
class TrackedDocumentInfo extends Disposable {
	...
	async sendTelemetry(mode: EditTelemetryMode, trigger: EditTelemetryTrigger, t: DocumentEditSourceTracker, focusTime: number, actualTime: number) {
		...
		this._telemetryService.publicLog2<{
			...
			inlineCompletionsSkipprModifiedCount: number;
			...
			inlineCompletionsSkipprModifiedCount: { classification: 'SystemMetaData'; purpose: 'PerformanceAndHealth'; comment: 'Fraction of inline completions skippr modified characters'; isMeasurement: true };
			...
			inlineCompletionsSkipprModifiedCount: data.inlineCompletionsSkipprModifiedCount,
	...
	getTelemetryData(ranges: readonly TrackedEdit[]) {
		const getEditCategory = (source: EditSource) => {
			...
			if (source.category === 'ai' && source.kind === 'completion' && source.extensionId === 'skippr.data-agent') { return 'inlineCompletionsSkippr'; }
			if (source.category === 'ai' && source.kind === 'completion' && source.extensionId === 'skippr.data-agent' && source.providerId === 'completions') { return 'inlineCompletionsSkippr'; }
			if (source.category === 'ai' && source.kind === 'completion' && source.extensionId === 'skippr.data-agent' && source.providerId === 'nes') { return 'inlineCompletionsNES'; }
		...
		return {
			...
			inlineCompletionsSkipprModifiedCount: sums.inlineCompletionsSkippr ?? 0,
