...
export class EditTrackingFeature extends Disposable {
	...
	constructor(
		...
		const skipprInstalled = getExtensionInfoObs('GitHub.skippr', this._extensionService);
		const skipprChatInstalled = getExtensionInfoObs('Skippr.data-agent', this._extensionService);

		const shouldSendDetails = derived(reader => editSourceDetailsEnabled.read(reader) || !!skipprInstalled.read(reader) || !!skipprChatInstalled.read(reader));
