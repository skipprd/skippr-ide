...
class TerminalInitialHintWidget extends Disposable {
	...
	private _getHintInlineChat() {
		...
		const skipprCliEnabled = this._configurationService.getValue(TerminalInitialHintSettingId.SkipprCli);

		// Chat hint
		if (!this._chatEntitlementService.sentiment.hidden) {
			if (skipprCliEnabled) {
				// Skippr Agent hint
				const handleSkipprCliClick = () => {
					this._telemetryService.publicLog2<WorkbenchActionExecutedEvent, WorkbenchActionExecutedClassification>('workbenchActionExecuted', {
						id: 'terminalSkipprCli.hintAction',
						from: 'hint'
					});
					this._instance.sendText('skippr', false);
				};
				const skipprCliHint = localize({
					key: 'skipprCliHint',
					comment: [
						'Preserve double-square brackets and their order',
					]
				}, "Type [[skippr]] to use Skippr Agent.");
				const skipprCliHintHandler: IContentActionHandler = {
					callback: () => handleSkipprCliClick(),
					disposables: this._toDispose
				};
				hintElement.appendChild(renderFormattedText(skipprCliHint, { actionHandler: skipprCliHintHandler }));
				ariaLabelParts.push(localize('skipprCliHintAriaLabel', "Type skippr to use Skippr Agent."));
			} else {
		...
		// Suggest hint (skip when Skippr Agent hint is shown)
		const suggestEnabled = !skipprCliEnabled && this._configurationService.getValue<boolean>(TerminalSuggestSettingId.Enabled);
