...
export class ToolTerminalCreator {
	...
	async createTerminal(shellOrProfile: string | ITerminalProfile, os: OperatingSystem, token: CancellationToken): Promise<IToolTerminal> {
		const instance = await this._createSkipprTerminal(shellOrProfile, os);
	...
	private _createSkipprTerminal(shellOrProfile: string | ITerminalProfile, os: OperatingSystem) {
		...
		const env: Record<string, string> = {
			...
			COPILOT_AGENT: '1',
			// Avoid making `git diff` interactive when called from skippr
			GIT_PAGER: 'cat',
