 ...
 *
 * Steps:
 * 1. Sign In — sessions-style sign-in hero with Skippr Data Agent, Google, and Apple options
...
export class OnboardingVariationA extends Disposable implements IOnboardingService {
	...
	private _renderSignInStep(container: HTMLElement): void {
		...
		// Skippr Data Agent disclaimer
		const skipprDisclaimer = append(disclaimerCol, $('.onboarding-a-signin-disclaimer'));
		skipprDisclaimer.append(localize('onboarding.signIn.disclaimer.prefix', "By signing in, you agree to {0}'s ", defaultChat.provider.default.name));
		this._createInlineLink(skipprDisclaimer, localize('onboarding.signIn.disclaimer.terms', "Terms"), defaultChat.termsStatementUrl);
		skipprDisclaimer.append(localize('onboarding.signIn.disclaimer.middle', " and "));
		this._createInlineLink(skipprDisclaimer, localize('onboarding.signIn.disclaimer.privacy', "Privacy Statement"), defaultChat.privacyStatementUrl);
		skipprDisclaimer.append(localize('onboarding.signIn.disclaimer.skipprPrefix', ". {0} Skippr may show ", defaultChat.provider.default.name));
		this._createInlineLink(skipprDisclaimer, localize('onboarding.signIn.disclaimer.publicCode', "public code"), defaultChat.publicCodeMatchesUrl);
		skipprDisclaimer.append(localize('onboarding.signIn.disclaimer.improveSuffix', " suggestions and use your data to improve the product."));
		skipprDisclaimer.append(' ');
		skipprDisclaimer.append(localize('onboarding.signIn.disclaimer.settingsPrefix', "You can change these "));
		this._createInlineLink(skipprDisclaimer, localize('onboarding.signIn.disclaimer.settings', "settings"), defaultChat.manageSettingsUrl);
		skipprDisclaimer.append(localize('onboarding.signIn.disclaimer.suffix', " anytime."));
	}
	...
	private async _handleSignIn(socialProvider?: string): Promise<void> {
		...
		try {
			const account = await this.defaultAccountService.signIn({
				extraAuthorizeParameters: { get_started_with: 'skippr-vscode' },
	...
	private _renderAgentSessionsStep(container: HTMLElement): void {
		...
		this._createFeatureCard(moreGrid, Codicon.rocket,
			...
			localize('onboarding.sessions.runAnywhere.desc', "Run agents locally for interactive work, in the background with Skippr Agent, or in the cloud with cloud agents that open a pull request your team can review."));

		this._createFeatureCard(moreGrid, Codicon.settingsGear,
			localize('onboarding.sessions.customize', "Customize Your Agents"),
			localize('onboarding.sessions.customize.desc', "Tailor Skippr to your project with custom instructions and agents, skills, reusable prompts, and MCP servers that connect to the tools and context you rely on."));

		// Tutorial link at bottom of content, above footer
		const docsRow = append(wrapper, $('.onboarding-a-sessions-docs'));
		this._createDocLink(docsRow, localize('onboarding.sessions.agentsTutorial', "Agents tutorial"), 'https://skippr.com/ide/docs/docs/skippr/agents/agents-tutorial', 'agentsTutorial');
	}
