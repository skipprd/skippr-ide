...
suite('Workbench - TerminalInstance', () => {
	...
	suite('TerminalLabelComputer', () => {
		...
		test('should use ${sequence} for agent CLI shell types', () => {
			...
			terminalLabelComputer.refreshLabel(createInstance({ capabilities, shellType: GeneralShellType.Skippr, sequence: 'Skippr Agent', processName: 'skippr' }));
			strictEqual(terminalLabelComputer.title, 'Skippr Agent');
		});
		...
		test('should prefer shellLaunchConfig.titleTemplate over agent CLI shell type override', () => {
			const terminalLabelComputer = createLabelComputer({ terminal: { integrated: { tabs: { separator: ' - ', title: '${process}', description: '${cwd}', allowAgentCliTitle: true } } } });
			terminalLabelComputer.refreshLabel(createInstance({ capabilities, shellType: GeneralShellType.Skippr, sequence: 'Skippr Agent', processName: 'skippr', shellLaunchConfig: { titleTemplate: '${process}' } }));
			strictEqual(terminalLabelComputer.title, 'skippr');
		});
