...
const agentCliTitlePatterns: ReadonlyMap<GeneralShellType, RegExp> = new Map([
	...
	[GeneralShellType.Skippr, /\bskippr\b/i],
			...
			switch (e.id) {
				...
				case TerminalCapability.CommandDetection: {
					...
					store.add(e.capability.onCommandExecuted(async (command) => {
						// Only generate ID if command doesn't already have one (i.e., it's a manual command, not Skippr-initiated)
	...
	private _relaunchWithShellIntegrationDisabled(exitMessage: string | undefined): void {
		...
		this.statusList.add({
			...
			hoverActions: [{
				...
				run: () => {
					this._openerService.open('https://skippr.com/ide/docs/docs/editor/integrated-terminal#_shell-integration');
				}
			}, {
