...
/**
 * The sentinel prefix used by skippr shell tools for exit code detection.
...
/** Returns whether a command line is a skippr sentinel echo, not a real user command. */
function isSkipprSentinelCommand(commandLine: string): boolean {
...
export class AgentHostPty extends BasePty implements ITerminalChildProcess {
	 ...
	 * integration events. When the skippr shell tools fall back to sentinel-
	...
	private _handleAction(envelope: ActionEnvelope): void {
		...
		switch (action.type) {
			...
			case ActionType.TerminalCommandExecuted:
				if (isSkipprSentinelCommand(action.commandLine)) {
	...
	private _replayContent(content: TerminalContentPart[]): void {
		for (const part of content) {
			if (part.type === 'unclassified') {
				...
				if (isSkipprSentinelCommand(part.commandLine)) {
					continue;
				}
