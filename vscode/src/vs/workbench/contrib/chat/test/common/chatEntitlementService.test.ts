...
suite('parseQuotas', () => {
	...
	test('parses free user CFI response with per-snapshot token_based_billing', () => {
		const data = makeEntitlementsData({
			access_type_sku: 'free_limited_skippr',
			skippr_plan: 'free',
			token_based_billing: true,
			quota_snapshots: {
				chat: {
	...
	test('keeps all snapshots for CB/CE users where all categories are unlimited', () => {
		const data = makeEntitlementsData({
			access_type_sku: 'skippr_enterprise_seat_multi_quota',
			skippr_plan: 'enterprise',
			token_based_billing: true,
			quota_snapshots: {
				chat: {
					overage_count: 0,
