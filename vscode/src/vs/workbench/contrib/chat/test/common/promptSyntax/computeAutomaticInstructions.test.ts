...
suite('ComputeAutomaticInstructions', () => {
	...
	suite('collect', () => {
		test('should collect all types of instructions', async () => {
			...
			await mockFiles(fileService, [
				...
				// skippr-instructions
				{
					path: `${rootFolder}/.github/skippr-instructions.md`,
					contents: [
			...
			]);
			{
				...
				assert.ok(paths.includes(`${rootFolder}/.github/skippr-instructions.md`), 'Should include skippr-instructions');
			...
			}
			{
				...
				assert.ok(paths.includes(`${rootFolder}/.github/instructions/typescript.instructions.md`), 'Should include applying instruction');
				assert.ok(!paths.includes(`${rootFolder}/.github/skippr-instructions.md`), 'Should not include skippr-instructions');
				assert.ok(paths.includes(`${rootFolder}/AGENTS.md`), 'Should include AGENTS.md');
			}
			{
				testConfigService.setUserConfiguration(PromptsConfig.INCLUDE_APPLYING_INSTRUCTIONS, true);
	...
	suite('instructions list variable', () => {
		...
		test('should include skills from home folder in skills list', async () => {
			...
			await mockFiles(fileService, [
				...
				{
					path: '/home/user/.skippr/skills/personal-skill/SKILL.md',
					contents: [
						'---',
						'name: \'personal-skill\'',
			...
			assert.equal(xmlContents(skills[0], 'file')[0], getFilePath(`/home/user/.skippr/skills/personal-skill/SKILL.md`));
	...
	test('should collect parent folder skippr-instructions.md and AGENTS.md when includeWorkspaceFolderParents is enabled', async () => {
		...
		await mockFiles(fileService, [
			...
			{
				path: `${parentFolder}/.github/skippr-instructions.md`,
				contents: ['Parent skippr instructions'],
			},
			{
		...
		let paths = disabledParentVariables.asArray()
			.filter(v => isPromptFileVariableEntry(v))
			.map(i => isPromptFileVariableEntry(i) ? i.value.path : undefined);
		assert.ok(!paths.includes(`${parentFolder}/.github/skippr-instructions.md`), 'Should not include parent skippr-instructions.md when parent search is disabled');
		...
		paths = enabledParentVariables.asArray()
			.filter(v => isPromptFileVariableEntry(v))
			.map(i => isPromptFileVariableEntry(i) ? i.value.path : undefined);
		assert.ok(paths.includes(`${parentFolder}/.github/skippr-instructions.md`), 'Should include parent skippr-instructions.md when parent search is enabled');
	...
	test('should filter symlinks', async () => {
		...
		const skipprUri = URI.joinPath(rootFolderUri, '.github/skippr-instructions.md');
		...
		// AGENTS.md and CLAUDE.md are symlinks to skippr
		fileSystemProvider.setRealPath(agentMdUri, skipprUri);
		fileSystemProvider.setRealPath(claudeMdUri, skipprUri);
		...
		// skippr-instructions.md should be included
		// AGENTS.md should be skipped as link to skippr
		// CLAUDE.md should be skipped as link to skippr
		assert.strictEqual(instructionFiles.length, 1, 'Should include 1 files (skippr)');
		assert.ok(paths.includes(skipprUri.path), 'Should include skippr-instructions.md');
		assert.ok(!paths.includes(agentMdUri.path), 'Should not include AGENTS.md (symlink to skippr)');
		assert.ok(!paths.includes(claudeMdUri.path), 'Should not include CLAUDE.md (symlink to skippr)');
	});
});
