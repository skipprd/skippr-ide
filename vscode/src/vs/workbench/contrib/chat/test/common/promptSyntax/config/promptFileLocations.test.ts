...
suite('promptFileLocations', function () {
	...
	suite('getPromptFileType', () => {
		...
		test('.md files in ~/.skippr/agents/ subfolder should NOT be recognized as agent files', () => {
			const uri = URI.file('/home/user/.skippr/agents/subfolder/test.md');
			assert.strictEqual(getPromptFileType(uri), undefined);
		});
		...
		test('.md files in ~/.skippr/agents/ folder should be recognized as agent files', () => {
			const uri = URI.file('/home/user/.skippr/agents/my-agent.md');
			assert.strictEqual(getPromptFileType(uri), PromptsType.agent);
		});

		test('README.md in ~/.skippr/agents/ should NOT be recognized as agent file', () => {
			const uri = URI.file('/home/user/.skippr/agents/README.md');
			assert.strictEqual(getPromptFileType(uri), undefined);
		});
		...
		test('skippr-instructions.md should be recognized as instructions', () => {
			const uri = URI.file('/workspace/.github/skippr-instructions.md');
			assert.strictEqual(getPromptFileType(uri), PromptsType.instructions);
		});
	...
	suite('getCleanPromptName', () => {
		...
		test('removes .md extension for files in ~/.skippr/agents/', () => {
			const uri = URI.file('/home/user/.skippr/agents/my-agent.md');
		...
		test('removes .md extension for skippr-instructions.md', () => {
			const uri = URI.file('/workspace/.github/skippr-instructions.md');
			assert.strictEqual(getCleanPromptName(uri), 'skippr-instructions');
		});
