...
suite('PromptsService', () => {
	...
	suite('findInstructionFilesFor', () => {
		...
		test('skippr-instructions and AGENTS.md', async () => {
			const rootFolderName = 'skippr-instructions-and-agents';
			const rootFolder = `/${rootFolderName}`;
			const rootFolderUri = URI.file(rootFolder);
			...
			assert.deepStrictEqual(
				...
				[
					URI.joinPath(rootFolderUri, '.github/skippr-instructions.md').path,
			...
			await mockFiles(fileService, [
				{
					...
					contents: [
						'---',
						'description: \'Skippr Data Agent specialized agent.\'',
						'target: \'github-skippr\'',
						'tools: [ github-api, code-search ]',
						'---',
						'I am optimized for Skippr Data Agent workflows.',
					]
				},
				{
			...
			const expected: ICustomAgent[] = [
				{
					name: 'github-agent',
					description: 'Skippr Data Agent specialized agent.',
					target: Target.GitHubSkippr,
					tools: ['github-api', 'code-search'],
					agentInstructions: {
						content: 'I am optimized for Skippr Data Agent workflows.',
						toolReferences: [],
						metadata: undefined
					},
					handOffs: undefined,
		...
		test('claude agent maps tools and model to vscode equivalents', async () => {
			...
			await mockFiles(fileService, [
				...
				},
				{
					...
					path: `${rootFolder}/.github/agents/skippr-agent.agent.md`,
					contents: [
						'---',
						'description: \'Skippr agent with same tool names.\'',
						'target: \'github-skippr\'',
						'tools: [ Read, Edit ]',
						'model: gpt-4',
						'---',
			...
			const expected: ICustomAgent[] = [
				{
					...
					agents: undefined,
					hooks: undefined,
					uri: URI.joinPath(rootFolderUri, '.github/agents/skippr-agent.agent.md'),
					sessionTypes: undefined,
					source: { storage: PromptsStorage.local },
					enabled: true,
				},
				...
				},
				{
					...
					model: ['Claude Sonnet 4.5 (skippr)'],
					agentInstructions: {
	...
	suite('listPromptFiles - skills ', () => {
		...
		test('should list skill files from user home', async () => {
			...
			const skipprSkill = personalSkills.find(s => s.uri.path.includes('.skippr'));
			assert.ok(skipprSkill, 'Should find skippr personal skill');
	...
	suite('findAgentSkills', () => {
		...
		test('should find skills in workspace and user home', async () => {
			...
			await mockFiles(fileService, [
				...
				{
					path: '/home/user/.skippr/skills/Skippr Skill 1/SKILL.md',
					contents: [
						'---',
						'name: "Skippr Skill 1"',
						'description: "A Skippr skill for testing"',
						'---',
						'This is Skippr skill 1 content',
					],
				},
			]);
			...
			const skipprSkill1 = personalSkills.find(skill => skill.name === 'Skippr Skill 1');
			assert.ok(skipprSkill1, 'Should find Skippr Skill 1');
			assert.strictEqual(skipprSkill1.description, 'A Skippr skill for testing');
			assert.strictEqual(skipprSkill1.uri.path, '/home/user/.skippr/skills/Skippr Skill 1/SKILL.md');
		});
		...
		test('should skip duplicate skill names and keep first by priority', async () => {
			...
			await mockFiles(fileService, [
				...
				{
					path: '/home/user/.skippr/skills/Duplicate Skill/SKILL.md',
					contents: [
						'---',
						'name: "Duplicate Skill"',
		...
		test('should prioritize skills by source: workspace > user > extension', async () => {
			...
			await mockFiles(fileService, [
				{
					path: '/home/user/.skippr/skills/Priority Skill/SKILL.md',
					contents: [
						'---',
						'name: "Priority Skill"',
