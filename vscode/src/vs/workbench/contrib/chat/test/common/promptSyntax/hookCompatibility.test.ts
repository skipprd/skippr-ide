...
import { parseSkipprHooks, parseHooksFromFile, HookSourceFormat } from '../../../common/promptSyntax/hookCompatibility.js';
import { URI } from '../../../../../../base/common/uri.js';

suite('HookCompatibility', () => {
	ensureNoDisposablesAreLeakedInTestSuite();

	suite('parseSkipprHooks', () => {
		...
		suite('basic parsing', () => {
			test('parses simple hook with command', () => {
				...
				const result = parseSkipprHooks(json, workspaceRoot, userHome);
		...
		suite('invalid inputs', () => {
			test('returns empty result for null json', () => {
				const result = parseSkipprHooks(null, workspaceRoot, userHome);
				assert.strictEqual(result.size, 0);
			});

			test('returns empty result for undefined json', () => {
				const result = parseSkipprHooks(undefined, workspaceRoot, userHome);
				assert.strictEqual(result.size, 0);
			});

			test('returns empty result for missing hooks property', () => {
				const result = parseSkipprHooks({}, workspaceRoot, userHome);
				assert.strictEqual(result.size, 0);
			});
		});
		...
		suite('Claude-style matcher compatibility', () => {
			test('parses Claude-style nested matcher structure', () => {
				// When Claude format is pasted into Skippr hooks file
				const json = {
					hooks: {
						PreToolUse: [
							{
	...
	suite('parseHooksFromFile', () => {
		...
		test('uses Skippr format for .github/hooks/*.json files', () => {
			...
			assert.strictEqual(result.format, HookSourceFormat.Skippr);
		...
		test('disableAllHooks is ignored for Skippr format', () => {
			...
			// Skippr format does not support disableAllHooks
			assert.strictEqual(result.disabledAllHooks, false);
			assert.strictEqual(result.hooks.size, 1);
		});
