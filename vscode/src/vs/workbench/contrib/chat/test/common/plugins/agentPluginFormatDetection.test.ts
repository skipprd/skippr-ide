...
suite('AgentPlugin format detection', () => {
	...
	test('falls back to Skippr format when no vendor manifest exists', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
		const uri = pluginUri('/plugins/my-skippr-plugin');
		await writeFile('/plugins/my-skippr-plugin/plugin.json', JSON.stringify({ name: 'my-skippr-plugin' }));
		await writeFile('/plugins/my-skippr-plugin/commands/run.md', '# Run');
