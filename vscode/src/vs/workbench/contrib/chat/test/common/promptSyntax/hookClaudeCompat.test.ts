...
suite('HookSourceFormat', () => {
	...
	suite('getHookSourceFormat', () => {
		...
		test('returns Skippr format for .github/hooks/hooks.json', () => {
			assert.strictEqual(getHookSourceFormat(URI.file('/workspace/.github/hooks/hooks.json')), HookSourceFormat.Skippr);
		});

		test('returns Skippr format for arbitrary .json file', () => {
			assert.strictEqual(getHookSourceFormat(URI.file('/workspace/.github/hooks/my-hooks.json')), HookSourceFormat.Skippr);
		});

		test('returns Skippr format for settings.json not inside .claude', () => {
			assert.strictEqual(getHookSourceFormat(URI.file('/workspace/.vscode/settings.json')), HookSourceFormat.Skippr);
		});
	});

	suite('buildNewHookEntry', () => {
		test('builds Skippr format entry', () => {
			assert.deepStrictEqual(buildNewHookEntry(HookSourceFormat.Skippr), {
				type: 'command',
				command: ''
			});
		});
		...
		test('Skippr format entry serializes correctly in JSON', () => {
			const entry = buildNewHookEntry(HookSourceFormat.Skippr);
			const hooksContent = {
				hooks: {
					SubagentStart: [entry]
				}
