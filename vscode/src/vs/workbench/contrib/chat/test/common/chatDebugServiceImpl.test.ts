...
suite('ChatDebugServiceImpl', () => {
	...
	const skipprCliSession = URI.parse('skipprcli:/test-session-id');
	...
	suite('addEvent and getEvents', () => {
		...
		test('should handle different event kinds', () => {
			...
			const modelTurn: IChatDebugModelTurnEvent = {
				...
				outputTokens: 50,
				totalTokens: 150,
				skipprUsageNanoAiu: 5_000_000_000,
			...
			assert.strictEqual((events[1] as IChatDebugModelTurnEvent).skipprUsageNanoAiu, 5_000_000_000);
		});
	});
	...
	suite('log', () => {
		...
		test('should log events for skipprcli sessions', () => {
			const firedEvents: IChatDebugEvent[] = [];
			disposables.add(service.onDidAddEvent(e => firedEvents.push(e)));

			service.log(skipprCliSession, 'cli-event', 'details');

			assert.strictEqual(firedEvents.length, 1);
			assert.strictEqual(service.getEvents(skipprCliSession).length, 1);
		});
	...
	suite('invokeProviders', () => {
		...
		test('should invoke providers for skipprcli sessions', async () => {
			let providerCalled = false;
			...
			const provider: IChatDebugLogProvider = {
				provideChatDebugLog: async () => {
					...
					return [{
						kind: 'generic',
						sessionResource: skipprCliSession,
						created: new Date(),
			...
			disposables.add(service.registerProvider(provider));
			await service.invokeProviders(skipprCliSession);

			assert.strictEqual(providerCalled, true);
			assert.ok(service.getEvents(skipprCliSession).length > 0);
		});
