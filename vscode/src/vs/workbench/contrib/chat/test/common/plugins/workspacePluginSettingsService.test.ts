...
suite('WorkspacePluginSettingsService', () => {
	...
	async function writeSkipprSettings(content: string): Promise<void> {
		const uri = URI.from({ scheme: Schemas.inMemory, path: '/workspace/.github/skippr/settings.json' });
		await fileService.writeFile(uri, VSBuffer.fromString(content));
	}
	...
	test('merges enabledPlugins from Claude and Skippr settings', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
		await writeClaudeSettings(JSON.stringify({
			enabledPlugins: { 'from-claude@mp': true }
		}));
		await writeSkipprSettings(JSON.stringify({
			enabledPlugins: { 'from-skippr@mp': true }
		}));
		...
		assert.strictEqual(enabled.get('from-skippr@mp'), true);
	}));

	test('Claude enabledPlugins take precedence over Skippr for same key', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
		await writeClaudeSettings(JSON.stringify({
	...
	test('deduplicates marketplaces across Claude and Skippr by canonical ID', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
		await writeClaudeSettings(JSON.stringify({
			extraKnownMarketplaces: {
		...
		await writeSkipprSettings(JSON.stringify({
			extraKnownMarketplaces: {
				'skippr-name': { source: 'github', repo: 'owner/repo' }
			}
		}));
