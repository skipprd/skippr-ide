...
suite('PromptFileParser', () => {
	...
	test('agent with all header fields including colons in description', async () => {
		...
		const content = [
			...
			`model: ['Claude Haiku 4.5 (skippr)', 'Gemini 3 Flash (Preview) (skippr)', 'Auto (skippr)']`,
			'target: vscode',
			'user-invocable: false',
		...
		assert.deepEqual(result.header.model, ['Claude Haiku 4.5 (skippr)', 'Gemini 3 Flash (Preview) (skippr)', 'Auto (skippr)']);
