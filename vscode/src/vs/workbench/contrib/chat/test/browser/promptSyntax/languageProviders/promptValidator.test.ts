...
suite('PromptValidator', () => {
	...
	setup(async () => {
		...
		const testModels: ILanguageModelChatMetadata[] = [
			...
			{ id: 'mae-4.1', name: 'MAE 4.1', vendor: 'skippr', version: '1.0', family: 'mae', modelPickerCategory: undefined, extension: new ExtensionIdentifier('a.b'), isUserSelectable: true, maxInputTokens: 8192, maxOutputTokens: 1024, capabilities: { agentMode: true, toolCalling: true }, isDefaultForLocation: { [ChatAgentLocation.Chat]: true } } satisfies ILanguageModelChatMetadata,
			{ id: 'mae-3.5-turbo', name: 'MAE 3.5 Turbo', vendor: 'skippr', version: '1.0', family: 'mae', modelPickerCategory: undefined, extension: new ExtensionIdentifier('a.b'), isUserSelectable: true, maxInputTokens: 8192, maxOutputTokens: 1024, isDefaultForLocation: { [ChatAgentLocation.Chat]: true } } satisfies ILanguageModelChatMetadata
		];
	...
	suite('agents', () => {
		...
		test('github-skippr agent with supported attributes', async () => {
			const content = [
				'---',
				'name: "GitHub_Skippr_Custom_Agent"',
				'description: "Skippr Data Agent agent"',
				'target: github-skippr',
			...
			const markers = await validate(content, PromptsType.agent);
			assert.deepStrictEqual(markers, [], 'Expected no validation issues for github-skippr target');
		});

		test('github-skippr agent warns about model and handoffs attributes', async () => {
			const content = [
				'---',
				'name: "GitHubAgent"',
				'description: "Skippr Data Agent agent"',
			...
			assert.deepStrictEqual(messages, [
				'Attribute \'model\' is not supported in custom Skippr Data Agent agent files. Supported: description, github, infer, mcp-servers, name, target, tools.',
				'Attribute \'handoffs\' is not supported in custom Skippr Data Agent agent files. Supported: description, github, infer, mcp-servers, name, target, tools.',
			], 'Model and handoffs are not validated for github-skippr target');
		});

		test('github-skippr agent does not validate variable references', async () => {
			const content = [
				'---',
				'name: "GitHubAgent"',
				'description: "Skippr Data Agent agent"',
			...
			const markers = await validate(content, PromptsType.agent);
			// Variable references should not be validated for github-skippr target
			assert.deepStrictEqual(markers, [], 'Variable references are not validated for github-skippr target');
		});

		test('github-skippr agent rejects unsupported attributes', async () => {
			const content = [
				'---',
				'name: "GitHubAgent"',
				'description: "Skippr Data Agent agent"',
		...
		test('github-skippr agent with valid permissions', async () => {
			const content = [
				'---',
				'name: "IssueTriage"',
		...
		test('github-skippr agent with invalid permission scope', async () => {
			const content = [
				'---',
				'name: "TestAgent"',
				'description: "Test"',
		...
		test('github-skippr agent with invalid permission value', async () => {
			const content = [
				'---',
				'name: "TestAgent"',
				'description: "Test"',
		...
		test('github-skippr agent with non-map github attribute', async () => {
			const content = [
				'---',
				'name: "TestAgent"',
				'description: "Test"',
		...
		test('github-skippr agent with unknown github sub-property', async () => {
			const content = [
				'---',
				'name: "TestAgent"',
				'description: "Test"',
		...
		test('github-skippr target requires name attribute', async () => {
			// Missing name with github-skippr target
			{
				const content = [
					'---',
					'description: "Skippr Data Agent agent"',
			...
			// Valid name with github-skippr target
			{
				const content = [
					'---',
					'name: "GitHubAgent"',
				...
				const markers = await validate(content, PromptsType.agent);
				assert.deepStrictEqual(markers, [], 'Valid github-skippr agent with name should not produce errors');
			}
	...
	suite('body', () => {
		...
		test('body with tool not present in tools list', async () => {
			const content = [
				...
				'---',
				'I need',
				'#tool:ms-azuretools.vscode-azure-github-skippr/azure_recommend_custom_modes',
			...
			assert.deepEqual(actual, [
				{ message: `Unknown tool or toolset 'ms-azuretools.vscode-azure-github-skippr/azure_recommend_custom_modes'.`, startColumn: 7, endColumn: 77 },
