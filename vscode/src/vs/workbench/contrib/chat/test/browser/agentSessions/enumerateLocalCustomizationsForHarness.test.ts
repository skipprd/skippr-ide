...
suite('enumerateLocalCustomizationsForHarness', () => {
	...
	test('emits built-in skills with BUILTIN_STORAGE even when no other storage source has files', async () => {
		...
		const result = await enumerateLocalCustomizationsForHarness(promptsService, new FakeSyncProvider(), SessionType.SkipprCLI, CancellationToken.None);
	...
	test('marks built-in skills disabled when the sync provider says so', async () => {
		...
		const result = await enumerateLocalCustomizationsForHarness(promptsService, syncProvider, SessionType.SkipprCLI, CancellationToken.None);
