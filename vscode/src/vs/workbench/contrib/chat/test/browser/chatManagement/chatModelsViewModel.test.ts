...
class MockLanguageModelsService implements ILanguageModelsService {
	...
	getVendors(): ILanguageModelProviderDescriptor[] {
		return this.vendors.map(v => ({ ...v, isDefault: v.vendor === 'skippr' }));
	}
...
suite('ChatModelsViewModel', () => {
	...
	setup(async () => {
		...
		languageModelsService.addModel('skippr', 'skippr-gpt-4', {
			extension: new ExtensionIdentifier('skippr.data-agent'),
			id: 'gpt-4',
			name: 'GPT-4',
			family: 'gpt-4',
			version: '1.0',
			...
			modelPickerCategory: { label: 'Skippr', order: 1 },
		...
		languageModelsService.addModel('skippr', 'skippr-gpt-4o', {
			extension: new ExtensionIdentifier('skippr.data-agent'),
			id: 'gpt-4o',
			name: 'GPT-4o',
			family: 'gpt-4',
	...
	test('should filter by provider name (vendor ID and display name)', () => {
		const resultsBySkipprId = viewModel.filter('@provider:skippr');
		assert.strictEqual(resultsBySkipprId.length, 3);
		assert.strictEqual(resultsBySkipprId[0].type, 'vendor');
		assert.strictEqual(resultsBySkipprId[0].vendorEntry.vendor.vendor, 'skippr');
		assert.strictEqual(resultsBySkipprId[1].type, 'model');
		assert.strictEqual(resultsBySkipprId[1].model.identifier, 'skippr-gpt-4');
		assert.strictEqual(resultsBySkipprId[2].type, 'model');
		assert.strictEqual(resultsBySkipprId[2].model.identifier, 'skippr-gpt-4o');
	...
	test('should filter by multiple providers with OR logic', () => {
		const results = viewModel.filter('@provider:skippr @provider:openai');
	...
	test('should combine provider and capability filters', () => {
		const results = viewModel.filter('@provider:skippr @capability:vision');

		const models = results.filter(r => !isLanguageModelProviderEntry(r) && !isLanguageModelGroupEntry(r)) as ILanguageModelEntry[];
		assert.strictEqual(models.length, 2);
		assert.ok(models.every(m =>
			m.model.provider.vendor.vendor === 'skippr' &&
			m.model.metadata.capabilities?.vision === true
		));
	});
	...
	test('should filter by text matching model id', () => {
		...
		const models = results.filter(r => !isLanguageModelProviderEntry(r) && !isLanguageModelGroupEntry(r)) as ILanguageModelEntry[];
		assert.strictEqual(models.length, 1);
		assert.strictEqual(models[0].model.identifier, 'skippr-gpt-4o');
	...
	test('should filter by text matching vendor name', () => {
		...
		const models = results.filter(r => !isLanguageModelProviderEntry(r) && !isLanguageModelGroupEntry(r)) as ILanguageModelEntry[];
		assert.strictEqual(models.length, 2);
		assert.ok(models.every(m => m.model.provider.group.name === 'Skippr Data Agent'));
	});
	...
	test('should toggle vendor collapsed state', () => {
		const vendorEntry = viewModel.viewModelEntries.find(r => isLanguageModelProviderEntry(r) && r.vendorEntry.vendor.vendor === 'skippr') as ILanguageModelProviderEntry;
		viewModel.toggleCollapsed(vendorEntry);

		const results = viewModel.filter('');
		const skipprVendor = results.find(r => isLanguageModelProviderEntry(r) && (r as ILanguageModelProviderEntry).vendorEntry.vendor.vendor === 'skippr') as ILanguageModelProviderEntry;
		assert.ok(skipprVendor);
		assert.strictEqual(skipprVendor.collapsed, true);

		// Models should not be shown when vendor is collapsed
		const skipprModelsAfterCollapse = results.filter(r =>
			!isLanguageModelProviderEntry(r) && (r as ILanguageModelEntry).model.provider.vendor.vendor === 'skippr'
		);
		assert.strictEqual(skipprModelsAfterCollapse.length, 0);

		// Toggle back
		viewModel.toggleCollapsed(vendorEntry);
		const resultsAfterExpand = viewModel.filter('');
		const skipprModelsAfterExpand = resultsAfterExpand.filter(r =>
			!isLanguageModelProviderEntry(r) && (r as ILanguageModelEntry).model.provider.vendor.vendor === 'skippr'
		);
		assert.strictEqual(skipprModelsAfterExpand.length, 2);
	});
	...
	test('should remove filter keywords from text search', () => {
		const results = viewModel.filter('@provider:skippr @capability:vision GPT');

		const models = results.filter(r => !isLanguageModelProviderEntry(r) && !isLanguageModelGroupEntry(r)) as ILanguageModelEntry[];
		// Should only search 'GPT' in model names, not the filter keywords
		assert.strictEqual(models.length, 2);
		assert.ok(models.every(m => m.model.provider.vendor.vendor === 'skippr'));
	});
	...
	function createSingleVendorViewModel(includeSecondModel: boolean = true): { service: MockLanguageModelsService; viewModel: ChatModelsViewModel } {
		...
		service.addModel('skippr', 'skippr-gpt-4', {
			extension: new ExtensionIdentifier('skippr.data-agent'),
			id: 'gpt-4',
			name: 'GPT-4',
			family: 'gpt-4',
		...
		if (includeSecondModel) {
			service.addModel('skippr', 'skippr-gpt-4o', {
				extension: new ExtensionIdentifier('skippr.data-agent'),
				id: 'gpt-4o',
				name: 'GPT-4o',
				family: 'gpt-4',
	...
	test('should always place skippr vendor at the top when multiple vendors exist', async () => {
		// Test with default setup (skippr and openai)
		let results = viewModel.filter('');
		let vendors = results.filter(isLanguageModelProviderEntry) as ILanguageModelProviderEntry[];
		assert.strictEqual(vendors[0].vendorEntry.vendor.vendor, 'skippr');
	...
	test('should get configured vendors', () => {
		...
		assert.ok(vendors.length > 0);
		assert.ok(vendors.some(v => v.vendor.vendor === 'skippr'));
	...
	test('should filter by case-insensitive provider name', () => {
		...
		const results2 = viewModel.filter('@provider:skippr');
