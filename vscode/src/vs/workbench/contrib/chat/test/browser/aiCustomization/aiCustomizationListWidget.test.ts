...
suite('aiCustomizationListWidget', () => {
	...
	suite('extractExtensionIdFromPath', () => {
		test('extracts extension ID from skippr-chat extension path', () => {
			assert.strictEqual(
				extractExtensionIdFromPath('/Users/josh/.vscode-insiders/extensions/skippr.data-agent-0.43.2026040602/assets/prompts/skills/agent-customization/SKILL.md'),
				'skippr.data-agent'
			);
		});
		...
		test('extracts extension ID from Code OSS dev path', () => {
			assert.strictEqual(
				extractExtensionIdFromPath('/Users/josh/.vscode-oss-dev/extensions/skippr.data-agent-0.43.2026040602/assets/prompts/skills/troubleshoot/SKILL.md'),
				'skippr.data-agent'
			);
		});
		...
		test('extracts extension ID from Windows-style path', () => {
			assert.strictEqual(
				extractExtensionIdFromPath('C:/Users/dev/.vscode/extensions/skippr-python.python-2024.1.1/skills/SKILL.md'),
		...
		test('returns undefined for user home paths', () => {
			assert.strictEqual(
				extractExtensionIdFromPath('/Users/josh/.skippr/skills/ios-project-setup/SKILL.md'),
				undefined
			);
		});
		...
		test('extracts extension ID from User/globalStorage path (Skippr Data Agent ask agent)', () => {
			assert.strictEqual(
				extractExtensionIdFromPath('/Users/josh/.vscode-oss-dev/User/globalStorage/skippr.data-agent/ask-agent/Ask.agent.md'),
				'skippr.data-agent'
			);
		});
		...
		test('extracts extension ID from User/globalStorage path on Insiders', () => {
			assert.strictEqual(
				extractExtensionIdFromPath('/Users/josh/Library/Application Support/Code - Insiders/User/globalStorage/skippr.data-agent/ask-agent/Ask.agent.md'),
				'skippr.data-agent'
			);
		});
