...
suite('ChatModelSelectionLogic', () => {
	...
	suite('shouldRestorePersistedModel', () => {
		...
		test('restores model that was explicitly chosen (not default)', () => {
			...
			const result = shouldRestorePersistedModel('skippr/gpt', false, [model], ChatAgentLocation.Chat);
			assert.strictEqual(result.shouldRestore, true);
			assert.strictEqual(result.model?.identifier, 'skippr/gpt');
		});
		...
		test('restores model that was default and is still default', () => {
			...
			const result = shouldRestorePersistedModel('skippr/gpt', true, [model], ChatAgentLocation.Chat);
			assert.strictEqual(result.shouldRestore, true);
		});
		...
		test('does NOT restore model that no longer exists', () => {
			...
			const result = shouldRestorePersistedModel('skippr/gpt', false, [otherModel], ChatAgentLocation.Chat);
			assert.strictEqual(result.shouldRestore, false);
			assert.strictEqual(result.model, undefined);
		});

		test('handles empty models list', () => {
			const result = shouldRestorePersistedModel('skippr/gpt', false, [], ChatAgentLocation.Chat);
			assert.strictEqual(result.shouldRestore, false);
			assert.strictEqual(result.model, undefined);
		});
		...
		test('user choice is preserved when default changes to a different model', () => {
			...
			const claude = createModel('claude', 'Claude');
			const result = shouldRestorePersistedModel('skippr/gpt-4o', false, [gpt, claude], ChatAgentLocation.Chat);
			assert.strictEqual(result.shouldRestore, true);
		...
		test('default tracking: follows new default when user never explicitly chose', () => {
			...
			const result = shouldRestorePersistedModel('skippr/gpt-4o', true, [gpt, claude], ChatAgentLocation.Chat);
	...
	suite('mergeModelsWithCache', () => {
		...
		test('uses live models when available', () => {
			...
			const result = mergeModelsWithCache([liveModel], [cachedModel], new Set(['skippr']));
		...
		test('falls back to cached models when no live models', () => {
			const cachedModel = createModel('cached-gpt', 'Cached GPT');
			const result = mergeModelsWithCache([], [cachedModel], new Set(['skippr']));
			assert.strictEqual(result.length, 1);
		...
		test('evicts cached models from vendors no longer contributed', () => {
			...
			const result = mergeModelsWithCache(
				[liveModel],
				[cachedRemovedVendor],
				new Set(['skippr']), // removed-vendor is NOT contributed
			);
			assert.strictEqual(result.length, 1);
			assert.strictEqual(result[0].metadata.id, 'gpt');
		});
		...
		test('does not duplicate models from same vendor', () => {
			...
			// Both are vendor 'skippr', live vendor takes priority
			assert.strictEqual(result.length, 1);
			assert.strictEqual(result[0].metadata.id, 'gpt');
		});
	...
	suite('model switching scenarios', () => {
		...
		test('persisted model selection survives when model is still default', () => {
			...
			const result = shouldRestorePersistedModel('skippr/gpt-4o', true, [model], ChatAgentLocation.Chat);
			assert.strictEqual(result.shouldRestore, true);
		});
		...
		test('persisted model selection does NOT restore when a new default is assigned', () => {
			...
			const result = shouldRestorePersistedModel('skippr/gpt-4o', true, [gpt4o, newDefault], ChatAgentLocation.Chat);
			assert.strictEqual(result.shouldRestore, false);
		});
		...
		test('user explicit model choice persists even when default changes', () => {
			...
			const result = shouldRestorePersistedModel('skippr/claude', false, [claude, newDefault], ChatAgentLocation.Chat);
			assert.strictEqual(result.shouldRestore, true);
	...
	suite('onDidChangeLanguageModels race conditions', () => {
		...
		test('model temporarily removed then re-added loses user choice', () => {
			...
			assert.strictEqual(shouldResetOnModelListChange('skippr/gpt', [gpt, claude]), false);

			// Step 2: Extension reloads, GPT temporarily disappears from model list
			assert.strictEqual(shouldResetOnModelListChange('skippr/gpt', [claude]), true);
			...
			assert.strictEqual(shouldResetOnModelListChange('skippr/claude', [gpt, claude]), false);
		...
		test('reset when models list is empty', () => {
			assert.strictEqual(shouldResetOnModelListChange('skippr/gpt', []), true);
		});
		...
		test('cache bridges the gap when live models temporarily unavailable', () => {
			...
			const merged = mergeModelsWithCache([], [cachedGpt, cachedClaude], new Set(['skippr']));
			assert.strictEqual(merged.length, 2);

			// Selected model is still found in the cached list
			assert.strictEqual(shouldResetOnModelListChange('skippr/gpt', merged), false);
		});
		...
		test('cache evicted for uncontributed vendor once live models arrive', () => {
			...
			assert.strictEqual(shouldResetOnModelListChange('skippr/gpt', merged), true);
		});
	});
	...
	suite('late-arriving model restoration', () => {
		...
		test('restores explicitly-chosen model that arrives late', () => {
			const model = createModel('gpt', 'GPT');
			assert.strictEqual(
				shouldRestoreLateArrivingModel('skippr/gpt', false, model, ChatAgentLocation.Chat),
				true,
			);
		});
		...
		test('restores model that was default and is still default for location', () => {
			const model = createDefaultModelForLocation('gpt', 'GPT', ChatAgentLocation.Chat);
			assert.strictEqual(
				shouldRestoreLateArrivingModel('skippr/gpt', true, model, ChatAgentLocation.Chat),
				true,
			);
		});
		...
		test('does NOT restore model that is not user-selectable', () => {
			...
			assert.strictEqual(
				shouldRestoreLateArrivingModel('skippr/internal', false, model, ChatAgentLocation.Chat),
				false,
			);
		});
		...
		test('does NOT restore model with isUserSelectable=undefined (treated as falsy)', () => {
			...
			assert.strictEqual(
				shouldRestoreLateArrivingModel('skippr/undef-sel', false, model, ChatAgentLocation.Chat),
				false,
			);
		});
		...
		test('restores model arriving late at a different location where it is default', () => {
			...
			assert.strictEqual(
				shouldRestoreLateArrivingModel('skippr/gpt', true, model, ChatAgentLocation.Terminal),
				true,
			);
			// But not in Chat
			assert.strictEqual(
	...
	suite('checkModelSupported interaction patterns', () => {
		...
		test('initSelectedModel → checkModelSupported: restored model passes Agent check', () => {
			...
			const restoreResult = shouldRestorePersistedModel('skippr/agent-model', false, [agentModel], ChatAgentLocation.Chat);
		...
		test('initSelectedModel → checkModelSupported: restored model FAILS Agent check', () => {
			...
			// 1. shouldRestorePersistedModel says "restore"
			const restoreResult = shouldRestorePersistedModel('skippr/ask-only', false, [askOnlyModel, agentModel], ChatAgentLocation.Chat);
			assert.strictEqual(restoreResult.shouldRestore, true);
	...
	suite('mode with forced model (mode.model property)', () => {
		...
		test('mode forces model — simulating switchModelByQualifiedName success', () => {
			...
			// Simulate: mode wants "GPT-4o (skippr)"
			const qualifiedName = 'GPT-4o (skippr)';
		...
		test('mode forces model — skippr vendor shorthand works', () => {
			const gpt = createModel('gpt-4o', 'GPT-4o');
			// For skippr vendor, just the name works
		...
		test('mode forces model — nonexistent model gracefully misses', () => {
			const gpt = createModel('gpt-4o', 'GPT-4o');
			const match = [gpt].find(m => ILanguageModelChatMetadata.matchesQualifiedName('NonExistent (skippr)', m.metadata));
	...
	suite('realistic multi-step race simulations', () => {
		...
		test('startup: cached model → live models arrive → user choice preserved', () => {
			...
			assert.strictEqual(shouldResetOnModelListChange('skippr/gpt', cachedModels), false);
			...
			assert.strictEqual(shouldResetOnModelListChange('skippr/gpt', liveModels), false);
		});

		test('startup: no cache → models arrive late → persisted choice restored', () => {
			// Step 1: No models available at all
			const emptyModels = computeAvailableModels([], [], new Set(['skippr']), undefined, ChatModeKind.Ask, ChatAgentLocation.Chat);
			assert.strictEqual(emptyModels.length, 0);

			// initSelectedModel: model not found, enters _waitForPersistedLanguageModel path
			const restoreResult = shouldRestorePersistedModel('skippr/gpt', false, emptyModels, ChatAgentLocation.Chat);
			...
			assert.strictEqual(
				shouldRestoreLateArrivingModel('skippr/gpt', false, arrivedModel, ChatAgentLocation.Chat),
				true,
			);
		});
		...
		test('extension reload: selected model flickers out then back', () => {
			...
			// Step 2: Extension reloads, skippr vendor has no live models
			// But cache bridges the gap
			const duringReload = mergeModelsWithCache([], [gpt, claude], new Set(['skippr']));
			assert.strictEqual(shouldResetOnModelListChange('skippr/gpt', duringReload), false);

			// Step 3: Extension finishes loading, live models back
			const afterReload = mergeModelsWithCache([gpt, claude], [gpt, claude], new Set(['skippr']));
			assert.strictEqual(shouldResetOnModelListChange('skippr/gpt', afterReload), false);
		});
		...
		test('extension reload without cache: model lost', () => {
			...
			const duringReload = mergeModelsWithCache([], [], new Set(['skippr']));
			assert.strictEqual(duringReload.length, 0);
			assert.strictEqual(shouldResetOnModelListChange('skippr/gpt', duringReload), true);
			// → Model is lost, reset to default

			// Step 3: Models come back but user's choice is already gone
			const afterReload = mergeModelsWithCache([gpt], [], new Set(['skippr']));
