...
suite('ChatTipService', () => {
	...
	function createProductService(hasSkippr: boolean): IProductService {
		return {
			_serviceBrand: undefined,
			defaultChatAgent: hasSkippr ? { chatExtensionId: 'skippr.data-agent' } : undefined,
		} as IProductService;
	}

	function createService(hasSkippr: boolean = true, tipsEnabled: boolean = true): ChatTipService {
		instantiationService.stub(IProductService, createProductService(hasSkippr));
	...
	test('returns Auto switch tip when current model is persisted and context key is empty', () => {
		storageService.store('chat.currentLanguageModel.panel', 'skippr/gpt-4.1-2025-04-14', StorageScope.APPLICATION, StorageTarget.USER);
		const service = createService();
		contextKeyService.createKey(ChatContextKeys.chatModelId.key, '');
	...
	test('returns undefined when Skippr is not enabled', () => {
		const service = createService(/* hasSkippr */ false);

		const tip = service.getWelcomeTip(contextKeyService);
		assert.strictEqual(tip, undefined, 'Should not return a tip when Skippr is not enabled');
	});
	...
	test('returns undefined when tips setting is disabled', () => {
		const service = createService(/* hasSkippr */ true, /* tipsEnabled */ false);
	...
	test('excludes tip.customInstructions when skippr-instructions.md exists in workspace', async () => {
		const tip = createMockTip({
			id: 'tip.customInstructions',
			excludeWhenPromptFilesExist: { promptType: PromptsType.instructions, agentFileType: AgentInstructionFileType.skipprInstructionsMd, excludeUntilChecked: true },
		});
		...
		const tracker = testDisposables.add(new TipEligibilityTracker(
			...
			storageService,
			createMockPromptsService([{ uri: { path: '/.github/skippr-instructions.md' }, realPath: undefined, type: AgentInstructionFileType.skipprInstructionsMd } as IAgentInstructionFile]) as IPromptsService,
			createMockToolsService(),
			new NullLogService(),
		));
		...
		assert.strictEqual(tracker.isExcluded(tip), true, 'Should be excluded when skippr-instructions.md exists');
	});
