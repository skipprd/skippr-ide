...
suite('stateToProgressAdapter', () => {
	...
	suite('turnsToHistory', () => {
		...
		test('per-turn model id and display name flow from usage.model', () => {
			...
			const lookup = makeLookup('agent-host-skippr:', { 'gpt-5': 'GPT-5', 'opus-4.7': 'Claude Opus 4.7' });
			...
			assert.deepStrictEqual(
				...
				[
					{ type: 'request', modelId: 'agent-host-skippr:gpt-5' },
					{ type: 'response', details: 'GPT-5' },
					{ type: 'request', modelId: 'agent-host-skippr:opus-4.7' },
		...
		test('falls back to session-level model when turn has no usage.model', () => {
			...
			const lookup = makeLookup('agent-host-skippr:', { 'gpt-5': 'GPT-5' }, 'gpt-5');
		...
		test('request history includes restored model id', () => {
			...
			const lookup = makeLookup('agent-host-skippr:', {}, 'gpt-5');
			...
			assert.deepStrictEqual(history[0], {
				...
				modelId: 'agent-host-skippr:gpt-5',
				...
				responseParts: [{
					kind: ResponsePartKind.ToolCall, toolCall: createCompletedToolCall({
						...
						content: [
							...
							{ type: ToolResultContentType.Subagent, resource: 'skippr://session/subagent/tc-1', title: 'Explore', agentName: 'explore', description: 'Explores the codebase' },
						],
						success: true,
					})
				} as ToolCallResponsePart],
		...
		test('sets subagent toolSpecificData from content and notifies state observers', () => {
			...
			const runningTc: ToolCallRunningState = {
				...
				content: [{
					...
					resource: 'skippr://session/subagent/tc-1',
					title: 'Explore',
					agentName: 'explore',
