...
suite('LanguageModelToolsService', () => {
	...
	test('bypass approvals does not auto-approve tools in toolIdsThatCannotBeAutoApproved for CLI sessions', async () => {
		...
		// Create a CLI session URI (authority = 'skipprcli' instead of 'local')
	...
	test('gpt-5.5 readFile setting controls Skippr read tool availability', () => {
		const readTool: IToolData = {
			id: SkipprToolId.ReadFile,
		...
		const gpt55Model = { id: 'gpt-5.5', vendor: 'skippr', family: 'gpt-5.5', version: '1.0' } as ILanguageModelChatMetadata;

		configurationService.setUserConfiguration(SkipprChatSettingId.Gpt55ReadFileToolEnabled, false);

		const disabledTools = Array.from(service.getTools(gpt55Model));
		assert.ok(!disabledTools.some(tool => tool.id === SkipprToolId.ReadFile), 'readFile should not be returned from getTools when disabled for gpt-5.5');
		...
		assert.ok(!Array.from(disabledReadToolSet.getTools()).some(tool => tool.id === SkipprToolId.ReadFile), 'readFile should not be included as a read tool-set member when disabled for gpt-5.5');
		...
		configurationService.setUserConfiguration(SkipprChatSettingId.Gpt55ReadFileToolEnabled, true);

		const enabledTools = Array.from(service.getTools(gpt55Model));
		assert.ok(enabledTools.some(tool => tool.id === SkipprToolId.ReadFile), 'readFile should be returned from getTools when enabled for gpt-5.5');
		...
		assert.ok(Array.from(enabledReadToolSet.getTools()).some(tool => tool.id === SkipprToolId.ReadFile), 'readFile should be included as a read tool-set member when enabled for gpt-5.5');
