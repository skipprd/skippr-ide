...
class MockAgentHostService extends mock<IAgentHostService>() {
	...
	public agents = [{ provider: 'skippr' as const, displayName: 'Agent Host - Skippr', description: 'test', requiresAuth: true }];
	...
	override async createSession(config?: IAgentCreateSessionConfig): Promise<URI> {
		...
		const session = config?.session ?? AgentSession.uri('skippr', `sdk-session-${this._nextId++}`);
...
function createContribution(disposables: DisposableStore, opts?: { authServiceOverride?: Partial<IAuthenticationService>; workingDirectoryResolver?: { resolve(sessionResource: URI): URI | undefined; isNewSession?: (sessionResource: URI) => boolean }; languageModels?: ReadonlyMap<string, ILanguageModelChatMetadata> }) {
	...
	const listController = disposables.add(instantiationService.createInstance(AgentHostSessionListController, 'agent-host-skippr', 'skippr', agentHostService, undefined, 'local'));
	const sessionHandler = disposables.add(instantiationService.createInstance(AgentHostSessionHandler, {
		provider: 'skippr' as const,
		agentId: 'agent-host-skippr',
		sessionType: 'agent-host-skippr',
		fullName: 'Agent Host - Skippr',
		description: 'Skippr SDK agent running in a dedicated process',
...
function makeRequest(overrides: Partial<{ message: string; sessionResource: URI; variables: IChatAgentRequest['variables']; userSelectedModelId: string; modelConfiguration: Record<string, unknown>; agentHostSessionConfig: Record<string, string>; agentId: string }> = {}): IChatAgentRequest {
	return upcastPartial<IChatAgentRequest>({
		...
		requestId: 'req-1',
		agentId: overrides.agentId ?? 'agent-host-skippr',
...
async function startTurn(
	...
	const agentId = overrides?.agentId ?? 'agent-host-skippr';
...
suite('AgentHostChatContribution', () => {
	...
	suite('registration', () => {
		...
		test('registers agent', () => {
			...
			assert.ok(chatAgentService.registeredAgents.has('agent-host-skippr'));
		});
	});
	...
	suite('disposal', () => {
		...
		test('fires onWillDispose before session is disposed', async () => {
			...
			const sessionResource = URI.from({ scheme: 'agent-host-skippr', path: '/dispose-test' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
	...
	suite('session list', () => {
		...
		test('refresh populates items from agent host', async () => {
			...
			agentHostService.addSession({ session: AgentSession.uri('skippr', 'aaa'), startTime: 1000, modifiedTime: 2000, summary: 'My session' });
			agentHostService.addSession({ session: AgentSession.uri('skippr', 'bbb'), startTime: 3000, modifiedTime: 4000 });
			...
			assert.strictEqual(listController.items[0].resource.scheme, 'agent-host-skippr');
		...
		test('refresh fires onDidChangeChatSessionItems', async () => {
			...
			agentHostService.addSession({ session: AgentSession.uri('skippr', 'x'), startTime: 1000, modifiedTime: 2000 });
			await listController.refresh(CancellationToken.None);
		...
		test('refresh marks archived sessions as archived items', async () => {
			...
			agentHostService.addSession({
				session: AgentSession.uri('skippr', 'archived'),
				startTime: 1000,
				modifiedTime: 2000,
		...
		test('agent host restart invalidates cache so next refresh re-fetches', async () => {
			...
			agentHostService.addSession({ session: AgentSession.uri('skippr', 'aaa'), startTime: 1000, modifiedTime: 2000, summary: 'Before restart' });
		...
		test('newChatSessionItem creates final-looking resource used for requested backend session', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			assert.ok(item);
			assert.strictEqual(item.resource.scheme, 'agent-host-skippr');
			...
			assert.strictEqual(agentHostService.createSessionCalls[0].session?.toString(), AgentSession.uri('skippr', item.resource.path.substring(1)).toString());
		...
		test('newChatSessionItem rebinds untitled provisional to real resource so chip-selected config survives first send', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const untitledResource = URI.from({ scheme: 'agent-host-skippr', path: '/untitled-abc' });
	...
	suite('session ID resolution', () => {
		...
		test('reuses SDK session for same resource on second message', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const resource = URI.from({ scheme: 'agent-host-skippr', path: '/new-reuse' });
			...
			const registered = chatAgentService.registeredAgents.get('agent-host-skippr')!;
		...
		test('uses sessionId from agent-host scheme resource', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const { turnPromise, session, turnId, fire } = await startTurn(sessionHandler, agentHostService, chatAgentService, disposables, {
				message: 'Hi',
				sessionResource: URI.from({ scheme: 'agent-host-skippr', path: '/existing-session-42' }),
			});
			fire({ type: 'session/turnComplete', session, turnId } as SessionAction);
			await turnPromise;
		...
		test('rejects generic contributed-chat untitled resource', async () => {
			...
			await assert.rejects(
				() => sessionHandler.provideChatSessionContent(URI.from({ scheme: 'agent-host-skippr', path: '/untitled-abc123' }), CancellationToken.None),
				/created by the sessions provider/
			);
			assert.strictEqual(agentHostService.createSessionCalls.length, 0);
			assert.strictEqual(chatAgentService.registeredAgents.has('agent-host-skippr'), true);
		});
		test('passes raw model id extracted from language model identifier', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const { turnPromise, session, turnId, fire } = await startTurn(sessionHandler, agentHostService, chatAgentService, disposables, {
				message: 'Hi',
				userSelectedModelId: 'agent-host-skippr:claude-sonnet-4-20250514',
			});
			fire({ type: 'session/turnComplete', session, turnId } as SessionAction);
			await turnPromise;
		...
		test('does not create backend session eagerly for untitled sessions', async () => {
			...
			const sessionResource = URI.from({ scheme: 'agent-host-skippr', path: '/new-deferred' });
	...
	suite('progress routing', () => {
		...
		test('live turn returns model credit details from usage', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const languageModels = new Map<string, ILanguageModelChatMetadata>([
				['agent-host-skippr:opus-4.7', upcastPartial<ILanguageModelChatMetadata>({ name: 'Opus 4.7', pricing: '15x' })],
			]);
		...
		test('events from other sessions are ignored', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			agentHostService.fireAction({
				action: { type: 'session/delta', session: AgentSession.uri('skippr', 'other-session').toString(), turnId, partId: 'md-other', content: 'wrong' } as SessionAction,
				serverSeq: 100,
				origin: undefined,
			});
		...
		test('input request completion from another client clears local question carousel', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const sessionResource = URI.from({ scheme: 'agent-host-skippr', path: '/new-input-request-test' });
		...
		test('input request completion echo applies authoritative answers after local submit', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const { sessionHandler, agentHostService, chatAgentService, chatWidgetService } = createContribution(disposables);
			const sessionResource = URI.from({ scheme: 'agent-host-skippr', path: '/new-local-input-request-test' });
			chatWidgetService.setWidgetForSession(sessionResource);
		...
		test('input request cancellation does not show draft answers as submitted', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			const { sessionHandler, agentHostService, chatAgentService, chatWidgetService } = createContribution(disposables);
			const sessionResource = URI.from({ scheme: 'agent-host-skippr', path: '/new-cancelled-input-request-test' });
			chatWidgetService.setWidgetForSession(sessionResource);
	...
	suite('history loading', () => {
		...
		test('loads user and assistant messages into history', async () => {
			...
			const sessionUri = AgentSession.uri('skippr', 'sess-1');
			agentHostService.sessionStates.set(sessionUri.toString(), {
				...createSessionState({ resource: sessionUri.toString(), provider: 'skippr', title: 'Test', status: SessionStatus.Idle, createdAt: Date.now(), modifiedAt: Date.now() }),
				lifecycle: SessionLifecycle.Ready,
				turns: [{
					id: 'turn-1',
			...
			const sessionResource = URI.from({ scheme: 'agent-host-skippr', path: '/sess-1' });
			const session = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => session.dispose()));
		...
		test('untitled sessions have empty history', async () => {
			...
			const sessionResource = URI.from({ scheme: 'agent-host-skippr', path: '/new-xyz' });
			const session = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => session.dispose()));
		...
		test('history requests get per-turn modelId from usage, with active turn falling back to session model', async () => {
			const languageModels = new Map<string, ILanguageModelChatMetadata>([
				['agent-host-skippr:opus-4.7', upcastPartial<ILanguageModelChatMetadata>({ name: 'Opus 4.7', pricing: '15x' })],
				['agent-host-skippr:sonnet-4.6', upcastPartial<ILanguageModelChatMetadata>({ name: 'Sonnet 4.6', pricing: '2x' })],
			]);
			const { sessionHandler, agentHostService } = createContribution(disposables, { languageModels });

			const sessionUri = AgentSession.uri('skippr', 'sess-models');
			agentHostService.sessionStates.set(sessionUri.toString(), {
				...createSessionState({
					resource: sessionUri.toString(), provider: 'skippr', title: 'Test',
			...
			const sessionResource = URI.from({ scheme: 'agent-host-skippr', path: '/sess-models' });
			const session = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => session.dispose()));
			...
			assert.deepStrictEqual(
				...
				[
					{ prompt: 'Q1', modelId: 'agent-host-skippr:opus-4.7' },
					{ prompt: 'Q2', modelId: 'agent-host-skippr:sonnet-4.6' },
					{ prompt: 'Q3', modelId: 'agent-host-skippr:sonnet-4.6' },
				],
			);
	...
	suite('history with tool events', () => {
		...
		test('tool_start and tool_complete appear as toolInvocationSerialized in history', async () => {
			const { sessionHandler, agentHostService } = createContribution(disposables);
			const sessionUri = AgentSession.uri('skippr', 'tool-hist');
			...
			const sessionResource = URI.from({ scheme: 'agent-host-skippr', path: '/tool-hist' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => chatSession.dispose()));
		...
		test('orphaned tool_start is marked complete in history', async () => {
			const { sessionHandler, agentHostService } = createContribution(disposables);
			const sessionUri = AgentSession.uri('skippr', 'orphan-tool');
			...
			const sessionResource = URI.from({ scheme: 'agent-host-skippr', path: '/orphan-tool' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => chatSession.dispose()));
		...
		test('non-terminal tool_complete sets pastTenseMessage in history', async () => {
			const { sessionHandler, agentHostService } = createContribution(disposables);
			const sessionUri = AgentSession.uri('skippr', 'generic-tool');
			...
			const sessionResource = URI.from({ scheme: 'agent-host-skippr', path: '/generic-tool' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => chatSession.dispose()));
		...
		test('empty session produces empty history', async () => {
			...
			const sessionUri = AgentSession.uri('skippr', 'empty-sess');
			agentHostService.sessionStates.set(sessionUri.toString(), {
				...createSessionState({ resource: sessionUri.toString(), provider: 'skippr', title: 'Test', status: SessionStatus.Idle, createdAt: Date.now(), modifiedAt: Date.now() }),
				lifecycle: SessionLifecycle.Ready,
				turns: [],
			...
			const sessionResource = URI.from({ scheme: 'agent-host-skippr', path: '/empty-sess' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => chatSession.dispose()));
	...
	suite('session list provider filtering', () => {
		...
		test('filters sessions to only the matching provider', async () => {
			...
			// Add sessions from both providers (use a non-skippr scheme to test filtering)
			agentHostService.addSession({ session: AgentSession.uri('skippr', 'cp-1'), startTime: 1000, modifiedTime: 2000 });
			agentHostService.addSession({ session: URI.from({ scheme: 'other-provider', path: '/cl-1' }), startTime: 1000, modifiedTime: 2000 });
			agentHostService.addSession({ session: AgentSession.uri('skippr', 'cp-2'), startTime: 3000, modifiedTime: 4000 });

			await listController.refresh(CancellationToken.None);

			// The list controller is configured for 'skippr', so only skippr sessions
			assert.strictEqual(listController.items.length, 2);
			assert.ok(listController.items.every(item => item.resource.scheme === 'agent-host-skippr'));
		});
	});
	...
	suite('language model provider', () => {
		...
		test('maps models with correct metadata', async () => {
			const provider = disposables.add(new AgentHostLanguageModelProvider('agent-host-skippr', 'agent-host-skippr'));
			provider.updateModels([
				{ provider: 'skippr', id: 'gpt-4o', name: 'GPT-4o', maxContextWindow: 128000, supportsVision: true, _meta: { multiplierNumeric: 1.5 } },
			]);

			const models = await provider.provideLanguageModelChatInfo({}, CancellationToken.None);

			assert.strictEqual(models.length, 1);
			assert.strictEqual(models[0].identifier, 'agent-host-skippr:gpt-4o');
			...
			assert.strictEqual(models[0].metadata.targetChatSessionType, 'agent-host-skippr');
		});

		test('filters out disabled models', async () => {
			const provider = disposables.add(new AgentHostLanguageModelProvider('agent-host-skippr', 'agent-host-skippr'));
			provider.updateModels([
				{ provider: 'skippr', id: 'gpt-4o', name: 'GPT-4o', maxContextWindow: 128000, supportsVision: false, policyState: PolicyState.Enabled },
				{ provider: 'skippr', id: 'gpt-3.5', name: 'GPT-3.5', maxContextWindow: 16000, supportsVision: false, policyState: PolicyState.Disabled },
			]);
	...
	suite('IAgentConnection config', () => {
		...
		test('handler uses custom extensionId from config', async () => {
			...
			disposables.add(instantiationService.createInstance(AgentHostSessionHandler, {
				provider: 'skippr' as const,
				agentId: 'remote-test-skippr',
				sessionType: 'remote-test-skippr',
				fullName: 'Remote Skippr',
			...
			const registered = chatAgentService.registeredAgents.get('remote-test-skippr');
			assert.ok(registered);
		...
		test('handler forwards request session config via SessionConfigChanged on eager-create path', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const sessionUri = AgentSession.uri('skippr', 'eager-config');
			agentHostService.sessionStates.set(sessionUri.toString(), {
				...createSessionState({ resource: sessionUri.toString(), provider: 'skippr', title: 'Test', status: SessionStatus.Idle, createdAt: Date.now(), modifiedAt: Date.now() }),
				lifecycle: SessionLifecycle.Ready,
				turns: [],
			...
			const sessionResource = URI.from({ scheme: 'agent-host-skippr', path: '/eager-config' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => chatSession.dispose()));
		...
		test('handler does not clobber picker-set session config on eager-create path', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const sessionUri = AgentSession.uri('skippr', 'eager-picker');
			agentHostService.sessionStates.set(sessionUri.toString(), {
				...createSessionState({ resource: sessionUri.toString(), provider: 'skippr', title: 'Test', status: SessionStatus.Idle, createdAt: Date.now(), modifiedAt: Date.now() }),
				lifecycle: SessionLifecycle.Ready,
				turns: [],
			...
			const sessionResource = URI.from({ scheme: 'agent-host-skippr', path: '/eager-picker' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => chatSession.dispose()));
		...
		test('list controller includes description in items', async () => {
			...
			const controller = disposables.add(instantiationService.createInstance(
				AgentHostSessionListController, 'remote-test', 'skippr', agentHostService, 'My Remote Host', 'local'));

			agentHostService.addSession({ session: AgentSession.uri('skippr', 'sess-1'), startTime: 1000, modifiedTime: 2000, summary: 'Test session' });
		...
		test('list controller omits description when undefined', async () => {
			...
			const controller = disposables.add(instantiationService.createInstance(
				AgentHostSessionListController, 'agent-host-skippr', 'skippr', agentHostService, undefined, 'local'));

			agentHostService.addSession({ session: AgentSession.uri('skippr', 'sess-2'), startTime: 1000, modifiedTime: 2000, summary: 'Test' });
			await controller.refresh(CancellationToken.None);
		...
		test('list controller surfaces only working directory in metadata (git state is now per-session state, not summary)', async () => {
			...
			agentHostService.addSession({
				session: AgentSession.uri('skippr', 'sess-git'),
				startTime: 1000,
				modifiedTime: 2000,
				summary: 'With git',
				workingDirectory,
	...
	suite('reconnection to active turn', () => {
		...
		test('loads completed turns as history and active turn request/response', async () => {
			...
			const sessionUri = AgentSession.uri('skippr', 'reconnect-1');
			agentHostService.sessionStates.set(sessionUri.toString(), makeSessionStateWithActiveTurn(sessionUri.toString()));

			const sessionResource = URI.from({ scheme: 'agent-host-skippr', path: '/reconnect-1' });
			const session = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => session.dispose()));
		...
		test('sets isCompleteObs to false and populates progressObs for active turn', async () => {
			...
			const sessionUri = AgentSession.uri('skippr', 'reconnect-2');
			agentHostService.sessionStates.set(sessionUri.toString(), makeSessionStateWithActiveTurn(sessionUri.toString()));

			const sessionResource = URI.from({ scheme: 'agent-host-skippr', path: '/reconnect-2' });
			const session = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => session.dispose()));
		...
		test('provides interruptActiveResponseCallback when reconnecting', async () => {
			...
			const sessionUri = AgentSession.uri('skippr', 'reconnect-3');
			agentHostService.sessionStates.set(sessionUri.toString(), makeSessionStateWithActiveTurn(sessionUri.toString()));

			const sessionResource = URI.from({ scheme: 'agent-host-skippr', path: '/reconnect-3' });
			const session = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => session.dispose()));
		...
		test('interrupt callback dispatches turnCancelled action', async () => {
			...
			const sessionUri = AgentSession.uri('skippr', 'reconnect-cancel');
			agentHostService.sessionStates.set(sessionUri.toString(), makeSessionStateWithActiveTurn(sessionUri.toString()));

			const sessionResource = URI.from({ scheme: 'agent-host-skippr', path: '/reconnect-cancel' });
			const session = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => session.dispose()));
		...
		test('streams new text deltas into progressObs after reconnection', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const sessionUri = AgentSession.uri('skippr', 'reconnect-stream');
			...
			const sessionResource = URI.from({ scheme: 'agent-host-skippr', path: '/reconnect-stream' });
			const session = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => session.dispose()));
		...
		test('marks session complete when turn finishes', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const sessionUri = AgentSession.uri('skippr', 'reconnect-complete');
			agentHostService.sessionStates.set(sessionUri.toString(), makeSessionStateWithActiveTurn(sessionUri.toString()));

			const sessionResource = URI.from({ scheme: 'agent-host-skippr', path: '/reconnect-complete' });
			const session = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => session.dispose()));
		...
		test('handles active turn with running tool call', async () => {
			...
			const sessionUri = AgentSession.uri('skippr', 'reconnect-tool');
			...
			const sessionResource = URI.from({ scheme: 'agent-host-skippr', path: '/reconnect-tool' });
			const session = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => session.dispose()));
		...
		test('handles active turn with pending tool confirmation', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const sessionUri = AgentSession.uri('skippr', 'reconnect-perm');
			const sessionState = makeSessionStateWithActiveTurn(sessionUri.toString());
			sessionState.activeTurn!.responseParts.push({
				kind: ResponsePartKind.ToolCall,
				toolCall: {
			...
			const sessionResource = URI.from({ scheme: 'agent-host-skippr', path: '/reconnect-perm' });
			const session = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => session.dispose()));
		...
		test('no active turn loads completed history only with isComplete true', async () => {
			...
			const sessionUri = AgentSession.uri('skippr', 'no-active-turn');
			agentHostService.sessionStates.set(sessionUri.toString(), {
				...createSessionState({ resource: sessionUri.toString(), provider: 'skippr', title: 'Done', status: SessionStatus.Idle, createdAt: Date.now(), modifiedAt: Date.now() }),
				lifecycle: SessionLifecycle.Ready,
				turns: [{
					id: 'turn-done',
			...
			const sessionResource = URI.from({ scheme: 'agent-host-skippr', path: '/no-active-turn' });
			const session = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => session.dispose()));
		...
		test('includes reasoning in initial progress', async () => {
			...
			const sessionUri = AgentSession.uri('skippr', 'reconnect-reasoning');
			agentHostService.sessionStates.set(sessionUri.toString(), makeSessionStateWithActiveTurn(sessionUri.toString(), {
				streamingText: 'text',
				reasoning: 'Let me think...',
			}));

			const sessionResource = URI.from({ scheme: 'agent-host-skippr', path: '/reconnect-reasoning' });
			const session = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => session.dispose()));
	...
	suite('server-initiated turns', () => {
		...
		test('syncs queued messages added to restored active sessions', async () => {
			...
			const backendSession = AgentSession.uri('skippr', 'restored-pending-sync');
			...
			const sessionResource = URI.from({ scheme: 'agent-host-skippr', path: '/restored-pending-sync' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => chatSession.dispose()));
		...
		test('syncs text updates for existing queued pending messages', async () => {
			...
			const backendSession = AgentSession.uri('skippr', 'pending-text-update');
			agentHostService.sessionStates.set(backendSession.toString(), {
				...createSessionState({
					resource: backendSession.toString(),
					provider: 'skippr',
			...
			const sessionResource = URI.from({ scheme: 'agent-host-skippr', path: '/pending-text-update' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => chatSession.dispose()));
		...
		test('detects server-initiated turn and fires onDidStartServerRequest', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const sessionResource = URI.from({ scheme: 'agent-host-skippr', path: '/new-server-turn' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => chatSession.dispose()));
		...
		test('server-initiated turn streams progress through progressObs', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const sessionResource = URI.from({ scheme: 'agent-host-skippr', path: '/new-server-progress' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => chatSession.dispose()));
		...
		test('disposing chat session does not call disposeSession on connection', async () => {
			...
			const sessionResource = URI.from({ scheme: 'agent-host-skippr', path: '/existing-session-1' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
		...
		test('client-dispatched turns are not treated as server-initiated', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const sessionResource = URI.from({ scheme: 'agent-host-skippr', path: '/new-no-dupe' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => chatSession.dispose()));
		...
		test('server-initiated turn does not duplicate tool calls on repeated state changes', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const sessionResource = URI.from({ scheme: 'agent-host-skippr', path: '/new-server-tool-dedup' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => chatSession.dispose()));
		...
		test('server-initiated turn picks up markdown arriving with turnStarted', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const sessionResource = URI.from({ scheme: 'agent-host-skippr', path: '/new-server-md-initial' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => chatSession.dispose()));
		...
		test('removes consumed queued message from chat model when server-initiated turn appears', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const sessionResource = URI.from({ scheme: 'agent-host-skippr', path: '/new-queue-removal' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => chatSession.dispose()));
		...
		test('removes steering message from chat model when steering id changes', () => runWithFakedTimers({ useFakeTimers: true }, async () => {
			...
			const sessionResource = URI.from({ scheme: 'agent-host-skippr', path: '/new-steering-removal' });
			const chatSession = await sessionHandler.provideChatSessionContent(sessionResource, CancellationToken.None);
			disposables.add(toDisposable(() => chatSession.dispose()));
	...
	suite('provideChatInputCompletions', () => {
		...
		test('forwards text/offset to the agent host and maps file attachments back to chat input items', async () => {
			...
			const sessionResource = URI.from({ scheme: 'agent-host-skippr', path: '/abc' });
		...
		test('returns undefined when the request is cancelled', async () => {
			...
			cts.cancel();
			const result = await sessionHandler.provideChatInputCompletions(
				URI.from({ scheme: 'agent-host-skippr', path: '/abc' }),
				{ text: '', offset: 0 },
				cts.token,
			);
			cts.dispose();
