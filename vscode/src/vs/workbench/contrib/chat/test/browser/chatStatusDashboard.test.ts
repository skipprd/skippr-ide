...
suite('ChatStatusDashboard', () => {
	...
	test('Callout: shows paused when quota exhausted and overage not permitted', () => {
		...
		assert.strictEqual(getCalloutText(dashboard.element), 'Skippr is paused until the limit resets.');
	});
	...
	test('Callout: shows warning when quota >= 75% used and overage not permitted', () => {
		...
		assert.strictEqual(getCalloutText(dashboard.element), 'Skippr will pause when the limit is reached.');
	});
	...
	test('Callout: shows paused for enterprise when quota exhausted', () => {
		...
		assert.strictEqual(getCalloutText(dashboard.element), 'Skippr is paused until the limit resets. Contact your administrator for more information.');
	});
