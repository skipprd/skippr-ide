...
suite('PromptHoverProvider', () => {
	...
	setup(async () => {
		...
		const testModels: ILanguageModelChatMetadata[] = [
			...
			{ id: 'claude-sonnet-4.5', name: 'Claude Sonnet 4.5', vendor: 'skippr', version: '1.0', family: 'claude', modelPickerCategory: undefined, extension: new ExtensionIdentifier('a.b'), isUserSelectable: true, maxInputTokens: 200000, maxOutputTokens: 8192, capabilities: { agentMode: true, toolCalling: true }, isDefaultForLocation: {} } satisfies ILanguageModelChatMetadata,
			{ id: 'claude-opus-4.6', name: 'Claude Opus 4.6', vendor: 'skippr', version: '1.0', family: 'claude', modelPickerCategory: undefined, extension: new ExtensionIdentifier('a.b'), isUserSelectable: true, maxInputTokens: 200000, maxOutputTokens: 8192, capabilities: { agentMode: true, toolCalling: true }, isDefaultForLocation: {} } satisfies ILanguageModelChatMetadata,
			{ id: 'claude-haiku-4.5', name: 'Claude Haiku 4.5', vendor: 'skippr', version: '1.0', family: 'claude', modelPickerCategory: undefined, extension: new ExtensionIdentifier('a.b'), isUserSelectable: true, maxInputTokens: 200000, maxOutputTokens: 8192, capabilities: { agentMode: true, toolCalling: true }, isDefaultForLocation: {} } satisfies ILanguageModelChatMetadata,
		];
	...
	suite('agent hovers', () => {
		test('hover on target attribute shows description', async () => {
			...
			assert.strictEqual(hover, 'The target to which the header attributes like tools apply to. Possible values are `github-skippr` and `vscode`.');
		});

		test('hover on model attribute with github-skippr target shows note', async () => {
			const content = [
				'---',
				'description: "Test"',
			...
			const expected = [
				...
				'',
				'Note: This attribute is not used when target is github-skippr.'
			].join('\n');
		...
		test('hover on handoffs attribute with github-skippr target shows note', async () => {
			const content = [
				'---',
				'description: "Test"',
				'target: github-skippr',
			...
			const expected = [
				...
				'',
				'Note: This attribute is not used in Skippr Data Agent or Claude targets.'
			].join('\n');
			assert.strictEqual(hover, expected);
		});
		...
		test('hover on github-skippr tool shows simple description', async () => {
			const content = [
				'---',
				'description: "Test"',
				'target: github-skippr',
		...
		test('hover on github-skippr tool with target undefined', async () => {
			const content = [
				'---',
				'name: "Test"',
				'description: "Test"',
		...
		test('hover on model attribute with vscode target and model array', async () => {
			const content = [
				...
				'description: "Test"',
				'target: vscode',
				`model: ['MAE 4 (olama)', 'MAE 4.1 (skippr)']`,
				'---',
			].join('\n');
			const hover = await getHover(content, 4, 10, PromptsType.agent);
			const expected = [
