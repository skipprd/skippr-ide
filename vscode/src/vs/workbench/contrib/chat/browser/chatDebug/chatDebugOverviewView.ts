...
export class ChatDebugOverviewView extends Disposable {
	...
	private renderMetricsContent(container: HTMLElement, events: readonly IChatDebugEvent[]): void {
		...
		const totalSkipprUsageNanoAiu = modelTurns.reduce((sum, e) => sum + (e.skipprUsageNanoAiu ?? 0), 0);
		...
		if (totalSkipprUsageNanoAiu > 0) {
			const aic = totalSkipprUsageNanoAiu / NANO_AIU_PER_AIC;
			metrics.push({ label: localize('chatDebug.metric.skipprUsage', "Skippr Usage (AIC)"), value: aicFormatter.value.format(aic) });
		}
