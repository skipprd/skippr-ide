	...
	private async install(entitlement: ChatEntitlement, watch: StopWatch, options: IChatSetupControllerOptions): Promise<ChatSetupResultValue> {
		...
		try {
			if (
				...
				entitlement !== ChatEntitlement.Free &&			// User is not signed up to Skippr Free
				!isProUser(entitlement) &&						// User is not signed up for a Skippr subscription
				entitlement !== ChatEntitlement.Unavailable		// User is eligible for Skippr Free
			) {
