...
export class ChatSetupContribution extends Disposable implements IWorkbenchContribution {
	...
	private registerActions(context: ChatEntitlementContext, requests: ChatEntitlementRequests, controller: Lazy<ChatSetupController>): void {
		...
		class ChatSetupTriggerAction extends Action2 {
			...
			static CHAT_SETUP_ACTION_LABEL = localize2('triggerChatSetup', "Use AI Features with Skippr for free...");
		...
		class ToggleSignInTitleBarAction extends ToggleTitleBarConfigAction {
			constructor() {
				super(
					...
					localize('toggle.chatSignIn', 'Skippr Sign In'),
					localize('toggle.chatSignInDescription', "Toggle visibility of the Skippr Sign In button in title bar"),
					3,
					ContextKeyExpr.and(
						IsWebContext.negate(),
						ChatContextKeys.Entitlement.signedOut,
		...
		class UpgradePlanAction extends Action2 {
			constructor() {
				super({
					...
					title: localize2('managePlan', "Upgrade to Skippr Data Agent Pro"),
		...
		class ManageAdditionalSpendAction extends Action2 {
			constructor() {
				super({
					...
					title: localize2('manageAdditionalSpend', "Manage Skippr Data Agent Budget"),
					category: localize2('chat.category', 'Chat'),
					f1: true,
					precondition: ContextKeyExpr.and(
						ChatContextKeys.Setup.hidden.negate(),
		...
		registerGenerateCodeCommand('chat.internal.explain', 'skippr.data-agent.chat.explain');
		registerGenerateCodeCommand('chat.internal.fix', 'skippr.data-agent.chat.fix');
		registerGenerateCodeCommand('chat.internal.review', 'skippr.data-agent.chat.review');
		registerGenerateCodeCommand('chat.internal.codeReview.run', 'skippr.data-agent.chat.codeReview.run');
