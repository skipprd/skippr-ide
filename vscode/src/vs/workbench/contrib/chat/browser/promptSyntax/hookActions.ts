...
import { getSkipprCliHookTypeName, resolveSkipprCliHookType } from '../../common/promptSyntax/hookSkipprCliCompat.js';
...
/**
 * Detects if existing hooks use Skippr Agent naming convention (camelCase).
 * Returns true if any existing key matches the Skippr Agent format.
 */
function usesSkipprCliNaming(hooksObj: Record<string, unknown>): boolean {
	for (const key of Object.keys(hooksObj)) {
		// Check if any key resolves to a Skippr Agent hook type
		if (resolveSkipprCliHookType(key) !== undefined) {
			return true;
		}
	}
	return false;
 ...
 */
function getHookTypeKeyName(hookTypeId: HookType, useSkipprCliNamingConvention: boolean): string {
	if (useSkipprCliNamingConvention) {
		const skipprCliName = getSkipprCliHookTypeName(hookTypeId);
		if (skipprCliName) {
			return skipprCliName;
		}
	}
...
async function addHookToFile(
	...
	const useSkipprCliNamingConvention = !isClaude && usesSkipprCliNaming(hooksContent.hooks);
	const hookTypeKeyName = isClaude
		? (getClaudeHookTypeName(hookTypeId) ?? hookTypeId)
		: getHookTypeKeyName(hookTypeId, useSkipprCliNamingConvention);
			...
			switch (step) {
				case Step.SelectHookType: {
					...
					if (options?.target) {
						...
						// No target: group into Default (shared), VS Code Only, Skippr Agent Only
						const vscodeTypes = new Set(Object.values(HOOKS_BY_TARGET[Target.VSCode]));
						const skipprTypes = new Set(Object.values(HOOKS_BY_TARGET[Target.GitHubSkippr]));
						const allEntries = Object.entries(HOOK_METADATA) as [HookType, IHookTypeMeta][];

						const shared = allEntries.filter(([h]) => vscodeTypes.has(h) && skipprTypes.has(h));
						const vscodeOnly = allEntries.filter(([h]) => vscodeTypes.has(h) && !skipprTypes.has(h));
						const skipprOnly = allEntries.filter(([h]) => !vscodeTypes.has(h) && skipprTypes.has(h));

						pickerItems = [];
						if (shared.length > 0) {
							pickerItems.push({ type: 'separator', label: localize('hookSection.default', "Local/Skippr Agent Agents") });
						...
						if (skipprOnly.length > 0) {
							pickerItems.push({ type: 'separator', label: localize('hookSection.skipprCliOnly', "Skippr Agent Agents") });
							pickerItems.push(...skipprOnly.map(makeItem));
						}
					}
