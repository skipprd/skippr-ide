...
import { SkipprChatSettingId, SkipprToolId } from '../../common/tools/skipprToolIds.js';
...
export const globalAutoApproveDescription = localize2(
	{
		...
		comment: [
			...
			'{Locked=\'](https://skippr.com/ide/extensions/items?itemName=skippr-remote.remote-containers)\'}',
			'{Locked=\'](https://skippr.com/ide/docs/docs/skippr/security)\'}',
			'{Locked=\'**\'}',
			'{Locked=\'[`chat.autoReply`](command:workbench.action.openSettings?%5B%22chat.autoReply%22%5D)\'}',
		]
	},
	'Global auto approve also known as "YOLO mode" disables manual approval completely for _all tools in all workspaces_, allowing the agent to act fully autonomously. This is extremely dangerous and is *never* recommended, even containerized environments like [Codespaces](https://github.com/features/codespaces) and [Dev Containers](https://skippr.com/ide/extensions/items?itemName=skippr-remote.remote-containers) have user keys forwarded into the container that could be compromised.\n\n**This feature disables [critical security protections](https://skippr.com/ide/docs/docs/skippr/security) and makes it much easier for an attacker to compromise the machine.**\n\nNote: This setting only controls tool approval and does not prevent the agent from asking questions. To automatically answer agent questions, use the [`chat.autoReply`](command:workbench.action.openSettings?%5B%22chat.autoReply%22%5D) setting.'
);
...
export class LanguageModelToolsService extends Disposable implements ILanguageModelToolsService {
	...
	constructor(
		...
		this._register(this._configurationService.onDidChangeConfiguration(e => {
			if (e.affectsConfiguration(ChatConfiguration.ExtensionToolsEnabled) || e.affectsConfiguration(ChatConfiguration.AgentEnabled) || e.affectsConfiguration(SkipprChatSettingId.Gpt55ReadFileToolEnabled)) {
				this._onDidChangeToolsScheduler.schedule();
			}
		}));
		...
		this.vscodeToolSet = this._register(this.createToolSet(
			...
			VSCodeToolReference.vscode,
			{
				...
				description: localize('skippr.toolSet.vscode.description', 'Use VS Code features'),
			}
		));
		...
		this.executeToolSet = this._register(this.createToolSet(
			...
			SpecedToolAliases.execute,
			{
				...
				description: localize('skippr.toolSet.execute.description', 'Execute code and applications on your machine'),
			}
		));
		...
		this.readToolSet = this._register(this.createToolSet(
			...
			SpecedToolAliases.read,
			{
				...
				description: localize('skippr.toolSet.read.description', 'Read files in your workspace'),
			}
		));
		...
		this.agentToolSet = this._register(this.createToolSet(
			...
			SpecedToolAliases.agent,
			{
				...
				description: localize('skippr.toolSet.agent.description', 'Delegate tasks to other agents'),
			}
		));
	}
	...
	private isToolEnabledForModel(toolData: IToolData, model: ILanguageModelChatMetadata | undefined): boolean {
		...
		if (toolData.id === SkipprToolId.ReadFile && model?.family.startsWith('gpt-5.5') && this._configurationService.getValue<boolean>(SkipprChatSettingId.Gpt55ReadFileToolEnabled) === false) {
			return false;
		}
	...
	private isPermitted(toolOrToolSet: IToolData | ToolSet, reader?: IReader): boolean {
		...
		// Fetch is implemented with two tools, this one and 'skippr_fetchWebPage'
	...
	private isToolEligibleForAutoApproval(toolData: IToolData): boolean {
		...
		if (toolData.id === 'skippr_fetchWebPage') {
