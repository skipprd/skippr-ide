...
import { AgentPluginService, ConfiguredAgentPluginDiscovery, SkipprCliAgentPluginDiscovery, ExtensionAgentPluginDiscovery, MarketplaceAgentPluginDiscovery } from '../common/plugins/agentPluginServiceImpl.js';
...
configurationRegistry.registerConfiguration({
	...
	properties: {
		...
		[ChatConfiguration.SessionSyncEnabled]: {
			...
			tags: ['experimental', 'advanced'],
			policy: {
				name: 'SkipprSessionSync',
				category: PolicyCategory.InteractiveSession,
				minimumVersion: '1.119',
				value: (policyData) => policyData.session_search === SkipprSessionSearchPolicy.Disabled ? false : undefined,
				localization: {
					description: {
		...
		[ChatConfiguration.AutoApprovedUrls]: {
			default: {
				'https://skippr.com/ide/docs': true,
		...
		[ChatConfiguration.PluginMarketplaces]: {
			...
			default: ['github/skippr-plugins', 'github/awesome-skippr'],
			scope: ConfigurationScope.APPLICATION,
			tags: ['experimental'],
		},
		...
		[PromptsConfig.HOOKS_LOCATION_KEY]: {
			...
			markdownDescription: nls.localize(
				...
				"Specify paths to hook configuration files that define custom shell commands to execute at strategic points in an agent's workflow. [Learn More]({0}).\n\nRelative paths are resolved from the root folder(s) of your workspace. Supports Skippr hooks (`*.json`) and Claude Code hooks (`settings.json`, `settings.local.json`).",
		...
		[PromptsConfig.USE_CLAUDE_HOOKS]: {
			...
			markdownDescription: nls.localize('chat.useClaudeHooks.description', "Controls whether hooks from Claude configuration files can execute. When disabled, only Skippr-format hooks are used. Hooks are loaded from the files configured in `#chat.hookFilesLocations#`.",),
			default: false,
			restricted: true,
			disallowConfigurationDefault: true,
			tags: ['preview', 'prompts', 'hooks', 'agent']
		...
		[ChatConfiguration.AIDisabled]: {
			type: 'boolean',
			description: nls.localize('chat.disableAIFeatures', "Disable and hide built-in AI features provided by Skippr Data Agent, including chat and inline suggestions."),
			default: false,
			scope: ConfigurationScope.WINDOW,
		},
		[ChatConfiguration.TitleBarSignInEnabled]: {
			type: 'boolean',
			description: nls.localize('chat.titleBar.signIn.enabled', "Controls whether the Skippr Sign In button is shown in the title bar when signed out. When disabled, the Sign In affordance falls back to the status bar."),
			default: true,
		},
		...
		[ChatConfiguration.GrowthNotificationEnabled]: {
			type: 'boolean',
			description: nls.localize('chat.growthNotification', "Controls whether to show a growth notification in the agent sessions view to encourage new users to try Skippr."),
			default: false,
			tags: ['experimental'],
			experiment: {
				mode: 'auto'
		...
		'chat.extensionUnification.enabled': {
			type: 'boolean',
			description: nls.localize('chat.extensionUnification.enabled', "Enables the unification of Skippr Data Agent extensions. When enabled, all Skippr Data Agent functionality is served from the Skippr Data Agent Chat extension. When disabled, the Skippr Data Agent and Skippr Data Agent Chat extensions operate independently."),
			default: true,
			tags: ['experimental'],
			experiment: {
				mode: 'auto'
...
class SkipprTelemetryContribution extends Disposable implements IWorkbenchContribution {

	static readonly ID = 'workbench.contrib.skipprTelemetry';
	...
	private updateSkipprTrackingId(): void {
		const chatTrackingId = this.chatEntitlementService.chatTrackingId;
		if (chatTrackingId) {
			// __GDPR__COMMON__ "common.chatTrackingId" : { "endPoint": "GoogleAnalyticsID", "classification": "EndUserPseudonymizedInformation", "purpose": "BusinessInsight", "comment": "The anonymized Skippr analytics tracking ID from the entitlement API." }
			this.telemetryService.setCommonProperty('common.chatTrackingId', chatTrackingId);
		}
	}
}
...
class ChatAgentSettingContribution extends Disposable implements IWorkbenchContribution {
	...
	private registerNewChatButtonIcon(): void {
		this.experimentService.getTreatment<string>('chatNewButtonIcon').then((value) => {
			const supportedValues = ['skippr', 'new-session', 'comment'];
...
registerWorkbenchContribution2(SkipprTelemetryContribution.ID, SkipprTelemetryContribution, WorkbenchPhase.BlockRestore);
...
agentPluginDiscoveryRegistry.register(new SyncDescriptor(SkipprCliAgentPluginDiscovery));
