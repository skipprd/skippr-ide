...
export class ChatStatusBarEntry extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		this._register(CommandsRegistry.registerCommand('workbench.action.chat.openSkipprStatus', () => {
	...
	private getEntryProps(): IStatusbarEntry {
		let text = '$(skippr)';
		let ariaLabel = localize('chatStatusAria', "Skippr status");
		...
		if (isNewUser(this.chatEntitlementService)) {
			...
			if (this.chatEntitlementService.sentiment.disabled || this.chatEntitlementService.sentiment.untrusted) {
				text = '$(skippr-unavailable)';
				ariaLabel = localize('skipprDisabledStatus', "Skippr disabled");
			}

			// Sessions in progress
			else if (this.runningSessionsCount > 0) {
				text = '$(skippr-in-progress)';
			...
			// Free Quota Exceeded
				...
				text = `$(skippr-warning) ${quotaWarning}`;
		...
		const baseResult = {
			name: localize('chatStatus', "Skippr Status"),
			text,
			ariaLabel,
	...
	private getSetupEntryProps(): IStatusbarEntry {
		...
		return {
			name: localize('chatStatus', "Skippr Status"),
			text: showSignInLabel ? `$(skippr) ${signInLabel}` : '$(skippr)',
			ariaLabel: showSignInLabel ? signInLabel : localize('chatStatusAria', "Skippr status"),
