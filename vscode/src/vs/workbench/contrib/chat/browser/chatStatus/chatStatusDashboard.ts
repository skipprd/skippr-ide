...
export class ChatStatusDashboard extends DomWidget {
	...
	private render(): void {
		...
		if (hasUsageSection && !this.options?.compactQuotaLayout) {
			...
			const header = this.renderHeader(headerHost, this._store, planName, toAction({
				id: 'workbench.action.manageSkippr',
				label: localize('quotaLabel', "Manage Skippr Settings"),
				tooltip: localize('quotaTooltip', "Manage Skippr Settings"),
	...
	private renderSetupSection(): void {
		...
		if (newUser && anonymousUser) {
			descriptionText = new MarkdownString(localize({ key: 'activeDescriptionAnonymous', comment: ['{Locked="]({2})"}', '{Locked="]({3})"}'] }, "By continuing with {0} Skippr, you agree to {1}'s [Terms]({2}) and [Privacy Statement]({3})", defaultChat.provider.default.name, defaultChat.provider.default.name, defaultChat.termsStatementUrl, defaultChat.privacyStatementUrl), { isTrusted: true });
			descriptionClass = `${descriptionClass}.terms`;
		} else if (newUser) {
			descriptionText = localize('activateDescription', "Set up Skippr to use AI features.");
		} else if (anonymousUser) {
			descriptionText = localize('enableMoreDescription', "Sign in to enable more Skippr AI features.");
		} else if (disabled) {
			descriptionText = localize('enableDescription', "Enable Skippr to use AI features.");
		} else {
			descriptionText = localize('signInDescription', "Sign in to use Skippr AI features.");
		}
		...
		if (newUser) {
			...
			buttonLabel = localize('enableSkipprButton', "Enable AI Features");
		} else {
		...
		const update = () => {
			...
			if (maxUsedPercentage >= 100 && additionalUsageEnabled) {
				...
				calloutText.textContent = isEnterpriseUser
					? localize('quotaPausedEnterprise', "Skippr is paused until the limit resets. Contact your administrator for more information.")
					: localize('quotaPaused', "Skippr is paused until the limit resets.");
			} else if (maxUsedPercentage >= 75 && !additionalUsageEnabled) {
				quotaCallout.style.display = '';
				quotaCallout.className = 'quota-callout info';
				calloutIcon.className = `callout-icon ${ThemeIcon.asClassName(Codicon.info)}`;
				calloutText.textContent = isEnterpriseUser
					? localize('quotaWarningEnterprise', "Skippr will pause when the limit is reached. Contact your administrator for more information.")
					: localize('quotaWarning', "Skippr will pause when the limit is reached.");
			} else {
				quotaCallout.style.display = 'none';
			}
