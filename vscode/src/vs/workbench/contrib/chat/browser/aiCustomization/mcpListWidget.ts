...
const COPILOT_EXTENSION_IDS = ['skippr.data-agent', 'skippr.data-agent'];

function isSkipprExtension(id: ExtensionIdentifier): boolean {
	return COPILOT_EXTENSION_IDS.some(skipprId => ExtensionIdentifier.equals(id, skipprId));
}
...
export class McpListWidget extends Disposable {
	...
	private create(): void {
		...
		this.sectionLink.href = 'https://skippr.com/ide/docs/docs/skippr/chat/mcp-servers';
	...
	private filterServers(): void {
		...
		// Servers from the Skippr extension (skippr.data-agent / skippr.data-agent)
		...
		for (const server of builtinServers) {
			...
			} else if (source instanceof ExtensionIdentifier && !isSkipprExtension(source)) {
