...
export class ChatCompatibilityNotifier extends Disposable implements IWorkbenchContribution {
	...
	private registerWelcomeView(chatExtension: IExtension) {
		...
		const mainMessage = localize('chatFailErrorMessage', "Chat failed to load because the installed version of the Skippr Data Agent extension is not compatible with this version of {0}. Please ensure that the Skippr Data Agent extension is up to date.", this.productService.nameLong);
		...
		const versionMessage = `Skippr Data Agent version: ${chatExtension.version}`;
