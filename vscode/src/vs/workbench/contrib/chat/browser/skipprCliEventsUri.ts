...
// Scheme conventions for `skipprcli` chat sessions:
// - Local AH:  `agent-host-skipprcli:/<id>`         (LOCAL_RESOURCE_SCHEME_PREFIX + provider)
// - Remote AH: `remote-<auth>-skipprcli:/<id>`      (remoteAgentHostSessionTypeId)
// - EH CLI ext: `skipprcli:/<id>`                   (extension's own session type)
const COPILOT_CLI_PROVIDER = 'skipprcli';
...
/**
 * Builds the local `events.jsonl` URI under `~/.skippr/session-state/<rawId>/`.
 *
 * Used for both the local Agent Host Skippr Agent provider and the
 * extension-host Skippr Agent provider, which share the same on-disk layout
 * and the same chat session URI shape (`skipprcli:/<rawId>`).
 */
export function buildLocalEventsUri(userHome: URI, rawSessionId: string): URI {
	return joinPath(userHome, '.skippr', 'session-state', rawSessionId, 'events.jsonl');
}
...
export function buildRemoteEventsUri(connection: IRemoteAgentHostConnectionInfo, rawSessionId: string): URI | undefined {
	...
	const remoteFileUri = URI.from({
		scheme: 'file',
		path: `${trimmed}/.skippr/session-state/${rawSessionId}/events.jsonl`,
	});
 ...
 * of the form `remote-<authority>-skipprcli`. Returns `undefined` for
 * any other scheme, including the local `skipprcli` scheme.
 */
 ...
 * `events.jsonl` URI for the corresponding Skippr Agent session, or
