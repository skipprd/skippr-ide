		...
		const promptResult = await this.dialogService.prompt({
			...
			custom: {
				...
				markdownDetails: [{
					...
					markdown: new MarkdownString(`[${localize('autoApprove.markdown2', 'Learn more about the potential risks and how to avoid them.')}](https://skippr.com/ide/docs/docs/skippr/security#_security-considerations)`)
				}],
			}
		});
