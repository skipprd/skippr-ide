...
export class ChatInputPart extends Disposable implements IHistoryNavigationWidget {
	...
	public switchModelByQualifiedName(qualifiedModelNames: readonly string[]): boolean {
		...
		this.logService.warn(`[chat] Node of the models "${qualifiedModelNames.join(', ')}" not found. Use format "<name> (<vendor>)", e.g. "GPT-4o (skippr)".`);
		return false;
	}
	...
	*/
	...
	private preselectModelFromSessionHistory(): void {
		...
		const tryMatch = () => {
			...
			// Try exact identifier match first (e.g. "skippr/gpt-4o")
