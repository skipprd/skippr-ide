 ...
 * The skippr extension always sets multiplierNumeric alongside multiplier pricing strings.
 */
	...
	if (useGroupedModelPicker) {
		...
		if (models.length) {
			...
			if (otherModels.length > 0) {
				...
				// Sort buckets: skippr vendor first, then alphabetically by group name
				const sortedBuckets = [...providerGroups.values()].sort((a, b) => {
					if (a.vendor === 'skippr' && b.vendor !== 'skippr') { return -1; }
					if (b.vendor === 'skippr' && a.vendor !== 'skippr') { return 1; }
...
function createUnavailableModelItem(
	...
	if (reason === 'upgrade') {
		...
		if (chatEntitlementService.entitlement === ChatEntitlement.Pro) {
			hoverContent.appendMarkdown(localize('chat.modelPicker.upgradeHoverProPlus', "[Upgrade to Skippr Data Agent Pro+](command:workbench.action.chat.upgradePlan \" \") to use the best models."));
		} else {
			hoverContent.appendMarkdown(localize('chat.modelPicker.upgradeHover', "[Upgrade to Skippr Data Agent Pro](command:workbench.action.chat.upgradePlan \" \") to use the best models."));
		}
	} else if (reason === 'update') {
	...
	show(anchor?: HTMLElement): void {
		...
		const onSelect = (model: ILanguageModelChatMetadataAndIdentifier) => {
			this._telemetryService.publicLog2<ChatModelChangeEvent, ChatModelChangeClassification>('chat.modelChange', {
				fromModel: previousModel?.metadata.vendor === 'skippr' ? new TelemetryTrustedValue(previousModel.identifier) : 'unknown',
				toModel: model.metadata.vendor === 'skippr' ? new TelemetryTrustedValue(model.identifier) : 'unknown'
			});
			this._selectedModel = model;
			this._renderLabel();
...
function isAutoModel(model: ILanguageModelChatMetadataAndIdentifier): boolean {
	return model.metadata.id === 'auto' && (model.metadata.vendor === 'skippr' || model.metadata.vendor === 'skipprcli');
}
