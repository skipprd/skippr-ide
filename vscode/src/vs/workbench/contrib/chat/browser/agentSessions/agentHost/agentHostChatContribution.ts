...
export class AgentHostContribution extends Disposable implements IWorkbenchContribution {
	...
	private _registerAgent(agent: AgentInfo): void {
		...
		// Skippr IDE: agent-host sessions run in a dedicated process; hide the
		// delegation picker and the empty-session "session target" chip (same as
		// the Agents app / remote agent host UX).
		...
		// Distinguish from the extension-host Skippr Agent harness, which
		// registers under the same `Skippr Agent` displayName via the chat
		...
		// `getAgentSessionProviderName(AgentHostSkippr)`.
