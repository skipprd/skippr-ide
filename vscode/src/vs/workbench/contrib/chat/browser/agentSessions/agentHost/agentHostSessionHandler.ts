...
function getSkipprCredits(usage: UsageInfo | undefined): number | undefined {
	const skipprUsage = usage?._meta?.skipprUsage;
	if (skipprUsage && typeof skipprUsage === 'object' && hasKey(skipprUsage, { totalNanoAiu: true })) {
		const totalNanoAiu = Object.getOwnPropertyDescriptor(skipprUsage, 'totalNanoAiu')?.value;
...
export class AgentHostSessionHandler extends Disposable implements IChatSessionContentProvider {
	 ...
	 * E.g. "agent-host-skippr:claude-sonnet-4-20250514" → "claude-sonnet-4-20250514".
	 */
	...
	private _createTurnModelLookup(sessionResource: URI, fallbackRawModelId: string | undefined): TurnModelLookup {
		...
		return {
			...
			toResponseDetails: (rawModelId, usage) => {
				...
				const credits = getSkipprCredits(usage);
