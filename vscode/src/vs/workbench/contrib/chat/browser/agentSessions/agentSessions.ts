...
export enum AgentSessionProviders {
	...
	Background = SessionType.SkipprCLI,
	Cloud = SessionType.SkipprCloud,
	Claude = SessionType.ClaudeCode,
	Codex = SessionType.Codex,
	Growth = SessionType.Growth,
	AgentHostSkippr = SessionType.AgentHostSkippr,
}
 ...
 * agent hosts like `remote-{authority}-skippr`).
...
export function getAgentSessionProvider(sessionResource: URI | string): AgentSessionProviders | undefined {
	...
	switch (type) {
		...
		case AgentSessionProviders.AgentHostSkippr:
			return type;
		default:
			return undefined;
	}
...
export function getAgentSessionProviderName(provider: AgentSessionTarget): string {
	switch (provider) {
		...
		case AgentSessionProviders.Background:
			return localize('chat.session.providerLabel.background', "Skippr Agent");
		case AgentSessionProviders.Cloud:
...
export function getAgentSessionProviderDescription(provider: AgentSessionTarget): string {
	switch (provider) {
		...
		case AgentSessionProviders.Background:
			return localize('chat.session.providerDescription.background', "Delegate tasks to a background agent running locally on your machine. The agent iterates via chat and works asynchronously in a Git worktree to implement changes isolated from your main workspace using the Skippr Data Agent CLI.");
		case AgentSessionProviders.Cloud:
			return localize('chat.session.providerDescription.cloud', "Delegate tasks to the Skippr Data Agent coding agent. The agent iterates via chat and works asynchronously in the cloud to implement changes and pull requests as needed.");
		case AgentSessionProviders.Claude:
			return localize('chat.session.providerDescription.claude', "Delegate tasks to the Claude Agent SDK using the Claude models included in your Skippr Data Agent subscription. The agent iterates via chat and works interactively to implement changes on your main workspace.");
		case AgentSessionProviders.Codex:
		...
		case AgentSessionProviders.Growth:
			return localize('chat.session.providerDescription.growth', "Learn about Skippr features.");
		case AgentSessionProviders.AgentHostSkippr:
			return 'Run a Skippr SDK agent in a dedicated process.';
		default:
			return '';
	}
}
