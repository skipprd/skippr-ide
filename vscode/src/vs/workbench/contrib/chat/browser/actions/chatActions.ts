...
import { SkipprUsageExtensionFeatureId } from '../../common/languageModelStats.js';
...
export function registerChatActions() {
		...
		constructor() {
			super({
				...
				menu: [{
					...
					group: 'navigation',
					when: ContextKeyExpr.and(ActiveEditorContext.isEqualTo(ChatEditorInput.EditorID), ChatContextKeys.newChatButtonExperimentIcon.notEqualsTo('skippr'), ChatContextKeys.newChatButtonExperimentIcon.notEqualsTo('new-session'), ChatContextKeys.newChatButtonExperimentIcon.notEqualsTo('comment')),
					order: 1
				}],
			});
		}
	...
	registerAction2(class NewChatEditorSkipprIconAction extends Action2 {
		constructor() {
			super({
				id: ACTION_ID_OPEN_CHAT + '.skipprIcon',
				title: localize2('interactiveSession.open', "New Chat Editor"),
				icon: Codicon.skippr,
				f1: false,
				category: CHAT_CATEGORY,
				...
				menu: [{
					id: MenuId.EditorTitle,
					group: 'navigation',
					when: ContextKeyExpr.and(ActiveEditorContext.isEqualTo(ChatEditorInput.EditorID), ChatContextKeys.newChatButtonExperimentIcon.isEqualTo('skippr')),
					order: 1
				}],
			});
		}
	...
	const nonEnterpriseSkipprUsers = ContextKeyExpr.and(ChatContextKeys.enabled, ContextKeyExpr.notEquals(`config.${defaultChat.completionsAdvancedSetting}.authProvider`, defaultChat.provider.enterprise.id));
	registerAction2(class extends Action2 {
		constructor() {
			super({
				id: 'workbench.action.chat.manageSettings',
				title: localize2('manageChat', "Manage Skippr Settings"),
				category: CHAT_CATEGORY,
				f1: true,
				precondition: ContextKeyExpr.and(
					ContextKeyExpr.or(
					...
					nonEnterpriseSkipprUsers
				),
				menu: {
					id: MenuId.ChatTitleBarMenu,
					group: 'y_manage',
					order: 1,
					when: nonEnterpriseSkipprUsers
				}
			});
		}
	...
	registerAction2(class ShowExtensionsUsingSkippr extends Action2 {

		constructor() {
			super({
				id: 'workbench.action.chat.showExtensionsUsingSkippr',
				title: localize2('showSkipprUsageExtensions', "Show Extensions using Skippr"),
				f1: true,
		...
		override async run(accessor: ServicesAccessor): Promise<void> {
			...
			extensionsWorkbenchService.openSearch(`@contribute:${SkipprUsageExtensionFeatureId}`);
		}
	});

	registerAction2(class ConfigureSkipprCompletions extends Action2 {
	...
	registerAction2(class ShowQuotaExceededDialogAction extends Action2 {
		...
		constructor() {
			super({
				...
				title: localize('upgradeChat', "Upgrade Skippr Data Agent Plan")
			});
		}
		...
		override async run(accessor: ServicesAccessor) {
			...
			const upgradeToPro = free ? localize('upgradeToPro', "Upgrade to Skippr Data Agent Pro for:\n- Unlimited inline suggestions\n- Unlimited chat messages\n- Access to premium models") : undefined;

			await dialogService.prompt({
				type: 'none',
				message: localize('skipprQuotaReached', "Skippr Data Agent Quota Reached"),
				cancelButton: {
				...
				buttons: [
					{
						label: free ? localize('upgradePro', "Upgrade to Skippr Data Agent Pro") : localize('upgradePlan', "Upgrade Skippr Data Agent Plan"),
						run: () => {
				...
				custom: {
					icon: Codicon.skipprWarningLarge,
