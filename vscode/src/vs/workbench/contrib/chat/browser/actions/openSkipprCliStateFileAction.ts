...
import { resolveEventsUri } from '../skipprCliEventsUri.js';

/**
 * Shared implementation of "Open Skippr Agent State File". Resolves the
 ...
 * this helper after resolving the active Skippr Agent session resource.
 */
export async function openSkipprCliStateFile(
	...
	switch (result.kind) {
		case 'ok':
			...
			return;
		case 'no-session':
			notificationService.info(localize('openSessionEventsFile.noSession', "No Skippr Agent session is active."));
			return;
		case 'unsupported-scheme':
			notificationService.info(localize('openSessionEventsFile.unsupported', "The active chat session is not a Skippr Agent session."));
			return;
 ...
 * find the active Skippr Agent chat session. Suitable for vscode where the
 * agents-window-specific `ISessionsManagementService` is not present.
 */
export class OpenSkipprCliStateFileAction extends Action2 {

	static readonly ID = 'workbench.action.chat.openSkipprCliStateFile';

	constructor() {
		super({
			id: OpenSkipprCliStateFileAction.ID,
			title: localize2('openSessionEventsFile', "Open Skippr Agent State File"),
			f1: true,
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		await openSkipprCliStateFile(accessor, sessionResource);
	}
}
