...
class ChatCommandLineHandler extends Disposable {
	...
	private async prompt(args: typeof this.environmentService.args.chat): Promise<void> {
		...
		const trusted = await this.workspaceTrustRequestService.requestWorkspaceTrust({
			message: localize('skipprWorkspaceTrust', "AI features are currently only supported in trusted workspaces.")
		});
...
function getSkipprAgentInfo(rootState: RootState | Error | undefined): AgentInfo | undefined {
 ...
 * `agent-host-${agent.provider}` (e.g. `agent-host-skipprcli`) only after it
 * surfaces an `AgentInfo` via `rootState`. This is asynchronous, so the static
 * `agent-host-skippr` umbrella commands need to wait for that registration
...
async function resolveAgentHostSessionType(agentHostService: IAgentHostService): Promise<string> {
	const agent = getSkipprAgentInfo(agentHostService.rootState.value);
	if (agent) {
	...
	const waitForAgent = new Promise<AgentInfo | undefined>(res => {
		const sub = agentHostService.rootState.onDidChange(state => {
			const found = getSkipprAgentInfo(state);
			if (found) {
				sub.dispose();
				res(found);
			}
...
CommandsRegistry.registerCommand(
	`workbench.action.chat.openNewChatSessionInPlace.${AgentSessionProviders.AgentHostSkippr}`,
...
// The dynamic per-agent commands (e.g. `agent-host-skippr`) are only
