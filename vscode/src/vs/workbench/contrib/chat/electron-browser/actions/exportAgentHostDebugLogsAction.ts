...
import { COPILOT_CLI_LOCAL_AH_SCHEME, parseRemoteAuthorityFromScheme, resolveEventsUri } from '../../browser/skipprCliEventsUri.js';
 ...
 * Skippr Agent session events file (if available), the window/shared/agent-host
 ...
 * or `undefined` if the scheme does not belong to a Skippr Agent agent-host
 * session (i.e. local AH or remote AH; the EH CLI extension's own
 * `skipprcli:` sessions are excluded).
 */
