...
export class RunSubagentTool extends Disposable implements IToolImpl {
	...
	getToolData(): IToolData {
		...
		properties.model = {
			type: 'string',
			description: 'Optional model for the subagent. Format: "Model Name (Vendor)", vendor is usually "skippr". Only use to enforce a specific model.',
		};
	...
	async invoke(invocation: IToolInvocation, _countTokens: CountTokensCallback, _progress: ToolProgress, token: CancellationToken): Promise<IToolResult> {
		...
		try {
			...
			modeTools['skippr_askQuestions'] = false;
