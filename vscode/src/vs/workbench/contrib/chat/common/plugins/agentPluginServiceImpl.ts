...
// ---------------------------------------------------------------------------
// Skippr Agent plugin discovery
// ---------------------------------------------------------------------------

/**
 * Directory under the Skippr Agent home where installed plugins are cached.
 * Layout is two levels deep: `<marketplace>/<plugin>/`. Direct (non-marketplace)
 * installs use the reserved marketplace segment `_direct`.
 *
 * See `src/plugins/manager.ts` in the skippr-agent-runtime repo.
 */
const COPILOT_CLI_INSTALLED_PLUGINS_DIR = '.skippr/installed-plugins';

/**
 * Discovers plugins installed by the Skippr Agent under
 * `~/.skippr/installed-plugins/<marketplace>/<plugin>/`. Each leaf directory
 ...
 */
export class SkipprCliAgentPluginDiscovery extends AbstractAgentPluginDiscovery {
	...
	public override start(enablementModel: IEnablementModel): void {
		...
		const setupWatchers = async () => {
			...
			// exist, so we need to watch an existing parent (e.g. ~/.skippr or
	...
	protected override async _discoverPluginSources(): Promise<readonly IPluginSource[]> {
		...
		try {
			...
			// Directory doesn't exist — Skippr Agent hasn't installed any plugins.
			return [];
		}
	...
	private async _promptRemove(resource: URI): Promise<void> {
		const { confirmed } = await this._dialogService.confirm({
			message: localize('skipprCliPlugin.remove.confirm', "This plugin was installed by the Skippr Agent. Remove it from disk?"),
			detail: localize('skipprCliPlugin.remove.detail', "The plugin directory '{0}' will be moved to the trash. You can reinstall it later via the Skippr Agent.", resource.fsPath),
			primaryButton: localize('skipprCliPlugin.remove.primary', "Remove"),
		});
		if (!confirmed) {
			return;
		}
		...
		try {
			...
			this._logService.error('[SkipprCliAgentPluginDiscovery] Failed to remove plugin', error);
		}
	}
}
