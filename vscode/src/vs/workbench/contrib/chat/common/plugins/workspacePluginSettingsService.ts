...
/** Skippr Agent settings folder inside `.github/`. */
const COPILOT_CONFIG_FOLDER = '.github/skippr';
...
export interface IWorkspacePluginSettingsService {
	 ...
	 * settings files (`.claude/settings.json`, `.github/skippr/settings.json`).
	 */
 ...
 * folder (e.g. `.claude/` or `.github/skippr/`) across all workspace
...
export class WorkspacePluginSettingsService extends Disposable implements IWorkspacePluginSettingsService {
	...
	constructor(
		...
		const skipprReader = this._register(new WorkspaceSettingsReader(
			COPILOT_CONFIG_FOLDER, '[SkipprPluginSettings]',
			fileService, workspaceContextService, logService,
		));
		...
		this.extraMarketplaces = derived(reader => {
			...
			const skippr = skipprReader.data.read(reader).marketplaces;
			const byCanonicalId = new Map<string, IWorkspaceMarketplaceEntry>();
			for (const entry of [...claude, ...skippr]) {
		...
		this.enabledPlugins = derived(reader => {
			...
			const skippr = skipprReader.data.read(reader).enabledPlugins;
			const merged = new Map<string, boolean>();
			for (const [key, value] of claude) {
				merged.set(key, value);
			}
			for (const [key, value] of skippr) {
				if (!merged.has(key)) {
					merged.set(key, value);
				}
			}
