...
| `agentPluginServiceImpl.ts` | `AgentPluginService` implementation, `AbstractAgentPluginDiscovery` base class, format adapters (Skippr / Claude / Open Plugin) |
...
1. **Format detection** — auto-detects whether a plugin uses the Skippr, Claude, or Open Plugin format based on path conventions and manifest existence.
...
| Hook parser | `parseSkipprHooks()` | `parseClaudeHooks()` | `parseClaudeHooks()` |
...
Auto-detection logic: if a `.plugin/plugin.json` manifest exists, the Open Plugin adapter is used; if the plugin URI path contains `.claude` or a `.claude-plugin/plugin.json` manifest exists, the Claude adapter is used; otherwise, the Skippr adapter is used.

### Plugin Contents (Filesystem Layout)

```
<plugin-root>/
├── plugin.json                                    # Skippr manifest
...
3. `.github/plugin/marketplace.json` → `MarketplaceType.Skippr`
