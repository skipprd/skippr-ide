...
/**
 * JSON Schema for Skippr Data Agent hook configuration files.
...
/**
 * Hook command schema for the Skippr Agent format.
 * Adds `bash`, `powershell`, and `timeoutSec` fields alongside the standard ones.
 */
const skipprCliHookCommandSchema: IJSONSchema = {
	type: 'object',
	additionalProperties: true,
	required: ['type'],
	anyOf: [
...
const skipprCliHookArraySchema: IJSONSchema = {
	type: 'array',
	items: skipprCliHookCommandSchema
};

/**
 * Hook properties for the Skippr Agent format.
 */
const skipprCliHookProperties: Record<string, IJSONSchema> = buildHookProperties(Target.GitHubSkippr, skipprCliHookArraySchema);

export const hookFileSchema: IJSONSchema = {
	$schema: 'http://json-schema.org/draft-07/schema#',
	type: 'object',
	description: nls.localize('hookFile.description', 'Skippr Data Agent hook configuration file. Hooks enable executing custom shell commands at strategic points in an agent\'s workflow.'),
	additionalProperties: true,
	required: ['hooks'],
	properties: {
		hooks: {
	...
	// whether the file uses the Skippr Agent format (detected by the "version" field).
	if: {
		required: ['version'],
		properties: {
			version: { type: 'number' }
	...
	then: {
		// Skippr Agent format: camelCase hook names, bash/powershell/timeoutSec fields
		properties: {
			version: {
				type: 'number',
			...
			hooks: {
				properties: skipprCliHookProperties
			}
		}
	},
	else: {
 ...
 * Only matches exact enum values. For tool-specific naming conventions (e.g., Claude, Skippr Agent),
 ...
 *
 * This allows Skippr format to handle Claude-style entries if pasted.
 ...
 * This ensures compatibility when Claude-style hooks are pasted into Skippr format.
 */
