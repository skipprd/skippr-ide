...
import { resolveSkipprCliHookType } from './hookSkipprCliCompat.js';
...
export enum HookSourceFormat {
	/** Skippr Data Agent hooks .json format */
	Skippr = 'skippr',
...
export function getHookSourceFormat(fileUri: URI): HookSourceFormat {
	...
	// Default to Skippr format
	return HookSourceFormat.Skippr;
}
...
/**
 * Parses hooks from a Skippr hooks .json file (our native format).
 */
export function parseSkipprHooks(
	json: unknown,
	...
	for (const originalId of Object.keys(hooksObj)) {
		const hookType = resolveSkipprCliHookType(originalId) ?? toHookType(originalId);
		if (!hookType) {
			continue;
		}
...
export function parseHooksFromFile(
	...
	switch (format) {
		...
		case HookSourceFormat.Skippr:
		default:
			hooks = parseSkipprHooks(json, workspaceRootUri, userHome);
			break;
	}
