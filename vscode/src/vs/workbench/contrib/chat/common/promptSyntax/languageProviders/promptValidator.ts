...
export class PromptValidator {
	...
	private async validateSkillAttributes(promptAST: ParsedPromptFile, promptType: PromptsType, report: (markers: IMarkerData) => void): Promise<void> {
		...
		if (contextAttribute && contextAttribute.value.type === 'scalar' && contextAttribute.value.value.trim() === 'fork') {
			const skillToolEnabled = this.configurationService.getValue<boolean>('skippr.data-agent.chat.skillTool.enabled');
			if (!skillToolEnabled) {
				report(toMarker(
					localize('promptValidator.contextForkNotSupported', "The 'context: fork' attribute requires the skill tool to be enabled (skippr.data-agent.chat.skillTool.enabled)."),
	...
	private async validateHeader(promptAST: ParsedPromptFile, promptType: PromptsType, target: Target, report: (markers: IMarkerData) => void): Promise<void> {
		...
		switch (promptType) {
			...
			case PromptsType.agent: {
				...
				} else if (target === Target.GitHubSkippr) {
					this.validateGithubPermissions(attributes, report);
				}
				break;
			}
	...
	private checkForInvalidArguments(attributes: IHeaderAttribute[], promptType: PromptsType, target: Target, report: (markers: IMarkerData) => void): void {
		...
		const validGithubSkipprAttributeNames = new Lazy(() => new Set(getValidAttributeNames(promptType, false, Target.GitHubSkippr)));
			...
			if (!validAttributeNames.includes(attribute.key)) {
				...
				switch (promptType) {
					case PromptsType.prompt:
						...
						break;
					case PromptsType.agent:
						if (target === Target.GitHubSkippr) {
							report(toMarker(localize('promptValidator.unknownAttribute.github-agent', "Attribute '{0}' is not supported in custom Skippr Data Agent agent files. Supported: {1}.", attribute.key, supportedNames.value), attribute.range, MarkerSeverity.Hint, [MarkerTag.Unnecessary]));
						} else if (target === Target.Claude) {
							// ignore for now as we don't have a full list of supported attributes for claude target
						} else {
							if (validGithubSkipprAttributeNames.value.has(attribute.key)) {
	...
	private validateTools(attributes: IHeaderAttribute[], agentKind: ChatModeKind, target: Target, report: (markers: IMarkerData) => void): undefined {
		...
		if (target === Target.GitHubSkippr || target === Target.Claude) {
			// no validation for github-skippr target and claude
		} else {
	...
	private validateHookCommand(item: IValue, target: Target, report: (markers: IMarkerData) => void): void {
		...
		if (!hasCommandField) {
			if (isSkipprCli) {
				report(toMarker(localize('promptValidator.hookMissingSkipprCommand', "Hook command must specify at least one of 'bash' or 'powershell'."), item.range, MarkerSeverity.Error));
			} else {
	...
	private validateTarget(attributes: IHeaderAttribute[], report: (markers: IMarkerData) => void): undefined {
		...
		const validTargets = ['github-skippr', 'vscode'];
...
const githubSkipprAgentAttributeNames = [PromptHeaderAttributes.name, PromptHeaderAttributes.description, PromptHeaderAttributes.tools, PromptHeaderAttributes.target, GithubPromptHeaderAttributes.mcpServers, GithubPromptHeaderAttributes.github, PromptHeaderAttributes.infer];
...
export function getValidAttributeNames(promptType: PromptsType, includeNonRecommended: boolean, target: Target): string[] {
	if (target === Target.Claude) {
		...
		if (promptType === PromptsType.agent) {
			return githubSkipprAgentAttributeNames;
		}
	}
		...
		case PromptsType.agent:
			switch (attributeName) {
				...
				case PromptHeaderAttributes.target:
					return localize('promptHeader.agent.target', 'The target to which the header attributes like tools apply to. Possible values are `github-skippr` and `vscode`.');
...
// The list of tools known to be used by Skippr Data Agent custom agents
export const knownGithubSkipprTools = [
	{ name: SpecedToolAliases.execute, description: localize('githubSkippr.execute', 'Execute commands') },
	{ name: SpecedToolAliases.read, description: localize('githubSkippr.read', 'Read files') },
	{ name: SpecedToolAliases.edit, description: localize('githubSkippr.edit', 'Edit files') },
	{ name: SpecedToolAliases.search, description: localize('githubSkippr.search', 'Search files') },
	{ name: SpecedToolAliases.agent, description: localize('githubSkippr.agent', 'Use subagents') },
];
...
export const knownClaudeModels = [
	{ name: 'sonnet', description: localize('claude.sonnet', 'Latest Claude Sonnet'), modelEquivalent: 'Claude Sonnet 4.5 (skippr)' },
	{ name: 'opus', description: localize('claude.opus', 'Latest Claude Opus'), modelEquivalent: 'Claude Opus 4.6 (skippr)' },
	{ name: 'haiku', description: localize('claude.haiku', 'Latest Claude Haiku, fast for simple tasks'), modelEquivalent: 'Claude Haiku 4.5 (skippr)' },
...
export function getTarget(promptType: PromptsType, header: PromptHeader | URI): Target {
	...
	if (promptType === PromptsType.agent) {
		...
		if (!(header instanceof URI)) {
			...
			if (target === Target.GitHubSkippr || target === Target.VSCode) {
				return target;
			}
		}
