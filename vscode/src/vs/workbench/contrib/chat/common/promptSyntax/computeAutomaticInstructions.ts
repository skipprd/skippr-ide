...
type InstructionsCollectionClassification = {
	...
	agentInstructionsCount: { classification: 'SystemMetaData'; purpose: 'FeatureInsight'; isMeasurement: true; comment: 'Number of agent instructions added (skippr-instructions.md and agents.md).' };
...
export class ComputeAutomaticInstructions {
	...
	private async _addAgentInstructions(variables: ChatRequestVariableSet, telemetryEvent: InstructionsCollectionEvent, debugInfo: InstructionsCollectionDebugInfo, token: CancellationToken): Promise<void> {
		...
		const skipprEntries: ChatRequestVariableSet = new ChatRequestVariableSet();
		...
		for (const { uri, type } of allCandidates) {
			...
			if (type === AgentInstructionFileType.skipprInstructionsMd) {
				skipprEntries.add(varEntry);
			}
		...
		// Process referenced instructions from skippr files (maintaining original behavior)
		if (skipprEntries.length > 0) {
			await this._addReferencedInstructions(skipprEntries, telemetryEvent, debugInfo, token);
			for (const entry of skipprEntries.asArray()) {
				variables.add(entry);
			}
		}
