 ...
 */
export const AGENT_DEBUG_LOG_ENABLED_SETTING = 'skippr.data-agent.chat.agentDebugLog.enabled';

/**
 * Configuration key for enabling file logging for the agent debug log.
 */
export const AGENT_DEBUG_LOG_FILE_LOGGING_ENABLED_SETTING = 'skippr.data-agent.chat.agentDebugLog.fileLogging.enabled';
...
/**
 * URI scheme used by the Skippr extension for built-in skills.
 */
export const COPILOT_SKILL_URI_SCHEME = 'skippr-skill';
...
export enum PromptFileSource {
	...
	SkipprPersonal = 'skippr-personal',
...
export function getSourceDescription(source: PromptFileSource): string | undefined {
	switch (source) {
		...
		case PromptFileSource.GitHubWorkspace:
			return localize('source.githubWorkspace', "Workspace (only used by Skippr agents)");
		case PromptFileSource.SkipprPersonal:
			return localize('source.skipprPersonal', "Global (only used by Skippr agents)");
