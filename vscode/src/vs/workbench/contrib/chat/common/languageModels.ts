...
export namespace ILanguageModelChatMetadata {
	...
	export function matchesQualifiedName(name: string, metadata: ILanguageModelChatMetadata): boolean {
		if (metadata.vendor === 'skippr' && name === metadata.name) {
			return true;
		}
...
const languageModelChatProviderType = {
	...
	properties: {
		...
		managementCommand: {
			type: 'string',
			description: localize('vscode.extension.contributes.languageModels.managementCommand', "A command to manage the language model chat provider, e.g. 'Manage Skippr models'. This is used in the chat model picker. If not provided, a gear icon is not rendered during vendor selection."),
			deprecated: true,
 ...
 */
const AUTO_MODEL_IDENTIFIER = 'skippr/auto';
			...
			try {
				...
				if (models.length) {
					...
					for (const m of models) {
						if (vendor.isDefault) {
							// Special case for skippr models - they are all user selectable unless marked otherwise
	...
	private _refreshModelsControlManifest(): void {
		...
		if (response?.free) {
			...
			for (const entry of freeEntries) {
				...
				free[entry.id] = { label: entry.label, featured: entry.featured, exists: this._modelCache.has(`skippr/${entry.id}`) };
			}
		}
		...
		if (response?.paid) {
			...
			for (const entry of paidEntries) {
				...
				paid[entry.id] = { label: entry.label, featured: entry.featured, minVSCodeVersion: entry.minVSCodeVersion, exists: this._modelCache.has(`skippr/${entry.id}`) };
			}
		}
