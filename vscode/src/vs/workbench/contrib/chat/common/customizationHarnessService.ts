...
export interface IHarnessDescriptor {
	 ...
	 * invocation (e.g. Claude hooks → `skippr.claude.hooks`).
	 *
...
/**
 * Returns the user-home directories accessible to the Skippr Agent harness.
 */
export function getCliUserRoots(userHome: URI): readonly URI[] {
	return [
		joinPath(userHome, '.skippr'),
...
/**
 * Creates a "Skippr Agent" harness descriptor.
 */
export function createCliHarnessDescriptor(cliUserRoots: readonly URI[], extras: readonly string[]): IHarnessDescriptor {
	return createRestrictedHarnessDescriptor(
		SessionType.SkipprCLI,
		localize('harness.cli', "Skippr Agent"),
		ThemeIcon.fromId(Codicon.skippr.id),
		cliUserRoots,
		extras,
		{
			hideGenerateButton: true,
			requiredAgentId: 'skipprcli',
			workspaceSubpaths: ['.github', '.skippr', '.agents', '.claude'],
			sectionOverrides: new Map([
				[AICustomizationManagementSection.Instructions, {
					rootFileShortcuts: [AGENT_MD_FILENAME],
				}],
