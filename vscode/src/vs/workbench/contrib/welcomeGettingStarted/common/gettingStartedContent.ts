...
export const skipprSettingsMessage = localize({ key: 'settings', comment: ['{Locked="["}', '{Locked="]({0})"}', '{Locked="]({1})"}'] }, "{0} Skippr may show [public code]({1}) suggestions and use your data to improve the product. You can change these [settings]({2}) anytime.", defaultChat.provider.default.name, defaultChat.publicCodeMatchesUrl, defaultChat.manageSettingsUrl);
...
const SkipprStepTitle = localize('gettingStarted.skipprSetup.title', "Use Skippr Data Agent");
const SkipprDescription = localize({ key: 'gettingStarted.skipprSetup.description', comment: ['{Locked="["}', '{Locked="]({0})"}'] }, "Use [Skippr Data Agent]({0}) to generate models, investigate lineage, and execute data engineering workflows with natural language.", defaultChat.documentationUrl ?? '');
const SkipprTermsString = localize({ key: 'gettingStarted.skipprSetup.terms', comment: ['{Locked="]({2})"}', '{Locked="]({3})"}'] }, "By continuing with Skippr Data Agent, you agree to Skippr's [Terms]({2}) and [Privacy Statement]({3})", defaultChat.provider.default.name, defaultChat.provider.default.name, defaultChat.termsStatementUrl, defaultChat.privacyStatementUrl);
const SkipprAnonymousButton = Button(localize('setupSkipprButton.setup', "Continue Without Login"), `command:skippr.openSplash`);
const SkipprSignedOutButton = Button(localize('setupSkipprButton.setup', "Sign In To Skippr"), `command:skippr.auth.signIn`);
const SkipprSignedInButton = Button(localize('setupSkipprButton.setup', "Open Skippr Data Agent"), `command:workbench.action.chat.open`);
const SkipprCompleteButton = Button(localize('setupSkipprButton.chatWithSkippr', "Open Skippr Data Agent"), 'command:workbench.action.chat.open');

function createSkipprSetupStep(id: string, button: string, when: string, includeTerms: boolean): BuiltinGettingStartedStep {
	...
	return {
		id,
		title: SkipprStepTitle,
		description,
		when: `${when} && !chatSetupHidden && !chatSetupDisabledInWorkspace`,
		media: {
			type: 'svg', altText: 'Skippr Data Agent', path: 'multi-file-edits.svg'
		},
	};
}
...
export const walkthroughs: GettingStartedWalkthroughContent = [
	{
		id: 'Setup',
		title: localize('gettingStarted.setup.title', "Get started with Skippr IDE"),
		description: localize('gettingStarted.setup.description', "Configure your workspace, sign in, and start building data pipelines"),
		isFeatured: true,
		icon: setupIcon,
		when: '!isWeb',
		walkthroughPageTitle: localize('gettingStarted.setup.walkthroughPageTitle', 'Setup Skippr IDE'),
		next: 'Beginner',
		content: {
			type: 'steps',
			steps: [
				createSkipprSetupStep('SkipprSetupAnonymous', SkipprAnonymousButton, 'chatAnonymous && !chatSetupCompleted', true),
				createSkipprSetupStep('SkipprSetupSignedOut', SkipprSignedOutButton, 'chatEntitlementSignedOut && !chatAnonymous', false),
				createSkipprSetupStep('SkipprSetupComplete', SkipprCompleteButton, 'chatSetupCompleted && !chatSetupDisabled && (chatAnonymous || chatPlanPro || chatPlanProPlus || chatPlanMax || chatPlanBusiness || chatPlanEnterprise || chatPlanFree)', false),
				createSkipprSetupStep('SkipprSetupSignedIn', SkipprSignedInButton, '!chatEntitlementSignedOut && (!chatSetupCompleted || chatSetupDisabled || chatPlanCanSignUp)', false),
				{
					id: 'pickColorTheme',
		...
		content: {
			...
			steps: [
				...
				},
				{
					id: 'scm',
					title: localize('gettingStarted.scm.title', "Track your code with Git"),
					description: localize('gettingStarted.scm.description.interpolated', "Track repository changes directly in Skippr IDE source control.\n{0}", Button(localize('openSCM', "Open Source Control"), 'command:workbench.view.scm')),
				...
				},
				{
					...
					description: localize('gettingStarted.workspaceTrust.description.interpolated', "{0} lets you decide whether your project folders should **allow or restrict** automatic code execution __(required for extensions, debugging, etc)__.\nOpening a file/folder will prompt to grant trust. You can always {1} later.", Button(localize('workspaceTrust', "Workspace Trust"), 'https://skippr.com/ide/docs/docs/editor/workspace-trust'), Button(localize('enableTrust', "enable trust"), 'command:toSide:workbench.trust.manage')),
