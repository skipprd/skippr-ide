...
const enum AddConfigurationSkipprCommand {
	/** Returns whether MCP enhanced setup is enabled. */
	IsSupported = 'skippr.data-agent.chat.mcp.setup.check',

	/** Takes an npm/pip package name, validates its owner. */
	ValidatePackage = 'skippr.data-agent.chat.mcp.setup.validatePackage',

	/** Returns the resolved MCP configuration. */
	StartFlow = 'skippr.data-agent.chat.mcp.setup.flow',
}
...
export class McpAddConfigurationCommand {
	...
	private async getServerType(): Promise<AddConfigurationType | undefined> {
		...
		try {
			aiSupported = await this._commandService.executeCommand<boolean>(AddConfigurationSkipprCommand.IsSupported);
		} catch {
			// ignored
		}
		...
		if (aiSupported) {
			...
			items.push(
				{ type: 'separator', label: localize('mcp.serverType.skippr', "Model-Assisted") },
				...elligableTypes
			);
		}
	...
	private async getAssistedConfig(type: AssistedConfigurationType): Promise<{ name?: string; server: Omit<IMcpStdioServerConfiguration, 'type'>; inputs?: IMcpServerVariable[]; inputValues?: Record<string, string> } | undefined> {
		...
		this._commandService.executeCommand<ValidatePackageResult>(
			AddConfigurationSkipprCommand.ValidatePackage,
			{
				type: packageType,
				name: packageName,
				targetConfig: {
		...
		const config = await this._commandService.executeCommand<AssistedServerConfiguration>(
			AddConfigurationSkipprCommand.StartFlow,
			{
				name: packageName,
				type: packageType
			}
