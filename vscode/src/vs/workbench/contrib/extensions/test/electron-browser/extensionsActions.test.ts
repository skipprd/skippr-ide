...
suite('EnableAIFeaturesInWorkspaceAction', () => {
	...
	setup(() => {
		...
		instantiationService.stub(IProductService, { defaultChatAgent: { chatExtensionId: 'Skippr.data-agent' } } as Partial<IProductService>);
	});
	...
	test('test enable AI in workspace updates workspace setting when AI is disabled globally', async () => {
		...
		const chatExtension = aLocalExtension('skippr-chat', { publisher: 'GitHub' }, { type: ExtensionType.System });
		...
		const skipprChat = extensions.find(e => e.identifier.id === 'skippr.data-agent');
		assert.ok(skipprChat);
		...
		disposables.add(instantiationService.createInstance(ExtensionContainers, [testObject]));
		testObject.extension = skipprChat;
		assert.ok(testObject.enabled);
