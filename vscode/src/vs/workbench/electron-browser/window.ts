...
export class NativeWindow extends BaseWindow {
		...
		ipcRenderer.on('vscode:showResolveShellEnvError', (event: unknown, ...argsRaw: unknown[]) => {
			...
			this.notificationService.prompt(
				...
				},
				{
					...
					run: () => this.openerService.open('https://skippr.com/go/fwlink/?linkid=2149667')
				}]
			);
		});
		...
		ipcRenderer.on('vscode:showCredentialsError', (event: unknown, ...argsRaw: unknown[]) => {
			...
			this.notificationService.prompt(
				...
				[{
					...
					run: () => this.openerService.open('https://skippr.com/go/fwlink/?linkid=2190713')
				}]
			);
		});
			...
			this.notificationService.prompt(
				...
				[{
					...
					run: () => {
						...
						const stableURL = 'https://skippr.com/ide/docs/docs/?dv=osx';
						const insidersURL = 'https://skippr.com/ide/docs/docs/?dv=osx&build=insiders';
	...
	private async handleWarnings(): Promise<void> {
		...
		}, () => shellEnv, () => this.openerService.open('https://skippr.com/go/fwlink/?linkid=2149667'));
	}
