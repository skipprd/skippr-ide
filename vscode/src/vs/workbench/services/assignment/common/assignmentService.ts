...
import { SkipprAssignmentFilterProvider } from './assignmentFilters.js';
...
export class WorkbenchAssignmentService extends Disposable implements IAssignmentService {
	...
	private async setupTASClient(): Promise<TASClient> {
		...
		const extensionsFilterProvider = this.instantiationService.createInstance(SkipprAssignmentFilterProvider);
...
registry.registerConfiguration({
	...
	'properties': {
		'workbench.enableExperiments': {
			'type': 'boolean',
			'description': localize('workbench.enableExperiments', "Fetches experiments to run from the configured online service."),
			'default': true,
