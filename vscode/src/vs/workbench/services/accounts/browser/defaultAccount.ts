...
import { SkipprSessionSearchPolicy, ISkipprTokenInfo, IDefaultAccount, IDefaultAccountAuthenticationProvider, IEntitlementsData, IPolicyData } from '../../../../base/common/defaultAccount.js';
...
export class DefaultAccountService extends Disposable implements IDefaultAccountService {
	...
	get skipprTokenInfo(): ISkipprTokenInfo | null { return this.defaultAccountProvider?.skipprTokenInfo ?? null; }
	...
	private readonly _onDidChangeSkipprTokenInfo = this._register(new Emitter<ISkipprTokenInfo | null>());
	readonly onDidChangeSkipprTokenInfo = this._onDidChangeSkipprTokenInfo.event;
	...
	setDefaultAccountProvider(provider: IDefaultAccountProvider): void {
		...
		provider.refresh().then(account => {
			...
			this._register(provider.onDidChangeSkipprTokenInfo(tokenInfo => this._onDidChangeSkipprTokenInfo.fire(tokenInfo)));
		});
	}
...
interface ICachedAccountData {
	...
	readonly skipprTokenInfo?: ISkipprTokenInfo;
}
...
interface IDefaultAccountData {
	...
	skipprTokenInfo: ISkipprTokenInfo | null;
}
...
class DefaultAccountProvider extends Disposable implements IDefaultAccountProvider {
	...
	private _skipprTokenInfo: ISkipprTokenInfo | null = null;
	get skipprTokenInfo(): ISkipprTokenInfo | null { return this._skipprTokenInfo; }
	...
	constructor(
		...
		this._skipprTokenInfo = cachedAccountData?.skipprTokenInfo ?? null;
	...
	private getCachedAccountData(): ICachedAccountData | null {
		...
		if (cached) {
			try {
				...
				// ICachedAccountData ({ accountPolicyData, skipprTokenInfo }).
				...
				const { accountId, policyData, tokenEntitlementsFetchedAt, mcpRegistryDataFetchedAt, skipprTokenInfo } = parsed;
				if (accountId && policyData) {
					this.logService.debug('[DefaultAccount] Initializing with cached policy data (migrating old format)');
					const result: ICachedAccountData = { accountPolicyData: { accountId, policyData, tokenEntitlementsFetchedAt, mcpRegistryDataFetchedAt }, skipprTokenInfo };
					this.storageService.store(CACHED_POLICY_DATA_KEY, JSON.stringify(result), StorageScope.APPLICATION, StorageTarget.MACHINE);
					return result;
				}

				// New format
				const { accountPolicyData, skipprTokenInfo: wrappedSkipprTokenInfo } = parsed;
				if (accountPolicyData?.accountId && accountPolicyData?.policyData) {
					...
					return { accountPolicyData, skipprTokenInfo: wrappedSkipprTokenInfo };
				}
			} catch (error) {
	...
	private setDefaultAccount(account: IDefaultAccountData | null): void {
		...
		if (account) {
			...
			this.setSkipprTokenInfo(account.skipprTokenInfo);
	...
	private setSkipprTokenInfo(skipprTokenInfo: ISkipprTokenInfo | null): void {
		if (equals(this._skipprTokenInfo, skipprTokenInfo)) {
			return;
		}
		this._skipprTokenInfo = skipprTokenInfo;
		this._onDidChangeSkipprTokenInfo.fire(this._skipprTokenInfo);
	}
	...
	private cachePolicyData(accountPolicyData: IAccountPolicyData | null): void {
		if (accountPolicyData) {
			...
			const cachedAccountData: ICachedAccountData = {
				accountPolicyData,
				skipprTokenInfo: this._skipprTokenInfo ?? undefined,
			};
	...
	private async getDefaultAccountFromAuthenticatedSessions(authenticationProvider: IDefaultAccountAuthenticationProvider, sessions: AuthenticationSession[], options?: { forceRefresh?: boolean }): Promise<IDefaultAccountData | null> {
		try {
			...
			return {
				...
				skipprTokenInfo: tokenEntitlementsResult?.data?.skipprTokenInfo ?? null,
			};
		} catch (error) {
	...
	private async getTokenEntitlements(sessions: AuthenticationSession[], accountPolicyData: IAccountPolicyData | undefined, options?: { forceRefresh?: boolean }): Promise<{ data: { policyData: Partial<IPolicyData>; skipprTokenInfo: ISkipprTokenInfo } | undefined; fetchedAt: number }> {
		if (!options?.forceRefresh && accountPolicyData?.tokenEntitlementsFetchedAt && !this.isDataStale(accountPolicyData.tokenEntitlementsFetchedAt)) {
			...
			return { data: { policyData: accountPolicyData.policyData, skipprTokenInfo: this._skipprTokenInfo ?? {} }, fetchedAt: accountPolicyData.tokenEntitlementsFetchedAt };
		}
		const data = await this.requestTokenEntitlements(sessions);
		return { data, fetchedAt: Date.now() };
	}

	private async requestTokenEntitlements(sessions: AuthenticationSession[]): Promise<{ policyData: Partial<IPolicyData>; skipprTokenInfo: ISkipprTokenInfo } | undefined> {
			...
			if (chatData) {
				...
				return {
					policyData: {
						...
						// Session search policy enum from Skippr token
						session_search: Number(tokenMap.get('session_search') ?? '0') as SkipprSessionSearchPolicy,
					},
					skipprTokenInfo: {
						sn: tokenMap.get('sn'),
	...
	private getEntitlementUrl(): string | undefined {
		if (this.getDefaultAccountAuthenticationProvider().enterprise) {
			try {
				...
				return `${enterpriseUrl.protocol}//api.${enterpriseUrl.hostname}${enterpriseUrl.port ? ':' + enterpriseUrl.port : ''}/skippr_internal/user`;
			} catch (error) {
	...
	private getTokenEntitlementUrl(): string | undefined {
		if (this.getDefaultAccountAuthenticationProvider().enterprise) {
			try {
				...
				return `${enterpriseUrl.protocol}//api.${enterpriseUrl.hostname}${enterpriseUrl.port ? ':' + enterpriseUrl.port : ''}/skippr_internal/v2/token`;
			} catch (error) {
				this.logService.error(error);
			}
		}
	...
	private getMcpRegistryDataUrl(): string | undefined {
		if (this.getDefaultAccountAuthenticationProvider().enterprise) {
			try {
				...
				return `${enterpriseUrl.protocol}//api.${enterpriseUrl.hostname}${enterpriseUrl.port ? ':' + enterpriseUrl.port : ''}/skippr/mcp_registry`;
			} catch (error) {
				this.logService.error(error);
			}
		}
