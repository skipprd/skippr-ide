...
import { SkipprSessionSearchPolicy, IDefaultAccount, IDefaultAccountAuthenticationProvider, IPolicyData } from '../../../../../base/common/defaultAccount.js';
...
suite('MultiplexPolicyService', () => {
	...
	test('session_search policy disabled overrides setting', async () => {
		...
		const policyData: IPolicyData = { session_search: SkipprSessionSearchPolicy.Disabled };
		defaultAccountService.setDefaultAccountProvider(new DefaultAccountProvider(BASE_DEFAULT_ACCOUNT, policyData));
		await defaultAccountService.refresh();
	...
	test('session_search policy enabled does not override setting', async () => {
		...
		const policyData: IPolicyData = { session_search: SkipprSessionSearchPolicy.Enabled };
		defaultAccountService.setDefaultAccountProvider(new DefaultAccountProvider(BASE_DEFAULT_ACCOUNT, policyData));
		await defaultAccountService.refresh();
	...
	test('session_search policy with no opinion values does not override setting', async () => {
		...
		for (const value of [SkipprSessionSearchPolicy.Unknown, SkipprSessionSearchPolicy.Unconfigured, SkipprSessionSearchPolicy.NoPolicy]) {
			...
			assert.strictEqual(policyService.getPolicyValue('PolicySettingF'), undefined, `Expected undefined for SkipprSessionSearchPolicy value ${value}`);
			assert.strictEqual(policyConfiguration.configurationModel.getValue('setting.F'), undefined, `Expected undefined for SkipprSessionSearchPolicy value ${value}`);
		}
	});
});
