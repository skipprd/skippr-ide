...
export interface IChatEntitlementService {
	...
	readonly chatTrackingId: string | undefined;
...
export function getChatPlanName(chatEntitlement: ChatEntitlement): string {
	switch (chatEntitlement) {
		case ChatEntitlement.EDU:
			return localize('plan.eduName', 'Skippr EDU');
		case ChatEntitlement.Pro:
			return localize('plan.proName', 'Skippr Pro');
		case ChatEntitlement.ProPlus:
			return localize('plan.proPlusName', 'Skippr Pro+');
		case ChatEntitlement.Max:
			return localize('plan.maxName', 'Skippr Max');
		case ChatEntitlement.Business:
			return localize('plan.businessName', 'Skippr Business');
		case ChatEntitlement.Enterprise:
			return localize('plan.enterpriseName', 'Skippr Enterprise');
		default:
			return localize('plan.freeName', 'Skippr Free');
	}
}
...
export class ChatEntitlementService extends Disposable implements IChatEntitlementService {
	...
	get chatTrackingId(): string | undefined {
		return this.context?.value.state.chatTrackingId;
	}
...
interface IEntitlements {
	...
	readonly sku?: string;
	readonly chatTrackingId?: string;
	readonly quotas?: IQuotas;
}
...
export class ChatEntitlementRequests extends Disposable {
	...
	private async doResolveEntitlement(defaultAccount: IDefaultAccount, token: CancellationToken): Promise<IEntitlements | undefined> {
		...
		if (entitlementsData.access_type_sku === 'free_limited_skippr') {
			entitlement = ChatEntitlement.Free;
		} else if (entitlementsData.can_signup_for_limited) {
			entitlement = ChatEntitlement.Available;
		} else if (entitlementsData.skippr_plan === 'individual_edu') {
			entitlement = ChatEntitlement.EDU;
		} else if (entitlementsData.skippr_plan === 'individual') {
			entitlement = ChatEntitlement.Pro;
		} else if (entitlementsData.skippr_plan === 'individual_pro') {
			entitlement = ChatEntitlement.ProPlus;
		} else if (entitlementsData.skippr_plan === 'individual_max') {
			entitlement = ChatEntitlement.Max;
		} else if (entitlementsData.skippr_plan === 'business') {
			entitlement = ChatEntitlement.Business;
		} else if (entitlementsData.skippr_plan === 'enterprise') {
		...
		const entitlements: IEntitlements = {
			...
			chatTrackingId: entitlementsData.analytics_tracking_id
		};
	...
	private update(state: IEntitlements): void {
		...
		this.context.update({ entitlement: this.state.entitlement, organisations: this.state.organisations, sku: this.state.sku, chatTrackingId: this.state.chatTrackingId });
	...
	private async onUnknownSignUpError(detail: string, logMessage: string): Promise<boolean> {
		...
		if (!this.lifecycleService.willShutdown) {
			const { confirmed } = await this.dialogService.confirm({
				type: Severity.Error,
				message: localize('unknownSignUpError', "An error occurred while signing up for the Skippr Free plan. Would you like to try again?"),
				detail,
	...
	private onUnprocessableSignUpError(logMessage: string, logDetails: string): void {
		...
		if (!this.lifecycleService.willShutdown) {
			this.dialogService.prompt({
				type: Severity.Error,
				message: localize('unprocessableSignUpError', "An error occurred while signing up for the Skippr Free plan."),
				detail: logDetails,
				buttons: [
					{
	...
	async signIn(options?: { useSocialProvider?: string; additionalScopes?: readonly string[] }): Promise<{ defaultAccount?: IDefaultAccount; entitlements?: IEntitlements }> {
		const defaultAccount = await this.defaultAccountService.signIn({
			...
			extraAuthorizeParameters: { get_started_with: 'skippr-ide' },
...
export interface IChatEntitlementContextState extends IChatSentiment {
	...
	/**
	 * User's chat tracking ID from the entitlement API.
	 */
	chatTrackingId: string | undefined;
}
...
export class ChatEntitlementContext extends Disposable {
	...
	update(context: { entitlement: ChatEntitlement; organisations: string[] | undefined; sku: string | undefined; chatTrackingId: string | undefined }): Promise<void>;
	async update(context: { completed?: boolean; installed?: boolean; disabled?: boolean; untrusted?: boolean; disabledInWorkspace?: boolean; hidden?: false; later?: boolean; entitlement?: ChatEntitlement; organisations?: string[]; sku?: string; chatTrackingId?: string }): Promise<void> {
		...
		if (typeof context.entitlement === 'number') {
			...
			this._state.chatTrackingId = context.chatTrackingId;
		...
		if (isAnonymous(this.configurationService, this._state.entitlement, this._state)) {
			this._state.sku = 'no_auth_limited_skippr';
		}
	...
	private updateContextSync(): void {
		...
		this.isInternalContextKey.set(Boolean(state.organisations?.some(org => org === 'skippr')));
