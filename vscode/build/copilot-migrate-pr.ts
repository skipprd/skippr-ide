...
// Migrates a pull request from microsoft/vscode-skippr-chat to microsoft/vscode.
//
// The diff is fetched, file paths are rewritten to prepend `extensions/skippr/`,
// and a new PR is created in microsoft/vscode via the `gh` CLI.
//
// Usage:
//   node build/skippr-migrate-pr.ts <PR_NUMBER> [--dry-run] [--verbose]
//
// Requirements:
...
const SOURCE_REPO = 'microsoft/vscode-skippr-chat';
const TARGET_REPO = 'microsoft/vscode';
const PATH_PREFIX = 'extensions/skippr';
...
function parseArgs(): Options {
	...
	if (!prNumber) {
		console.error('Usage: node build/skippr-migrate-pr.ts <PR_NUMBER> [--dry-run] [--verbose]');
		process.exit(1);
	}
...
function getMigrationBranchName(prNumber: number): string {
	return `vscode-skippr-chat/migrate-${prNumber}`;
}
...
/**
 * Rewrites file paths in a unified diff to prepend `extensions/skippr/`.
 *
 * Handles:
...
async function createBranchAndApplyCommits(
	...
	for (let i = 0; i < commits.length; i++) {
		...
		const tmpPatch = path.join(os.tmpdir(), `skippr-migrate-pr-${prNumber}-${i + 1}.patch`);
