...
<string>${translations?.['testModule']?.['test.desc'] || 'Default Desc'}</string>
		...
		test('should include VS Code specific URLs', () => {
			...
			assert.ok(result.includes('https://skippr.com/ide/docs/'));
			assert.ok(result.includes('https://skippr.com/ide/docs/docs/setup/enterprise'));
		});
...
<plist version="1.0">
	<dict>
		...
		<string>This profile manages VS Code. For more information see https://skippr.com/ide/docs/docs/setup/enterprise</string>
		<key>PayloadDisplayName</key>
		<string>VS Code</string>
		<key>PayloadIdentifier</key>
		<string>com.microsoft.vscode</string>
		...
		test('should include enterprise documentation link', () => {
			...
			assert.ok(result.profile.includes('https://skippr.com/ide/docs/docs/setup/enterprise'));
		});
