		...
		},
		{
			...
			localization: {
				description: {
					...
					value: 'Specify a list of extensions that are allowed to use. This helps maintain a secure and consistent development environment by restricting the use of unauthorized extensions. More information: https://skippr.com/ide/docs/docs/setup/enterprise#_configure-allowed-extensions'
				}
			},
			type: 'object',
			default: '*'
...
const mockProduct: ProductJson = {
	nameLong: 'Code - OSS',
	darwinBundleIdentifier: 'com.skippr.ide',
...
const frenchTranslations = [
	{
		...
		languageTranslations: {
			'': {
				...
				'autoApprove2.description': `L’approbation automatique globale, également appelée « mode YOLO », désactive complètement l’approbation manuelle pour tous les outils dans tous les espaces de travail, permettant à l’agent d’agir de manière totalement autonome. Ceci est extrêmement dangereux et est *jamais* recommandé, même dans des environnements conteneurisés comme [Codespaces](https://github.com/features/codespaces) et [Dev Containers](https://skippr.com/ide/extensions/items?itemName=skippr-remote.remote-containers), où des clés utilisateur sont transférées dans le conteneur et pourraient être compromises.

Cette fonctionnalité désactive [les protections de sécurité critiques](https://skippr.com/ide/docs/docs/skippr/security) et facilite considérablement la compromission de la machine par un attaquant.`,
				'mcp.gallery.serviceUrl': 'Configurer l’URL du service de la galerie MCP à laquelle se connecter',
				'extensions.allowed.policy': 'Spécifiez une liste d’extensions autorisées. Cela permet de maintenir un environnement de développement sécurisé et cohérent en limitant l’utilisation d’extensions non autorisées. Plus d’informations : https://skippr.com/ide/docs/docs/setup/enterprise#_configure-allowed-extensions',
...
suite('Policy E2E conversion', () => {
	...
	test('should render macOS policy profile from policies list', async () => {
		...
		const fixturePath = path.join(import.meta.dirname, 'fixtures', 'policies', 'darwin', 'com.skippr.ide.mobileconfig');
	...
	test('should render macOS manifest from policies list', async () => {
		...
		// Load the expected fixture file
		const fixturePath = path.join(import.meta.dirname, 'fixtures', 'policies', 'darwin', 'en-us', 'com.skippr.ide.plist');
		const expectedContent = await fs.readFile(fixturePath, 'utf-8');
	...
	test('should render macOS manifest with fr-fr locale', async () => {
		...
		// Load the expected fixture file
		const fixturePath = path.join(import.meta.dirname, 'fixtures', 'policies', 'darwin', 'fr-fr', 'com.skippr.ide.plist');
		const expectedContent = await fs.readFile(fixturePath, 'utf-8');
