...
KNOWN_SUITES="api-folder api-workspace colorize terminal-suggest typescript markdown emmet git git-base ipynb notebook-renderers configuration-editing github-authentication skippr css html"
...
if should_run_suite skippr; then
echo
echo "### Skippr tests"
echo
npm run test-extension -- -l skippr "${GREP_ARGS[@]}"
kill_app
fi
