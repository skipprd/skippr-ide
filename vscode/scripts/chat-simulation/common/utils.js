...
async function resolveBuild(buildArg) {
	...
	if (isVersionString(buildArg)) {
		...
		// Check if skippr is already bundled as a built-in extension
		// (recent Insiders/Stable builds ship it in the app's extensions/ dir).
		const builtinExtDir = getBuiltinExtensionsDir(exePath);
		const hasSkipprBuiltin = builtinExtDir && fs.existsSync(builtinExtDir)
			&& fs.readdirSync(builtinExtDir).some(e => e === 'skippr');

		if (hasSkipprBuiltin) {
			console.log(`[chat-simulation] Skippr is bundled as a built-in extension`);
		} else {
			// Install skippr-chat from the marketplace into our shared
 ...
 * BuiltinChatExtensionEnablementMigration from disabling the skippr
...
/**
 * Write VS Code settings that point the skippr extension at the mock server.
 * @param {string} userDataDir
 * @param {{ url: string }} mockServer
...
function writeSettings(userDataDir, mockServer, overrides) {
	...
	fs.writeFileSync(path.join(settingsDir, 'settings.json'), JSON.stringify({
		'skippr.data-agent.advanced.debug.overrideProxyUrl': mockServer.url,
		'skippr.data-agent.advanced.debug.overrideCapiUrl': mockServer.url,
