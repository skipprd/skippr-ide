 ...
 * Uses the `cot_summary` field in the delta, matching the Skippr API wire format.
...
function handleRequest(req, res) {
	...
	// /skippr_internal/v2/token, /skippr_internal/v2/nltoken
	if (path.startsWith('/skippr_internal/')) {
		if (path.includes('/token') || path.includes('/nltoken')) {
			...
			// /skippr_internal/user, /skippr_internal/content_exclusion, etc.
			json(200, {});
		}
		return;
	}
	...
	if (path.startsWith('/agents')) {
		// /agents/sessions — SkipprSessions
		...
		// /agents/swe/models — CCAModelsList
			...
			json(200, {
				data: [{
					id: MODEL, name: 'GPT-4o (Mock)', vendor: 'skippr',
