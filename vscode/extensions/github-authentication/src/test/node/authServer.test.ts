...
suite('LoopbackAuthServer', () => {
	...
	setup(async () => {
		server = new LoopbackAuthServer(__dirname, 'http://localhost:8080', 'https://skippr.com/ide/docs', false);
	...
	test('should resolve with code and state on /callback with valid parameters', async () => {
		...
		assert.strictEqual(response.status, 302);
		assert.strictEqual(response.headers.get('location'), `/?redirect_uri=https%3A%2F%2Fskippr.com%2Fide%2Fdocs&app_name=${encodeURIComponent(env.appName)}`);
		await Promise.race([
...
suite('LoopbackAuthServer (portable mode)', () => {
	...
	setup(async () => {
		server = new LoopbackAuthServer(__dirname, 'http://localhost:8080', 'https://skippr.com/ide/docs', true);
		port = await server.start();
	});
