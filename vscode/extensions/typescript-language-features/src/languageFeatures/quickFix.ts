...
import { CompositeCommand, EditorChatFollowUp, EditorChatFollowUp_Args, Expand } from './util/skippr';
...
class TypeScriptQuickFixProvider implements vscode.CodeActionProvider<VsCodeCodeAction> {
	...
	private getFixesForTsCodeAction(
		...
		const skippr = vscode.extensions.getExtension('skippr.data-agent');
		if (skippr?.isActive) {
