...
├── common/
│   ├── mock-llm-server.js    # Mock CAPI server matching @vscode/skippr-api URL structure
...
The mock LLM server (`common/mock-llm-server.js`) implements the full CAPI URL structure from `@vscode/skippr-api`'s `DomainService`:
