...
Policies allow enterprise administrators to lock configuration settings via OS-level mechanisms (Windows Group Policy, macOS managed preferences, Linux config files) or via Skippr account-level policy data. This skill covers the complete procedure.
...
If this policy should also be controllable via Skippr account policy data (from `IPolicyData`), add a `value` function:
...
- `Extensions`
- `IntegratedTerminal`
- `InteractiveSession` (used for all chat/Skippr policies)
- `Telemetry`
- `Update`
...
| `vscode-website` (`gulpfile.policies.js`) | `policyData.jsonc` | Enterprise policy reference table at skippr.com/ide/docs/docs/enterprise/policies |
| `vscode-docs` | Generated from website build | `docs/enterprise/policies.md` |

## GitHub Preview Features

If your setting is a **GitHub Preview Feature** — meaning it's a Skippr/chat feature that organizations can disable via their GitHub account-level policy — you **must** add a `value` function that checks `policyData.chat_preview_features_enabled`.

### When to add this flag

Add the `chat_preview_features_enabled` check when **all** of these apply:

- The setting controls a Skippr or chat feature (e.g., agent tools, hooks, MCP, auto-approve)
...
The `chat_preview_features_enabled` field on `IPolicyData` (defined in `src/vs/base/common/defaultAccount.ts`) is populated from the user's Skippr Data Agent token entitlements. When an organization admin disables preview features, `chat_preview_features_enabled` is set to `false`.
