---
name: otel
description: OpenTelemetry instrumentation for the Skippr Data Agent extension — covers the four agent execution paths, the IOTelService abstraction, span/metric/event conventions, and the relationship between code and the user/developer monitoring docs. Use when adding/changing OTel spans, metrics, or events; instrumenting a new agent surface; touching the Skippr Agent bridge or Claude span emission; or updating `extensions/skippr/docs/monitoring/agent_monitoring*.md`.
---

# OpenTelemetry Instrumentation Skill

When adding, changing, or reviewing OTel telemetry in the Skippr Data Agent extension, **always read the two source-of-truth docs first** and **always keep them in sync with the code you change**.

## 1. Authoritative Documents

The `extensions/skippr/docs/monitoring/` directory contains the two specs that define the OTel contract for the extension. Treat them like the layout / layer specs in `vs/sessions`.

| Document | Path | Audience | Covers |
|---|---|---|---|
| User-facing | `extensions/skippr/docs/monitoring/agent_monitoring.md` | Extension users | Quick start, settings, env vars, exported spans/metrics/events, backend setup guides |
| Architecture | `extensions/skippr/docs/monitoring/agent_monitoring_arch.md` | Developers | Multi-agent strategies, span hierarchies, file structure, instrumentation points, `IOTelService`, configuration channels |
| Visual flow | `extensions/skippr/docs/monitoring/otel-data-flow.html` | Developers | Renders the bridge data flow for the in-process Skippr Agent agent |
...
| **Skippr Agent in-process** | Extension host (same process) | **Bridge SpanProcessor** — SDK creates spans natively; bridge forwards to debug panel | SDK native spans via bridge |
| **Skippr Agent terminal** | Separate terminal process | Forward OTel env vars | N/A (separate process) |
...
```
extensions/skippr/src/platform/otel/
├── common/
...
│   ├── agentOTelEnv.ts         # deriveSkipprCliOTelEnv / deriveClaudeOTelEnv
...
extensions/skippr/src/extension/
├── chatSessions/
│   ├── skipprcli/node/
│   │   ├── skipprCliBridgeSpanProcessor.ts  # Bridge: SDK spans → IOTelService (+ hook span enrichment)
│   │   ├── skipprcliSession.ts              # Root invoke_agent skipprcli span + traceparent + hook stash
│   │   └── skipprcliSessionService.ts       # Bridge installation + env var setup
│   └── claude/
...
`IOTelService` ([otelService.ts](../../../extensions/skippr/src/platform/otel/common/otelService.ts)) is the only abstraction consumers should depend on — never import the OTel SDK directly outside `node/otelServiceImpl.ts`. Three implementations:
...
Selection happens in [`src/extension/extension/vscode-node/services.ts`](../../../extensions/skippr/src/extension/extension/vscode-node/services.ts): exactly one of `NodeOTelService` or `InMemoryOTelService` is bound to `IOTelService` per extension host based on `resolveOTelConfig().enabled`.

## 5. Span / Metric / Event Conventions

Follow the [OTel GenAI semantic conventions](https://github.com/open-telemetry/semantic-conventions/blob/main/docs/gen-ai/). **Always use the constants from [`genAiAttributes.ts`](../../../extensions/skippr/src/platform/otel/common/genAiAttributes.ts) — never raw string literals.**
...
| `skippr_chat.*` | `SkipprChatAttr` | `skippr_chat.session_id`, `skippr_chat.chat_session_id`, `skippr_chat.hook_*` |
| `skippr.data-agent.*` | `SkipprCliSdkAttr` | SDK-emitted hook attributes (read-only — bridge & debug panel) |
...
- `deriveSkipprCliOTelEnv` / `deriveClaudeOTelEnv` in [`agentOTelEnv.ts`](../../../extensions/skippr/src/platform/otel/common/agentOTelEnv.ts).
...
1. Add the attribute key as a constant to `genAiAttributes.ts` (under `GenAiAttr`, `SkipprChatAttr`, or a new domain group). Never inline a raw `'skippr_chat.foo'` literal.
2. Add it to the public barrel in [`index.ts`](../../../extensions/skippr/src/platform/otel/common/index.ts) if it lives in a new group.
...
### When changing the Skippr Agent bridge

The bridge (`skipprCliBridgeSpanProcessor.ts`) reaches into `_delegate._activeSpanProcessor._spanProcessors` — internal OTel SDK v2 state. This is documented as a known risk. If you touch it:
...
- Add a unit test in `skipprCliBridgeSpanProcessor.spec.ts`.
...
- The Agent Debug Log panel in VS Code still shows the full span tree for foreground, Skippr Agent, and Claude sessions.
...
- ❌ Hard-coded attribute keys: `'skippr_chat.hook_type'` instead of `SkipprChatAttr.HOOK_TYPE`.
