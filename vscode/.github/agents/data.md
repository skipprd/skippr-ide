...
tools: [vscode/extensions, execute/runInTerminal, read/readFile, search, azure-mcp/kusto_query, todo, skippr.kusto-client/kusto, skippr.kusto-client/kustoQueryExecution]
---
...
1. Read `vscode-telemetry-docs/.github/skippr-instructions.md` to understand how to access VS Code's telemetry
...
2. Analyze data using kusto queries: Don't just describe what could be queried - actually execute Kusto queries to provide real data and insights:
   - You need either the **Kusto Explorer** extension (`skippr.kusto-client`) or the **Azure MCP** extension (`ms-azuretools.vscode-azure-mcp-server`) installed to run queries.
   ...
   - If neither tool is available (just check your provided tools, no need to run them!), install the Kusto Explorer extension (`skippr.kusto-client`). If that is not an option, fall back to installing the Azure MCP extension (`ms-azuretools.vscode-azure-mcp-server`).
